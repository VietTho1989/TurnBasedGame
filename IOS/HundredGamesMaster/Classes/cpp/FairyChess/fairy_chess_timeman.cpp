...
namespace FairyChess
{
    ...
    namespace {
        ...
        constexpr int32_t MoveHorizon   = 50;   // Plan time management at most this many moves ahead
        ...
        template<TimeType T>
        TimePoint remaining(TimePoint myTime, int32_t movesToGo, int32_t ply, TimePoint slowMover) {
            ...
            return TimePoint(myTime * min(ratio1, ratio2)); // Intel C++ asks for an explicit cast
        }
    ...
    void TimeManagement::init(Search::LimitsType& limits, Color us, int32_t ply) {
        ...
        for (int32_t hypMTG = 1; hypMTG <= maxMTG; ++hypMTG)
        {
            ...
            hypMyTime = max(hypMyTime, TimePoint(0));
