...
namespace Shogi
{
    namespace {
        ...
        {
/*#ifdef Android
            __android_log_print(ANDROID_LOG_ERROR, "NativeCore", "generateBishopOrRookMoves\n");
#endif*/
...
/*#ifdef Android
            __android_log_print(ANDROID_LOG_ERROR, "NativeCore", "generateDropMoves\n");
#endif*/
            ...
            if (hand.exists<HPawn>()) {
                ...
                Square pawnsSquare;
                foreachBB(pawnsBB, pawnsSquare, [&](const int32_t part) {
                ...
                if (makeRank(ksq) != TRank9) {
                    ...
                    // TODO assert(isInSquare(pawnDropCheckSquare));
                    if(!(isInSquare(pawnDropCheckSquare))){
#ifdef Android
                        __android_log_print(ANDROID_LOG_ERROR, "NativeCore", "assert(isInSquare(pawnDropCheckSquare))\\n");
#else
                        printf("error, assert(isInSquare(pawnDropCheckSquare))\n");
#endif
                    }
            ...
            if (hand.exceptPawnExists()) {
                ...
                int32_t haveHandNum = 0; // 持ち駒の駒の種類の数
                
                // 桂馬、香車、それ以外の順番で格納する。(駒を打てる位置が限定的な順)
                if (hand.exists<HKnight>()) haveHand[haveHandNum++] = Knight;
                const int32_t noKnightIdx      = haveHandNum; // 桂馬を除く駒でループするときのループの初期値
                if (hand.exists<HLance >()) haveHand[haveHandNum++] = Lance;
                const int32_t noKnightLanceIdx = haveHandNum; // 桂馬, 香車を除く駒でループするときのループの初期値
                ...
                switch (haveHandNum - noKnightLanceIdx) {
                    ...
                    case 1: toBB = target & TRank1BB; FOREACH_BB(toBB, to, { Unroller<1>()([&](const int32_t i) { (*moveList++).move = makeDropMove(haveHand[noKnightLanceIdx + i], to); }); }); break;
                    case 2: toBB = target & TRank1BB; FOREACH_BB(toBB, to, { Unroller<2>()([&](const int32_t i) { (*moveList++).move = makeDropMove(haveHand[noKnightLanceIdx + i], to); }); }); break;
                    case 3: toBB = target & TRank1BB; FOREACH_BB(toBB, to, { Unroller<3>()([&](const int32_t i) { (*moveList++).move = makeDropMove(haveHand[noKnightLanceIdx + i], to); }); }); break;
                    case 4: toBB = target & TRank1BB; FOREACH_BB(toBB, to, { Unroller<4>()([&](const int32_t i) { (*moveList++).move = makeDropMove(haveHand[noKnightLanceIdx + i], to); }); }); break;
                    default: UNREACHABLE;
                }
                ...
                switch (haveHandNum - noKnightIdx) {
                    ...
                    case 1: toBB = target & TRank2BB; FOREACH_BB(toBB, to, { Unroller<1>()([&](const int32_t i) { (*moveList++).move = makeDropMove(haveHand[noKnightIdx + i], to); }); }); break;
                    case 2: toBB = target & TRank2BB; FOREACH_BB(toBB, to, { Unroller<2>()([&](const int32_t i) { (*moveList++).move = makeDropMove(haveHand[noKnightIdx + i], to); }); }); break;
                    case 3: toBB = target & TRank2BB; FOREACH_BB(toBB, to, { Unroller<3>()([&](const int32_t i) { (*moveList++).move = makeDropMove(haveHand[noKnightIdx + i], to); }); }); break;
                    case 4: toBB = target & TRank2BB; FOREACH_BB(toBB, to, { Unroller<4>()([&](const int32_t i) { (*moveList++).move = makeDropMove(haveHand[noKnightIdx + i], to); }); }); break;
                    case 5: toBB = target & TRank2BB; FOREACH_BB(toBB, to, { Unroller<5>()([&](const int32_t i) { (*moveList++).move = makeDropMove(haveHand[noKnightIdx + i], to); }); }); break;
                    default: UNREACHABLE;
                }
                ...
                switch (haveHandNum) {
                    case 0:
                        // TODO assert(false);
#ifdef Android
                        __android_log_print(ANDROID_LOG_ERROR, "NativeCore", "error, assert(false)\n");
#else
                        printf("error, assert(false)\n");
#endif
                        break; // 最適化の為のダミー
                    case 1:
                        FOREACH_BB(toBB, to, { Unroller<1>()([&](const int32_t i) { (*moveList++).move = makeDropMove(haveHand[i], to); }); });
                        break;
                    case 2:
                        FOREACH_BB(toBB, to, { Unroller<2>()([&](const int32_t i) { (*moveList++).move = makeDropMove(haveHand[i], to); }); });
                        break;
                    case 3:
                        FOREACH_BB(toBB, to, { Unroller<3>()([&](const int32_t i) { (*moveList++).move = makeDropMove(haveHand[i], to); }); });
                        break;
                    case 4:
                        FOREACH_BB(toBB, to, { Unroller<4>()([&](const int32_t i) { (*moveList++).move = makeDropMove(haveHand[i], to); }); });
                        break;
                    case 5:
                        FOREACH_BB(toBB, to, { Unroller<5>()([&](const int32_t i) { (*moveList++).move = makeDropMove(haveHand[i], to); }); });
                        break;
                    case 6:
                        FOREACH_BB(toBB, to, { Unroller<6>()([&](const int32_t i) { (*moveList++).move = makeDropMove(haveHand[i], to); }); });
                        break;
                    default:
                        UNREACHABLE;
                }
...
/*#ifdef Android
                __android_log_print(ANDROID_LOG_ERROR, "NativeCore", "GeneratePieceMoves operator\n");
#endif*/
        ...
        template <MoveType MT, Color US, bool ALL> struct GeneratePieceMoves<MT, Pawn, US, ALL> {
            FORCE_INLINE ExtMove* operator () (ExtMove* moveList, const Position& pos, const Bitboard& target, const Square /*ksq*/) {
/*#ifdef Android
                __android_log_print(ANDROID_LOG_ERROR, "NativeCore", "GeneratePieceMoves<MT, Pawn, US, ALL>\n");
#endif*/
                ...
                if (MT != NonCaptureMinusPro) {
                    ...
                    // TODO assert(!(target & TRank123BB));
                    if(target & TRank123BB){
                        printf("error, assert(!(target & TRank123BB))\n");
                    }
                }
        ...
        template <MoveType MT, Color US, bool ALL> struct GeneratePieceMoves<MT, Lance, US, ALL> {
            FORCE_INLINE ExtMove* operator () (ExtMove* moveList, const Position& pos, const Bitboard& target, const Square /*ksq*/) {
/*#ifdef Android
                __android_log_print(ANDROID_LOG_ERROR, "NativeCore", "GeneratePieceMoves<MT, Lance, US, ALL>\n");
#endif*/
        ...
        template <MoveType MT, Color US, bool ALL> struct GeneratePieceMoves<MT, Knight, US, ALL> {
            FORCE_INLINE ExtMove* operator () (ExtMove* moveList, const Position& pos, const Bitboard& target, const Square /*ksq*/) {
/*#ifdef Android
                __android_log_print(ANDROID_LOG_ERROR, "NativeCore", "GeneratePieceMoves<MT, Knight, US, ALL>\n");
#endif*/
        ...
        template <MoveType MT, Color US, bool ALL> struct GeneratePieceMoves<MT, Silver, US, ALL> {
            FORCE_INLINE ExtMove* operator () (ExtMove* moveList, const Position& pos, const Bitboard& target, const Square /*ksq*/) {
/*#ifdef Android
                __android_log_print(ANDROID_LOG_ERROR, "NativeCore", "GeneratePieceMoves<MT, Silver, US, ALL>\n");
#endif*/
        ...
        template <MoveType MT, Color US, bool ALL> struct GeneratePieceMoves<MT, Bishop, US, ALL> {
            FORCE_INLINE ExtMove* operator () (ExtMove* moveList, const Position& pos, const Bitboard& target, const Square ksq) {
/*#ifdef Android
                __android_log_print(ANDROID_LOG_ERROR, "NativeCore", "GeneratePieceMoves<MT, Bishop, US, ALL>\n");
#endif*/
        ...
        template <MoveType MT, Color US, bool ALL> struct GeneratePieceMoves<MT, Rook, US, ALL> {
            FORCE_INLINE ExtMove* operator () (ExtMove* moveList, const Position& pos, const Bitboard& target, const Square ksq) {
/*#ifdef Android
                __android_log_print(ANDROID_LOG_ERROR, "NativeCore", "GeneratePieceMoves<MT, Rook, US, ALL>\n");
#endif*/
        ...
        template <MoveType MT, Color US, bool ALL> struct GeneratePieceMoves<MT, King, US, ALL> {
            FORCE_INLINE ExtMove* operator () (ExtMove* moveList, const Position& pos, const Bitboard& target, const Square /*ksq*/) {
/*#ifdef Android
                __android_log_print(ANDROID_LOG_ERROR, "NativeCore", "GeneratePieceMoves<MT, King, US, ALL>\n");
#endif*/
...
/*#ifdef Android
            __android_log_print(ANDROID_LOG_ERROR, "NativeCore", "generateRecaptureMoves\n");
#endif*/
...
/*#ifdef Android
                __android_log_print(ANDROID_LOG_ERROR, "NativeCore", "struct GenerateMoves\n");
#endif*/
...
/*#ifdef Android
                __android_log_print(ANDROID_LOG_ERROR, "NativeCore", "GenerateMoves<Drop, US>\n");
#endif*/
        ...
        {
/*#ifdef Android
            __android_log_print(ANDROID_LOG_ERROR, "NativeCore", "makeBannedKingTo\n");
#endif*/
            switch (pos.piece(checkSq)) {
                ...
                case (THEM == Black ? BKnight    : WKnight):
                    ...
                    if(!(pos.piece(checkSq) == (THEM == Black ? BPawn   : WPawn) ||
                         pos.piece(checkSq) == (THEM == Black ? BKnight : WKnight))){
                        printf("error, assert(pos.piece(checkSq) == (THEM == Black ? BPawn   : WPawn) ||pos.piece(checkSq) == (THEM == Black ? BKnight : WKnight))");
                    }
                    break;
        ...
        template <Color US, bool ALL> struct GenerateMoves<Evasion, US, ALL> {
            /*FORCE_INLINE*/ ExtMove* operator () (ExtMove* moveList, const Position& pos) {
/*#ifdef Android
                __android_log_print(ANDROID_LOG_ERROR, "NativeCore", "GenerateMoves<Evasion, US, ALL>\n");
#endif*/
                // TODO assert(pos.isOK());
                if(!(pos.isOK())){
                    printf("error, assert(pos.isOK())\n");
                }
                // TODO assert(pos.inCheck());
                if(!(pos.inCheck())){
                    printf("error, pos.inCheck()\n");
                }
                ...
                do {
                    ...
                    // TODO assert(pieceToColor(pos.piece(checkSq)) == Them);
                    if(!(pieceToColor(pos.piece(checkSq)) == Them)){
                        printf("error, assert(pieceToColor(pos.piece(checkSq)) == Them)\n");
                    }
        ...
        template <Color US> struct GenerateMoves<NonEvasion, US> {
            /*FORCE_INLINE*/ ExtMove* operator () (ExtMove* moveList, const Position& pos) {
/*#ifdef Android
                __android_log_print(ANDROID_LOG_ERROR, "NativeCore", "GenerateMoves<NonEvasion, US>\n");
#endif*/
        ...
        template <Color US> struct GenerateMoves<Legal, US> {
            FORCE_INLINE ExtMove* operator () (ExtMove* moveList, const Position& pos) {
/*#ifdef Android
                __android_log_print(ANDROID_LOG_ERROR, "NativeCore", "GenerateMoves<Legal, US>\n");
#endif*/
        ...
        template <Color US> struct GenerateMoves<LegalAll, US> {
            FORCE_INLINE ExtMove* operator () (ExtMove* moveList, const Position& pos) {
/*#ifdef Android
                __android_log_print(ANDROID_LOG_ERROR, "NativeCore", "GenerateMoves<LegalAll, US>\n");
#endif*/
                ExtMove* curr = moveList;
                const Bitboard pinned = pos.pinnedBB();
...
/*#ifdef Android
        __android_log_print(ANDROID_LOG_ERROR, "NativeCore", "generateMoves(ExtMove* moveList, const Position& pos, const Square to)\n");
#endif*/
