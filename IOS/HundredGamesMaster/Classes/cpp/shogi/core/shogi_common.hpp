...
#include <intrin.h>
    FORCE_INLINE int32_t firstOneFromLSB(const u64 b) {
        unsigned long index;
        _BitScanForward64(&index, b);
        return index;
    }
    FORCE_INLINE int32_t lsb(const u64 b) {
        return firstOneFromLSB(b);
    }
    FORCE_INLINE int32_t firstOneFromMSB(const u64 b) {
        unsigned long index;
        _BitScanReverse64(&index, b);
        return 63 - index;
    }
    FORCE_INLINE int32_t msb(const u64 b) {
...
#include <nmmintrin.h>
    inline int32_t count1s(u64 x) {
        return _mm_popcnt_u64(x);
    }
#else
    inline int32_t count1s(u64 x) //任意の値の1のビットの数を数える。( x is not a const value.)
    {
    ...
    // 2進表示
    template <typename T>
    inline std::string putb(const T value, const int32_t msb = sizeof(T)*8 - 1, const int32_t lsb = 0) {
        std::string str;
        u64 tempValue = (static_cast<u64>(value) >> lsb);
        
        for (int32_t length = msb - lsb + 1; length; --length)
...
#undef SYNCENDL
    ...
    // Unroller<5>()([&](const int32_t i){std::cout << i << std::endl;});
    template <int32_t N> struct Unroller {
