...
#include "../../Platform.h"
#include "shogi_overloadEnumOperators.hpp"
...
namespace Shogi {
    ...
    inline EvalIndex inverseFileIndexOnBoard(const EvalIndex index, const EvalIndex indexBegin) {
        // TODO assert(f_pawn <= index);
        if(!(f_pawn <= index)){
            printf("error, assert(f_pawn <= index)\n");
        }
        
        // TODO assert(kppIndexBegin(index) == indexBegin); // この前提で引数を取る。
        if(!(kppIndexBegin(index) == indexBegin)){
            printf("error, assert(kppIndexBegin(index) == indexBegin)\n");
        }
    ...
    template <typename EvalElementType> struct EvaluatorBase {
        ...
        int64_t minKPPIndex(Square ksq, EvalIndex i, EvalIndex j) {
            ...
            auto p = min({&kpps.kpp[ksq][i][j], &kpps.kpp[ksq][j][i], &kpps.kpp[invk][invi][invj], &kpps.kpp[invk][invj][invi]});
...
#pragma omp for
    ...
    struct Evaluator /*: public EvaluatorBase<EvalElementType>*/ {
        ...
        static void init(const std::string& dirName) {
            printf("evaluate init: %s\n", dirName.c_str());
            if (!allocated) {
                allocated = true;
#ifdef Android
                __android_log_print(ANDROID_LOG_ERROR, "NativeCore", "evaluate init\n");
#else
                printf("evaluate init\n");
#endif
                KPP = (KPPEvalElementType1*)calloc(SquareNum, sizeof(KPPEvalElementType1));
                KKP = (KKPEvalElementType1*)calloc(SquareNum, sizeof(KKPEvalElementType1));
                printf("size KPP: %lu\n", sizeof(KPPEvalElementType1) * (size_t)SquareNum);
