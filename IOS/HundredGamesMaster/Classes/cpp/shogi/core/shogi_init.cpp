...
namespace Shogi
{
    namespace {
        ...
        // result  occupied
        Bitboard indexToOccupied(const int32_t index, const int32_t bits, const Bitboard& blockMask) {
        ...
        void initAttacks(const bool isBishop)
        {
            ...
            for (Square sq = SQ11; sq < SquareNum; ++sq) {
                ...
                const int32_t num1s = (isBishop ? BishopBlockBits[sq] : RookBlockBits[sq]);
        ...
        void initLanceAttacks() {
            for (Color c = Black; c < ColorNum; ++c) {
                for (Square sq = SQ11; sq < SquareNum; ++sq) {
                    ...
                    //const int32_t num1s = blockMask.popCount(); // 常に 7
                    const int32_t num1s = 7;
                    
                    // TODO assert(num1s == blockMask.popCount());
                    if(!(num1s == blockMask.popCount())){
                        printf("error, assert(num1s == blockMask.popCount()\n");
                    }
        ...
        void initEvalIndex() {
            ...
            for (EvalIndex i = (EvalIndex)0; i < fe_end; ++i) {
                ...
                // TODO assert(KPPIndexIsBlackArray[i] == kppIndexIsBlack(i));
                if(!(KPPIndexIsBlackArray[i] == kppIndexIsBlack(i))){
                    printf("error, assert(KPPIndexIsBlackArray[i] == kppIndexIsBlack(i))\n");
                }
            }
        }
    }
    ...
    u64 findMagic(const Square square, const bool isBishop) {
        ...
        int32_t num1s = (isBishop ? BishopBlockBits[square] : RookBlockBits[square]);
        ...
        for (u64 k = 0; k < UINT64_C(100000000); ++k) {
            ...
            for (int32_t i = 0; !fail && i < (1 << num1s); ++i) {
                const int32_t shiftBits = (isBishop ? BishopShiftBits[square] : RookShiftBits[square]);
