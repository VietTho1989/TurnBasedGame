...
namespace Shogi {
    ...
    struct LimitsType {
        ...
        int32_t time[ColorNum], inc[ColorNum], movesToGo, depth, moveTime, mate, infinite, ponder;
        s64 nodes;
        Timer startTime;
    };
    ...
    struct Stats {
        ...
        void update(const Piece pc, const Square to, const Score s) {
            if (std::abs(int(s)) >= 324)
                return;
            table[pc][to] -= table[pc][to] * std::abs(int(s)) / (CM ? 936 : 324);
    ...
    struct FromToStats {
        ...
        void update(const Color c, const Move m, const Score s) {
            ...
            table[c][from][to] -= table[c][from][to] * std::abs(int(s)) / 324;
    ...
    struct Thread {
        ...
        virtual void search();
        void mySearch(int32_t level, int32_t mr);
