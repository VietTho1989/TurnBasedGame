...
namespace NMM
{
    Board::Board(void) : num_white_pieces(0), num_black_pieces(0)
    {
    ...
    bool Board::is_placeable(const int32_t &pos, const bool &white_piece, bool &mill)
    {
        mill = false;
    ...
    bool Board::is_movable(const int32_t &pos, const SpotStatus &ss, const NMMAction &action, bool &mill, bool &finish, bool &white_win)
    {
        mill = finish = white_win = false;
        if(pos < 0 || pos >= BOARD_SPOT)
            return false;
        if(spot[pos]->status != ss)
            return false;
    ...
    bool Board::is_flyable(const int32_t &from, const int32_t &to, const SpotStatus &ss, bool &mill, bool &finish, bool &white_win)
    {
        mill = finish = white_win = false;
        if(from < 0 || from >= BOARD_SPOT || to < 0 || to >= BOARD_SPOT)
            return false;
        if(spot[from]->status != ss)
            return false;
    ...
    bool Board::is_removable(const int32_t &pos, const SpotStatus &ss, bool &finish, bool &white_win, const bool allmill)
    {
        ...
        int32_t pieces_left = spot[pos]->status == SS_White ? num_white_pieces : num_black_pieces;
    ...
    void Board::place_spot(const int32_t &pos, const bool &white_piece)
    {
        if(pos < 0 || pos >= BOARD_SPOT)
        {
    ...
    void Board::fly_spot(const int32_t &from, const int32_t &to)
    {
        if(from < 0 || from >= BOARD_SPOT || to < 0 || to >= BOARD_SPOT)
        {
    ...
    void Board::remove_spot(const int32_t &pos)
    {
        if(pos < 0 || pos >= BOARD_SPOT)
        {
    ...
    Spot* Board::get_spot(const int32_t &pos)
    {
        if(pos < 0 || pos >= BOARD_SPOT)
        {
    ...
    int32_t Board::in_mill(const int32_t &pos, const int32_t &sts)
    {
        return in_mill_v(pos, sts) + in_mill_h(pos, sts);
    }
    
    int32_t Board::in_mill_v(const int32_t &pos, const int32_t &sts)
    {
    ...
    int32_t Board::in_mill_h(const int32_t &pos, const int32_t &sts)
    {
        // se estado verificado é vazio, então não tem mill
        if(sts == SS_Empty)
            return 0;
            ...
            // Spot* spot[BOARD_SPOT]
            {
                ...
                for(int32_t i=0; i<BOARD_SPOT; i++){
                    if(pointerIndex+size<=length){
                        memcpy(ret + pointerIndex, &board->spot[i]->status, size);
                        pointerIndex+=size;
                    }else{
        ...
        while (pointerIndex < length) {
            ...
            switch (index) {
                case 0:
                {
                    ...
                    for(int32_t i=0; i<BOARD_SPOT; i++){
                        if(pointerIndex+size<=length){
                            memcpy(&board->spot[i]->status, bytes + pointerIndex, size);
                            pointerIndex+=size;
                        }else{
                    ...
                    // find number of piece
                    int32_t num_white_pieces = 0;
                    int32_t num_black_pieces = 0;
                    {
                        for(int32_t i=0; i<BOARD_SPOT; i++){
                            switch (board->spot[i]->status) {
                                case SS_Empty:
                                    break;
                                case SS_Black:
                                    num_black_pieces++;
                    ...
                    board->num_white_pieces = num_white_pieces;
                    board->num_black_pieces = num_black_pieces;
                }
                    break;
                default:
                {
