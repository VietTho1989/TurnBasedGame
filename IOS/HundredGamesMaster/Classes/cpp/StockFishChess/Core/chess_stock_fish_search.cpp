...
namespace StockFishChess
{
    ...
    namespace {
        ...
        template <NodeType NT, bool InCheck> Value qsearch(Position& pos, Stack* ss, Value alpha, Value beta, Depth depth = DEPTH_ZERO);
        ...
        void update_capture_stats(const Position& pos, Move move, Move* captures, int32_t captureCnt, int32_t bonus);
        ...
        for (int32_t imp = 0; imp <= 1; ++imp)
            for (int32_t d = 1; d < 64; ++d)
                for (int32_t mc = 1; mc < 64; ++mc) {
                    ...
                    Reductions[NonPV][imp][d][mc] = int32_t(std::round(r));
    ...
    void Thread::search() {
        ...
        while (   (rootDepth += ONE_PLY) < DEPTH_MAX
            ...
            for (PVIdx = 0; PVIdx < multiPV && !stop; ++PVIdx) {
                ...
                // high/low anymore.
                while (true) {
                    bestValue = StockFishChess::search<PV>(rootPos, ss, alpha, beta, rootDepth, false, false);
    ...
    namespace {
        ...
        template <NodeType NT> Value search(Position& pos, Stack* ss, Value alpha, Value beta, Depth depth, bool cutNode, bool skipEarlyPruning) {
            ...
            if (inCheck) {
                ...
                eval = ss->staticEval = (ss-1)->currentMove != MOVE_NULL ? evaluate(pos) : -(ss-1)->staticEval + 2 * Eval::Tempo;
                
                tte->save(posKey, VALUE_NONE, BOUND_NONE, DEPTH_NONE, MOVE_NONE, ss->staticEval, pos.thisThread->TT.generation());
            }
            ...
            if (   !PvNode
                ...
                if (nullValue >= beta) {
                    ...
                    int32_t nmp_ply = thisThread->nmp_ply;
                    int32_t pair = thisThread->pair;
        ...
        moves_loop: // When in check search starts from here
            ...
            while ((move = mp.next_move(skipQuiets)) != MOVE_NONE) {
                ...
                if ( !rootNode && pos.non_pawn_material(pos.side_to_move()) && bestValue > VALUE_MATED_IN_MAX_PLY) {
        ...
        template <NodeType NT, bool InCheck> Value qsearch(Position& pos, Stack* ss, Value alpha, Value beta, Depth depth) {
            // printf("qSearch\n");
            const bool PvNode = NT == PV;
            
            // assert(InCheck == bool(pos.checkers()));
            if(!(InCheck == bool(pos.checkers()))){
                printf("error, assert(InCheck == bool(pos.checkers()))\n");
            }
            ...
            if (pos.is_draw(ss->ply) || ss->ply >= MAX_PLY){
                // printf("Check for an instant draw or if the maximum ply has been reached\n");
                Value v = ss->ply >= MAX_PLY && !InCheck ? evaluate(pos) : VALUE_DRAW;
                if(v==VALUE_DRAW){
                    // printf("qSearch value draw\n");
                }
                return v;
            }
        ...
        Move Skill::pick_best(Thread* th, size_t multiPV) {
            ...
            if(rootMoves.size()>0){
                ...
                for (size_t i = 0; i < multiPV; ++i) {
                    ...
                    int32_t push = (weakness * int(topScore - rootMoves[i].score) + delta * (rng.rand<unsigned>() % weakness)) / 128;
                    // printf("pick best: multiPV: %zu, %d, %d\n", multiPV, push, weakness);
                ...
                printf("pick_best: don't have any rootMoves\n");
            }
            return best;
