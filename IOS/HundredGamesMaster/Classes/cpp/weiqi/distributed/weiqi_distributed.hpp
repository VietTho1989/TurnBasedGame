...
#ifndef weiqi_distributed_hpp
#define weiqi_distributed_hpp
...
int32_t mask = hash_mask(hash_bits); \
int32_t delta = (int)((path) >> (hash_bits)) | 1; \
hash = ((int)(path) ^ delta ^ (delta >> (hash_bits))) & mask; \
path_t cp = (table)[hash].coord_path; \
found = (cp == path); \
if (found | !cp) break; \
int32_t tries = 1 << ((hash_bits)-2); \
do { \
