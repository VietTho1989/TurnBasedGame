...
namespace Makruk
{
    ...
    void Thread::search() {
        ...
        while (   (rootDepth += ONE_PLY) < DEPTH_MAX
            ...
            for (PVIdx = 0; PVIdx < multiPV && !stop; ++PVIdx) {
                ...
                // high/low anymore.
                while (true) {
                    bestValue = Makruk::search<PV>(rootPos, ss, alpha, beta, rootDepth, false, false);
    ...
    namespace {
        ...
        template <NodeType NT> Value search(Position& pos, Stack* ss, Value alpha, Value beta, Depth depth, bool cutNode, bool skipEarlyPruning) {
            ...
            ttValue = ttHit ? value_from_tt(tte!=NULL ? tte->value() : VALUE_ZERO, ss->ply) : VALUE_NONE;
