        ...
        public bool processEvent(Event e)
        {
            bool isProcess = false;
            {
                ...
                if (!isProcess)
                {
                    ...
                    {
                        RoomHolder roomHolder = this.findCallBack<RoomHolder>();
                        if (roomHolder != null)
                        {
                            isProcess = roomHolder.useShortKey(e);
                        }
                        else
                        {
                            Debug.LogError("roomHolder null: " + this);
                        }
                    }
                }
            }
                ...
                if (room != null)
                {
                    ...
                    // btnDelete
                    {
                        if (btnDelete != null)
                        {
                            ...
                            {
                                // find you are server admin
                                bool isYouServerAdmin = false;
                                {
                                    User profileUser = Server.GetProfileUser(room);
                                    if (profileUser != null)
                                    {
                                        if (profileUser.role.v == User.Role.Admin)
                                        {
                                            isYouServerAdmin = true;
                                        }
                                    }
                                    else
                                    {
                                ...
                                if (isYouServerAdmin)
                                {
                                    if (room.state.v.getType() != Room.State.Type.End)
                                    {
                                        canDelete = true;
                                    }
                                }
                            }
                            btnDelete.gameObject.SetActive(canDelete);
                        }
                        else
                        {
    ...
    public void onClickBtnDelete()
    {
        if (this.data != null)
        {
            ...
            if (room != null)
            {
                ...
                if (roomListUIData != null)
                {
                    RoomAdminBtnDeleteUI.UIData roomAdminBtnDeleteUIData = roomListUIData.sub.newOrOld<RoomAdminBtnDeleteUI.UIData>();
                    {
                        roomAdminBtnDeleteUIData.room.v = new ReferenceData<Room>(room);
                    }
                    roomListUIData.sub.v = roomAdminBtnDeleteUIData;
                }
                else
                {
                    Debug.LogError("roomListUIData null");
                }
            }
            else
            {
