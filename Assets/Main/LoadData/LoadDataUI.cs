...
public class LoadDataUI : UIBehavior<LoadDataUI.UIData>
{
	...
	static LoadDataUI()
	{
		...
		txtCannotLoad.add (Language.Type.vi, "Không thể tải, không chọn file");
		...
		txtLoadRecord.add (Language.Type.vi, "Tải Bản Ghi");
		txtCannotLoadRecord.add (Language.Type.vi, "Không thể tải bản ghi, không chọn file");
		txtLoadingRecord.add (Language.Type.vi, "Đang tải bản ghi...");
		txtLoadRecordSuccess.add (Language.Type.vi, "Tải bản ghi thành công");
		txtLoadRecordFail.add (Language.Type.vi, "Tải bản ghi thất bại");
	}
	...
	public Transform viewSaveDataContainer;

	public ViewRecordUI viewRecordPrefab;
	public Transform viewRecordContainer;
		...
		// Child
		{
			...
			// loadData
			{
				...
				if (data is ViewSaveDataUI.UIData) {
					...
					// UI
					{
						UIUtils.Instantiate (viewSaveDataUIData, viewSaveDataPrefab, viewSaveDataContainer);
					}
					dirty = true;
					return;
				}
			...
			// loadRecord
			{
				...
				if (data is ViewRecordUI.UIData) {
					...
					// UI
					{
						UIUtils.Instantiate (viewRecordUIData, viewRecordPrefab, viewRecordContainer);
					}
					dirty = true;
					return;
				}
