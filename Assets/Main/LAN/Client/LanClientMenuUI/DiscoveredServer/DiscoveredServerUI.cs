...
public class DiscoveredServerUI : UIBehavior<DiscoveredServer>
{
	...
	public static readonly TxtLanguage txtSameVersion = new TxtLanguage();
	public static readonly TxtLanguage txtDifferentVersion = new TxtLanguage ();
	public static readonly TxtLanguage txtIpAddress = new TxtLanguage();
	public static readonly TxtLanguage txtPlayers = new TxtLanguage();
			...
			if (this.data != null) {
				...
				if (tvVersion != null) {
					...
					if (Global.VersionCode == version) {
						tvVersion.text = txtSameVersion.get ("Same version") + ": " + version;
					} else {
						tvVersion.text = txtDifferentVersion.get ("Different version") + ": " + version;
					}
				} else {
				...
				// tvIp
				if (tvIp != null) {
					tvIp.text = txtIpAddress.get ("Ip Address") + ": " + this.data.ipAddress.v;
				} else {
					Debug.LogError ("tvIp null: " + this);
				}
				// tvPlayers
				if (tvPlayers != null) {
					tvPlayers.text = txtPlayers.get ("Players") + ": " + this.data.player.v;
				} else {
