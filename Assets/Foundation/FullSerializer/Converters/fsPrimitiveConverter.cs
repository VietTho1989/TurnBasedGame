...
namespace FullSerializer.Internal {
    public class fsPrimitiveConverter : fsConverter {
        ...
        public override fsResult TryDeserialize(fsData storage, ref object instance, Type storageType) {
			...
			// TODO cai nay them vao
			/*if (UseUInt64 (storageType)) {
				// Exception: fsPrimitiveConverter expected number but got UInt64 in 18441976591291514880
				if (storage.IsUInt64 || storage.IsInt64) {
					instance = Convert.ChangeType (storage.AsUInt64, storageType);
				} else {
					return fsResult.Fail(GetType().Name + " expected UInt64 number but got " + storage.Type + " in " + storage);
				}
				return fsResult.Success;
			}*/
