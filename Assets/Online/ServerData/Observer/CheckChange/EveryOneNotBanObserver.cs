...
public class EveryOneNotBanObserver : GameObserver.CheckChange
{

    public EveryOneNotBanObserver(GameObserver gameObserver) : base(gameObserver)
    {
    ...
    public override void refreshObserverConnections()
    {
        ...
        if (this.data != null)
        {
            for (int i = this.data.users.vs.Count - 1; i >= 0; i--)
            {
                ...
                {
                    Debug.LogError("user is banned: " + user + "; " + this);
                }
            }
