            ...
            ConnectionInfo connectionInfo = null;
            {
                if (!connectionDict.dict.TryGetValue(connection, out connectionInfo))
                {
                    ...
                    connectionInfo = new ConnectionInfo();
                    {
                        ...
                        {
                            byte byteError = 0;
                            connectionInfo.outPacket = NetworkTransport.GetIncomingPacketCount(connection.hostId, connection.connectionId, out byteError);
                        }
                    }
                ...
                foreach (NetworkConnection connection in dictConnections.dict.Keys)
                {
                    ...
                    ConnectionInfo connectionInfo = dictConnections.dict[connection];
                    {
                        ...
                        else
                        {
                            ...
                            {
                                int newOutPacket = NetworkTransport.GetIncomingPacketCount(connection.hostId, connection.connectionId, out byteError);
