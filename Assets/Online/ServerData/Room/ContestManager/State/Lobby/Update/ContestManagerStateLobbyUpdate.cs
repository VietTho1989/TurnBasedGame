    ...
    public class ContestManagerStateLobbyUpdate : UpdateBehavior<ContestManagerStateLobby>
    {
        ...
        public override void update()
        {
            if (dirty)
            {
                ...
                if (this.data != null)
                {
                    Debug.LogError("ContestManagerStateLobbyUpdate: fast start: " + this.data.isFastStart);
                    if (this.data.isFastStart)
                    {
                        this.data.isFastStart = false;
                        if(this.data.contentFactory.v is SingleContestFactory)
                        {
                            SingleContestFactory singleContestFactory = this.data.contentFactory.v as SingleContestFactory;
                            singleContestFactory.isFastStart = true;
                            // Update
                            {
                                SingleContestFactoryUpdate singleContestFactoryUpdate = singleContestFactory.findCallBack<SingleContestFactoryUpdate>();
                                if (singleContestFactoryUpdate != null)
                                {
                                    singleContestFactoryUpdate.makeDirty();
                                }
                                else
                                {
                                    Debug.LogError("singleContestFactoryUpdate null");
                                }
                            }
                        }
                    }
