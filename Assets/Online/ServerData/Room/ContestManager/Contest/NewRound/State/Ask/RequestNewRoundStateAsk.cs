...
{
	public class RequestNewRoundStateAsk : RequestNewRound.State
	{
			...
			if (needRequest.canRequest) {
				if (!needRequest.needIdentity) {
					...
					if (DataIdentity.clientMap.TryGetValue (this, out dataIdentity)) {
						if (dataIdentity is RequestNewRoundStateAskIdentity) {
							RequestNewRoundStateAskIdentity requestNewRoundStateAskIdentity = dataIdentity as RequestNewRoundStateAskIdentity;
							requestNewRoundStateAskIdentity.requestAccept(userId);
						} else {
			...
			if (needRequest.canRequest) {
				if (!needRequest.needIdentity) {
					...
					if (DataIdentity.clientMap.TryGetValue (this, out dataIdentity)) {
						if (dataIdentity is RequestNewRoundStateAskIdentity) {
							RequestNewRoundStateAskIdentity requestNewRoundStateAskIdentity = dataIdentity as RequestNewRoundStateAskIdentity;
							requestNewRoundStateAskIdentity.requestCancel(userId);
						} else {
							Debug.LogError ("Why isn't correct identity");
						}
					} else {
		...
		public RequestNewRoundStateAsk() : base()
		{
