							...
							int lastKeyY = 4;
							{
								...
								if (gameMove != null) {
									...
									case GameMove.Type.EnglishDraughtMove:
										{
											EnglishDraughtMove englishDraughtMove = gameMove as EnglishDraughtMove;
											{
												int index = (int)((englishDraughtMove.SrcDst.v >> 6) & 63);
												if (index >= 0 && index < Common.BoardLocTo32.Length) {
													long dest = Common.FlipX (Common.BoardLocTo32 [index]) + 1;
													Common.convertSquareToXY (dest, out lastKeyX, out lastKeyY);
												} else {
									...
									case GameMove.Type.EnglishDraughtCustomSet:
										{
											...
											lastKeyX = englishDraughtCustomSet.square.v % 8;
											lastKeyY = englishDraughtCustomSet.square.v / 8;
										}
										break;
									case GameMove.Type.EnglishDraughtCustomMove:
										{
											...
											lastKeyX = englishDraughtCustomMove.destSquare.v % 8;
											lastKeyY = englishDraughtCustomMove.destSquare.v / 8;
										}
										break;
			...
			if (this.data != null) {
				...
				bool isActive = false;
				{
					...
					if (useRuleInputUIData != null) {
						englishDraught = useRuleInputUIData.englishDraught.v.data;
				...
				if (isActive) {
					if (square >= 0 && square < 64) {
						...
						{
							byte piece = EnglishDraught.getPiece (englishDraught.Sqs.vs, square);
							// process
							switch (piece) {
							case Common.BPIECE:
							case Common.BKING:
								{
									if (englishDraught.getPlayerIndex () == 0) {
										isCorrectPiece = true;
									}
								}
								break;
							...
							case Common.WKING:
								{
									if (englishDraught.getPlayerIndex () == 1) {
										isCorrectPiece = true;
									}
								}
								break;
