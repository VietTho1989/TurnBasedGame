...
namespace Shatranj.UseRule
{
	public class LegalMoveUI : UIBehavior<LegalMoveUI.UIData>
	{
		...
		{
			public VP<ReferenceData<ShatranjMove>> legalMove;
			...
			public UIData() : base()
			{
				this.legalMove = new VP<ReferenceData<ShatranjMove>>(this, (byte)Property.legalMove, new ReferenceData<ShatranjMove>(null));
			}
		...
		public override void refresh ()
		{
			if (dirty) {
				...
				if (this.data != null) {
					...
					// Update
					{
						ShatranjMove legalMove = this.data.legalMove.v.data;
						if (legalMove != null) {
							// position
							{
								int fromX = 0;
							...
							// imgType
							if (imgType != null) {
								ShatranjMove.Move move = new ShatranjMove.Move (legalMove.move.v);
								switch (move.type) {
		...
		public override void onAddCallBack<T> (T data)
		{
			...
			if (data is ShatranjMove) {
				// ShatranjMove legalMove = data as ShatranjMove;
				{
