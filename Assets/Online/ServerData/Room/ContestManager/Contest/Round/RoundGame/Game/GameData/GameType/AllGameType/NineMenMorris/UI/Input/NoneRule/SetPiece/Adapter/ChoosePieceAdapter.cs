...
namespace NineMenMorris.NoneRule
{
    public class ChoosePieceAdapter : SRIA<ChoosePieceAdapter.UIData, ChoosePieceHolder.UIData>
    {
        ...
        public class UIData : BaseParams
        {
            ...
            [NonSerialized]
            public List<Common.SpotStatus> pieces = new List<Common.SpotStatus>();
        ...
        public override void refresh()
        {
            if (dirty)
            {
                if (this.Initialized)
                {
                    ...
                    if (this.data != null)
                    {
                        List<Common.SpotStatus> pieces = new List<Common.SpotStatus>();
                        // get
                        {
                            // find alreadySelectPiece
                            Common.SpotStatus alreadySelectPiece = Common.SpotStatus.SS_Empty;
                            {
                                ...
                                if (noneRuleInputUIData != null)
                                {
                                    ...
                                    if (nineMenMorris != null)
                                    {
                                        ...
                                        int square = 0;
                                        {
                                            ...
                                            square = Common.ConvertXYToPosition(x, y);
                                        }
                                        // find piece
                                        alreadySelectPiece = (Common.SpotStatus)nineMenMorris.pieceOn(square);
                                    }
                                    else
                                    {
                        ...
                        // Make list
                        {
                            ...
                            // Add or Remove
                            {
                                if (pieces.Count > min)
                                {
                                    ...
                                    List<Common.SpotStatus> addItems = pieces.GetRange(min, insertCount);
