...
public class WaitHumanInputUI : UIBehavior<WaitHumanInputUI.UIData>
{
	...
	public static readonly TxtLanguage txtYourTurn = new TxtLanguage();
	public static readonly TxtLanguage txtNotYourTurn = new TxtLanguage ();
				...
				if (waitHumanInput != null) {
					...
					if (tvHuman != null) {
						if (Server.getProfileUserId (waitHumanInput) == waitHumanInput.userId.v) {
							tvHuman.text = txtYourTurn.get("Your Turn");
						} else {
							tvHuman.text = txtNotYourTurn.get("Not Your Turn");
						}
					} else {
