...
namespace MineSweeper.NoneRule
{
	public class ChoosePieceAdapter : SRIA<ChoosePieceAdapter.UIData, ChoosePieceHolder.UIData>
	{
		...
		public class UIData : BaseParams
		{
			...
			[NonSerialized]
			public List<sbyte> pieces = new List<sbyte>();
		...
		public override void refresh ()
		{
			if (dirty) {
				if (this.Initialized) {
					dirty = false;
					if (this.data != null) {
						List<sbyte> pieces = new List<sbyte> ();
						// get
						{
							// find alreadySelectPiece
							sbyte alreadySelectPiece = -1;
							{
								...
								if (noneRuleInputUIData != null) {
									...
									if (mineSweeper != null) {
										...
										if (setPieceUIData != null) {
											alreadySelectPiece = mineSweeper.getPiece (setPieceUIData.square.v);
										} else {
							...
							{
								sbyte[] CanChoosePieces = {-1, 0 };
								foreach (sbyte piece in CanChoosePieces) {
						...
						// Make list
						{
							...
							// Add or Remove
							{
								if (pieces.Count > min) {
									...
									List<sbyte> addItems = pieces.GetRange (min, insertCount);
			...
			// Parent
			{
				// noneRuleInputUIData
				{
					if (data is NoneRuleInputUI.UIData) {
						...
						// Child
						{
							noneRuleInputUIData.mineSweeper.allAddCallBack (this);
						}
						dirty = true;
						return;
					}
			...
			// Parent
			{
				// noneRuleInputUIData
				{
					if (data is NoneRuleInputUI.UIData) {
						...
						// Child
						{
							noneRuleInputUIData.mineSweeper.allRemoveCallBack (this);
						}
						return;
					}
					// Child
			...
			// Parent
			{
				// noneRuleInputUIData
				{
					if (wrapProperty.p is NoneRuleInputUI.UIData) {
						...
						case NoneRuleInputUI.UIData.Property.mineSweeper:
							{
