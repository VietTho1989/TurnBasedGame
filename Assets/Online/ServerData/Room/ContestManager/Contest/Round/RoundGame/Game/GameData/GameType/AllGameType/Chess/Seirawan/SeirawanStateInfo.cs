...
namespace Seirawan
{
	public class SeirawanStateInfo : Data
	{
		...
		public SeirawanStateInfo() : base()
		{
			...
			this.gatesBB = new VP<ulong> (this, (byte)Property.gatesBB, 0);
		...
		public static byte[] convertToBytes(SeirawanStateInfo st)
		{
			byte[] byteArray;
		...
		public static int parse(SeirawanStateInfo st, byte[] byteArray, int start)
		{
				...
				case 8:
					{
						...
						int size = sizeof(ulong);
						if (count + size <= byteArray.Length) {
							st.gatesBB.v = BitConverter.ToUInt64 (byteArray, count);
							count += size;
						} else {
							Debug.LogError ("array not enough length: gatesBB: " + count + "; " + byteArray.Length);
							isParseCorrect = false;
						}
					}
					break;
