...
namespace Weiqi
{
    public class WeiqiMoveAnimation : MoveAnimation
    {
        ...
        public const float AppearDuration = 0.25f;
        public const float StandDuration = 0.25f;
        public const float CaptureDuration = 0.25f;

        public override void initDuration()
        {
            if (this.captureCoords.vs.Count == 0)
            {
                this.duration.v = StandDuration * AnimationManager.DefaultDuration;
            }
            else
            {
