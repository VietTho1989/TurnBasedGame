...
namespace Seirawan
{
	public class SeirawanAIIdentity : DataIdentity
	{
		...
		private NetData<SeirawanAI> netData = new NetData<SeirawanAI>();
		...
		public override void onAddCallBack<T> (T data)
		{
			if (data is SeirawanAI) {
				...
				// Set property
				{
					this.serialize (this.searchInfor, seirawanAI.makeSearchInforms ());
					this.depth = seirawanAI.depth.v;
					this.skillLevel = seirawanAI.skillLevel.v;
					this.duration = seirawanAI.duration.v;
				}
				// Observer
				{
					...
					if (observer != null) {
						...
						observer.setCheckChangeData (seirawanAI);
					} else {
		...
		public void requestChangeDepth(uint userId, int newDepth)
		{
			...
			if (clientConnect != null) {
				clientConnect.CmdSeirawanAIChangeDepth (this.netId, userId, newDepth);
			} else {
		...
		public void requestChangeSkillLevel(uint userId, int newSkillLevel)
		{
			ClientConnectIdentity clientConnect = ClientConnectIdentity.findYourClientConnectIdentity (this.netData.clientData);
			if (clientConnect != null) {
				clientConnect.CmdSeirawanAIChangeSkillLevel (this.netId, userId, newSkillLevel);
			} else {
				Debug.LogError ("Cannot find clientConnect: " + this);
			}
		}
		...
		public void requestChangeDuration(uint userId, long newDuration)
		{
			ClientConnectIdentity clientConnect = ClientConnectIdentity.findYourClientConnectIdentity (this.netData.clientData);
			if (clientConnect != null) {
				clientConnect.CmdSeirawanAIChangeDuration (this.netId, userId, newDuration);
			} else {
				Debug.LogError ("Cannot find clientConnect: " + this);
			}
		}
