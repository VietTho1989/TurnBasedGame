			...
			if (dirty) {
				...
				if (this.data != null) {
					...
					// Legal moves
					{
						...
						{
							// Get legal move list
							List<FairyChessMove> legalMoves = new List<FairyChessMove>();
							{
								ShowUI.UIData show = this.data.findDataInParent<ShowUI.UIData> ();
								ClickDestUI.UIData clickDestUIData = this.data.findDataInParent<ClickDestUI.UIData> ();
								if (show != null) {
									for (int i = 0; i < show.legalMoves.vs.Count; i++) {
										FairyChessMove legalMove = show.legalMoves.vs [i];
										// check correct
							...
							// Make UI
							{
								for (int legalMoveIndex = 0; legalMoveIndex < legalMoves.Count; legalMoveIndex++) {
									FairyChessMove legalMove = legalMoves [legalMoveIndex];
									...
									if (legalMoveUIData != null) {
										// legalMove
										legalMoveUIData.legalMove.v = new ReferenceData<FairyChessMove> (legalMove);
									} else {
						...
						foreach (LegalMoveUI.UIData legalMoveUIData in oldLegalMoves) {
							legalMoveUIData.legalMove.v = new ReferenceData<FairyChessMove> (null);
						}
					}
				} else {
							...
							if (isSelectSameColor) {
								...
								bool haveMoveSameClick = false;
								{
									...
									if (show != null && clickDestUIData!=null) {
										...
										for (int i = 0; i < show.legalMoves.vs.Count; i++) {
											FairyChessMove check = show.legalMoves.vs [i];
											if (FairyChessMove.IsClickCorrectPosition (check.move.v, clickDestUIData.x.v, 
