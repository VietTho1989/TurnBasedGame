...
public class WaitInputActionUI : UIBehavior<WaitInputActionUI.UIData>
{
	...
	{
		txtTitle.add (Language.Type.vi, "Đợi Nước Đi");
		...
		txtNotReceiveMove.add (Language.Type.vi, "Không nhận được nước đi nào cả");
	}
	...
	public override void refresh ()
	{
		if (dirty) {
			...
			if (this.data != null) {
				...
				if (waitInputAction != null) {
					...
					if (tvCheckLegalMove != null) {
						if (waitInputAction.inputs.vs.Count > 0) {
							...
							tvCheckLegalMove.text = txtNotReceiveMove.get ("Don't receive any move");
						}
					} else {
				...
				{
					if (lbTitle != null) {
						lbTitle.text = txtTitle.get ("Wait Input Action");
					} else {
	...
	public Transform clientInputUIContainer;
	...
	public override void onAddCallBack<T> (T data)
	{
		...
		// Child
		{
					...
					case WaitInputAction.Sub.Type.Human:
						{
							...
							UIUtils.Instantiate (waitHumanInputUIData, waitHumanInputUIPrefab, subContainer);
						}
						break;
					case WaitInputAction.Sub.Type.AI:
						{
							WaitAIInputUI.UIData waitAIInputUIData = sub as WaitAIInputUI.UIData;
							UIUtils.Instantiate (waitAIInputUIData, waitAIInputUIPrefab, subContainer);
						}
						break;
					default:
			...
			if (data is ClientInputUI.UIData) {
				...
				{
					UIUtils.Instantiate (clientInputUIData, clientInputUIPrefab, clientInputUIContainer);
				}
				dirty = true;
				return;
			}
