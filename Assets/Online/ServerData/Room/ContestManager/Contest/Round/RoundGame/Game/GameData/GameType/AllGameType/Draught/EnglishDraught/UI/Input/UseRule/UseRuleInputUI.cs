...
namespace EnglishDraught
{
	public class UseRuleInputUI : UIBehavior<UseRuleInputUI.UIData>
	{
		...
		public IEnumerator TaskGetLegalMoves()
		{
			...
			{
				if (this.data != null) {
					if (this.data.englishDraught.v.data != null) {
						englishDraught = this.data.englishDraught.v.data;
					} else {
						Debug.LogError ("englishDraught null: " + this);
					}
				} else {
			...
			if (englishDraught != null) {
				List<EnglishDraughtMove> legalMoves = new List<EnglishDraughtMove> ();
				// Find legal inputData in other thread
				var mtask = UnityTask.Run (() => { 
					legalMoves = Core.unityGetLegalMoves(englishDraught, Core.CanCorrect);
				});
				// Wait
				{
						...
						// legalMoves
						{
							...
							if (legalMoves.Count > 0) {
								for (int i = 0; i < legalMoves.Count; i++) {
									EnglishDraughtMove legalMove = legalMoves [i];
									{
								...
								// Make change
								{
									List<Change<EnglishDraughtMove>> changes = new List<Change<EnglishDraughtMove>> ();
									{
										ChangeAdd<EnglishDraughtMove> changeAdd = new ChangeAdd<EnglishDraughtMove> ();
										{
											changeAdd.index = 0;
		...
		public override void onAddCallBack<T> (T data)
		{
			...
			// EnglishDraught
			{
				if (data is EnglishDraught) {
					...
					// Child
					{
						englishDraught.addCallBackAllChildren (this);
					}
					dirty = true;
					haveChange = true;
					return;
		...
		public override void onRemoveCallBack<T> (T data, bool isHide)
		{
			...
			// EnglishDraught
			{
				if (data is EnglishDraught) {
					...
					// Child
					{
						englishDraught.removeCallBackAllChildren (this);
					}
					return;
				}
				// Child
