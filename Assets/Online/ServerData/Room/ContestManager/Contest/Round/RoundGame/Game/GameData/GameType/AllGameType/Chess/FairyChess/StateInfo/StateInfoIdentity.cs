...
namespace FairyChess
{
    public class StateInfoIdentity : DataIdentity
    {
        ...
        private void OnNonPawnMaterialChanged(SyncList<System.Int32>.Operation op, int index)
        {
            if (this.netData.clientData != null)
            {
        ...
        public SyncListInt checksGiven = new SyncListInt();

        private void OnChecksGiveChanged(SyncList<System.Int32>.Operation op, int index)
        {
            if (this.netData.clientData != null)
            {
                IdentityUtils.onSyncListChange(this.netData.clientData.checksGiven, this.checksGiven, op, index);
            }
            else
            {
                // Debug.LogError ("clientData null: " + this);
        ...
        #region unpromotedCapturedPiece

        [SyncVar(hook = "onChangeUnpromotedCapturedPiece")]
        public System.Int32 unpromotedCapturedPiece;

        public void onChangeUnpromotedCapturedPiece(System.Int32 newUnpromotedCapturedPiece)
        {
            this.unpromotedCapturedPiece = newUnpromotedCapturedPiece;
            if (this.netData.clientData != null)
            {
                this.netData.clientData.unpromotedCapturedPiece.v = newUnpromotedCapturedPiece;
            }
            else
            {
                // Debug.LogError ("clientData null: " + this);
        ...
        public SyncListUInt64 pinners = new SyncListUInt64();

        private void OnPinnersForKingChanged(SyncListUInt64.Operation op, int index)
        {
            if (this.netData.clientData != null)
            {
                IdentityUtils.onSyncListChange(this.netData.clientData.pinners, this.pinners, op, index, MyUInt64.uLongConvert);
            }
            else
            {
        ...
        #region capturedpromoted

        [SyncVar(hook = "onChangeCapturedpromoted")]
        public bool capturedpromoted;

        public void onChangeCapturedpromoted(bool newCapturedpromoted)
        {
            this.capturedpromoted = newCapturedpromoted;
            if (this.netData.clientData != null)
            {
                this.netData.clientData.capturedpromoted.v = newCapturedpromoted;
            }
            else
            {
                // Debug.LogError ("clientData null: " + this);
        ...
        public override void addSyncListCallBack()
        {
            ...
            this.checksGiven.Callback += OnChecksGiveChanged;
            this.blockersForKing.Callback += OnBlockersForKingChanged;
            this.pinners.Callback += OnPinnersForKingChanged;
        ...
        public override void refreshClientData()
        {
            if (this.netData.clientData != null)
            {
                ...
                IdentityUtils.refresh(this.netData.clientData.checksGiven, this.checksGiven);
                ...
                this.onChangeUnpromotedCapturedPiece(this.unpromotedCapturedPiece);
                IdentityUtils.refresh(this.netData.clientData.blockersForKing, this.blockersForKing, MyUInt64.uLongConvert);
                IdentityUtils.refresh(this.netData.clientData.pinners, this.pinners, MyUInt64.uLongConvert);
                IdentityUtils.refresh(this.netData.clientData.checkSquares, this.checkSquares, MyUInt64.uLongConvert);
                this.onChangeCapturedpromoted(this.capturedpromoted);
            }
            else
            {
                // Debug.Log ("clientData null: " + this);
        ...
        public override int refreshDataSize()
        {
            int ret = GetDataSize(this.netId);
            {
                ...
                ret += GetDataSize(this.checksGiven);
                ...
                ret += GetDataSize(this.unpromotedCapturedPiece);
                ret += GetDataSize(this.blockersForKing);
                ret += GetDataSize(this.pinners);
                ret += GetDataSize(this.checkSquares);
                ret += GetDataSize(this.capturedpromoted);
            }
            return ret;
        }
        ...
        public override void onAddCallBack<T>(T data)
        {
            if (data is StateInfo)
            {
                ...
                // Set property
                {
                    ...
                    IdentityUtils.InitSync(this.checksGiven, stateInfo.checksGiven.vs);
                    ...
                    this.unpromotedCapturedPiece = stateInfo.unpromotedCapturedPiece.v;
                    IdentityUtils.InitSync(this.blockersForKing, stateInfo.blockersForKing, MyUInt64.myUInt64Convert);
                    IdentityUtils.InitSync(this.pinners, stateInfo.pinners, MyUInt64.myUInt64Convert);
                    IdentityUtils.InitSync(this.checkSquares, stateInfo.checkSquares, MyUInt64.myUInt64Convert);
                    this.capturedpromoted = stateInfo.capturedpromoted.v;
                }
                this.getDataSize();
                // Observer
                {
            ...
            if (wrapProperty.p is StateInfo)
            {
                switch ((StateInfo.Property)wrapProperty.n)
                {
                    ...
                    case StateInfo.Property.pliesFromNull:
                        ...
                        break;
                    case StateInfo.Property.checksGiven:
                        IdentityUtils.UpdateSyncList(this.checksGiven, syncs, GlobalCast<T>.CastingInt32);
                        break;
                    ...
                    case StateInfo.Property.capturedPiece:
                        ...
                        break;
                    case StateInfo.Property.unpromotedCapturedPiece:
                        this.unpromotedCapturedPiece = (int)wrapProperty.getValue();
                        break;
                    case StateInfo.Property.blockersForKing:
                        ...
                        break;
                    case StateInfo.Property.pinners:
                        IdentityUtils.UpdateSyncList(this.pinners, syncs, GlobalCast<T>.CastingMyUInt64);
                        break;
                    case StateInfo.Property.checkSquares:
                        IdentityUtils.UpdateSyncList(this.checkSquares, syncs, GlobalCast<T>.CastingMyUInt64);
                        break;
                    case StateInfo.Property.capturedpromoted:
                        this.capturedpromoted = (bool)wrapProperty.getValue();
                        break;
                    default:
