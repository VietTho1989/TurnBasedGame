	...
	public class AnimationSetDirtyUpdate : UpdateBehavior<BoardUI.UIData>
	{
							...
							case GameMove.Type.BanqiMove:
								{
									...
									foreach (PieceUI.UIData pieceUIData in this.data.pieces.vs) {
										if (pieceUIData.position.v == 8 * (3 - (banqiMoveAnimation.fromY.v - 1)) + (banqiMoveAnimation.fromX.v - 1)
										   || pieceUIData.position.v == 8 * (3 - (banqiMoveAnimation.destY.v - 1)) + (banqiMoveAnimation.destX.v - 1)) {
		...
		public override void onUpdateSync<T> (WrapProperty wrapProperty, List<Sync<T>> syncs)
		{
			...
			if (wrapProperty.p is BoardUI.UIData) {
				...
				case BoardUI.UIData.Property.banqi:
					break;
