...
namespace InternationalDraught
{
	public class InputUI : UIBehavior<InputUI.UIData>
	{
					...
					if (data != null) {
						...
						WaitInputAction waitInputAction = null;
						{
							...
							InternationalDraught internationalDraught = null;
							{
								...
								if (inputUIData != null) {
									internationalDraught = inputUIData.internationalDraught.v.data;
								} else {
									Debug.LogError ("inputUIData null: " + data);
								}
							}
							if (internationalDraught != null) {
								Game game = internationalDraught.findDataInParent<Game> ();
								if (game != null) {
								...
								Debug.LogError ("internationalDraught null: " + data);
							}
						}
						// Process
				...
				if (this.data != null) {
					...
					if (internationalDraught != null) {
						if (GameDataUI.UIData.IsAllowInput (this.data)) {
							// use rule or not
							if (GameData.IsUseRule (this.data.internationalDraught.v.data)) {
								// Find
								UseRuleInputUI.UIData useRuleInputUIData = this.data.sub.newOrOld<UseRuleInputUI.UIData> ();
								{
									// internationalDraught
									useRuleInputUIData.internationalDraught.v = new ReferenceData<InternationalDraught> (this.data.internationalDraught.v.data);
								}
								...
								{
									// internationalDraught
									noneRuleInputUIData.internationalDraught.v = new ReferenceData<InternationalDraught> (this.data.internationalDraught.v.data);
								}
		...
		private GameDataCheckChangeRule<InternationalDraught> gameDataCheckChangeRule = new GameDataCheckChangeRule<InternationalDraught>();
				...
				// internationalDraught
				{
					if (data is InternationalDraught) {
						...
						// Check change
						{
							...
							gameDataCheckChangeRule.setData (internationalDraught);
						}
						dirty = true;
						return;
					}
					// CheckChange
					if (data is GameDataCheckChangeRule<InternationalDraught>) {
						dirty = true;
						return;
					}
				}
		...
		public override void onUpdateSync<T> (WrapProperty wrapProperty, List<Sync<T>> syncs)
		{
			...
			// Child
			{
				// InternationalDraught
				{
					...
					// Check change
					if(wrapProperty.p is GameDataCheckChangeRule<InternationalDraught>){
						dirty = true;
						return;
					}
				}
