...
namespace EnglishDraught
{
	public class InputUI : UIBehavior<InputUI.UIData>
	{
					...
					if (data != null) {
						...
						WaitInputAction waitInputAction = null;
						{
							...
							EnglishDraught englishDraught = null;
							{
								...
								if (inputUIData != null) {
									englishDraught = inputUIData.englishDraught.v.data;
								} else {
									Debug.LogError ("inputUIData null: " + data);
								}
							}
							if (englishDraught != null) {
								Game game = englishDraught.findDataInParent<Game> ();
								if (game != null) {
								...
								Debug.LogError ("englishDraught null: " + data);
							}
						}
						// Process
				...
				if (this.data != null) {
					...
					if (englishDraught != null) {
						if (GameDataUI.UIData.IsAllowInput (this.data)) {
							// use rule or not
							if (GameData.IsUseRule (this.data.englishDraught.v.data)) {
								// Find
								UseRuleInputUI.UIData useRuleInputUIData = this.data.sub.newOrOld<UseRuleInputUI.UIData> ();
								{
									// englishDraught
									useRuleInputUIData.englishDraught.v = new ReferenceData<EnglishDraught> (this.data.englishDraught.v.data);
								}
								...
								{
									// englishDraught
									noneRuleInputUIData.englishDraught.v = new ReferenceData<EnglishDraught> (this.data.englishDraught.v.data);
								}
		...
		private GameDataCheckChangeRule<EnglishDraught> gameDataCheckChangeRule = new GameDataCheckChangeRule<EnglishDraught>();
				...
				// englishDraught
				{
					if (data is EnglishDraught) {
						...
						// Check change
						{
							...
							gameDataCheckChangeRule.setData (englishDraught);
						}
						dirty = true;
						return;
					}
					// CheckChange
					if (data is GameDataCheckChangeRule<EnglishDraught>) {
						dirty = true;
						return;
					}
				}
		...
		public override void onUpdateSync<T> (WrapProperty wrapProperty, List<Sync<T>> syncs)
		{
			...
			// Child
			{
				// Shogi
				{
					...
					// Check change
					if(wrapProperty.p is GameDataCheckChangeRule<EnglishDraught>){
						dirty = true;
						return;
					}
				}
