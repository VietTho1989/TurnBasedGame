                ...
                if (this.data != null)
                {
                    List<ShatranjMove> shatranjMoves = new List<ShatranjMove>();
                    {
                        ...
                        if (showUIData != null && clickDestUIData != null)
                        {
                            ...
                            {
                                ShatranjMove legalMove = showUIData.legalMoves.vs[i];
                                if (ShatranjMove.IsClickCorrectPosition(legalMove.move.v, fromX, fromY, destX, destY))
                                {
                                    shatranjMoves.Add(legalMove);
                                }
                            }
                        }
                    }
                    ...
                    // btnChoseMoves
                    if (shatranjMoves.Count == 0)
                    {
                        Debug.LogError("why shatranjMoves count = 0: " + this);
                    ...
                    else if (shatranjMoves.Count == 1)
                    {
                        ShatranjMove shatranjMove = shatranjMoves[0];
                        // Send by clientInput
                        {
                    ...
                    else
                    {
                        ...
                        BtnChosenMoveAdapter.UIData btnChosenMoveAdapterUIData = this.data.btnChosenMoveAdapter.newOrOld<BtnChosenMoveAdapter.UIData>();
                        {
                            // moves
                            {
                                List<ReferenceData<ShatranjMove>> referenceShatranjMoves = new List<ReferenceData<ShatranjMove>>();
                                {
                                    foreach (ShatranjMove shatranjMove in shatranjMoves)
                                    {
                                        referenceShatranjMoves.Add(new ReferenceData<ShatranjMove>(shatranjMove));
                                    }
                                }
                                btnChosenMoveAdapterUIData.moves.copyList(referenceShatranjMoves);
                            }
                            // onClick
