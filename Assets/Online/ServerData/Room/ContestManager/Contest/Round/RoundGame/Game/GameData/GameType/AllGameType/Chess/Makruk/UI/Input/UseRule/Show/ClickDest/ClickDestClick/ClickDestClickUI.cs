				...
				if (this.data != null) {
					...
					// Legal moves
					{
						...
						// Update
						{
							...
							List<MakrukMove> legalMoves = new List<MakrukMove>();
							{
								...
								if (show != null) {
									for (int i = 0; i < show.legalMoves.vs.Count; i++) {
										MakrukMove legalMove = show.legalMoves.vs [i];
										// check correct
							...
							// Make UI
							{
								for (int legalMoveIndex = 0; legalMoveIndex < legalMoves.Count; legalMoveIndex++) {
									MakrukMove legalMove = legalMoves [legalMoveIndex];
									...
									if (legalMoveUIData != null) {
										// legalMove
										legalMoveUIData.legalMove.v = new ReferenceData<MakrukMove> (legalMove);
									} else {
						...
						foreach (LegalMoveUI.UIData legalMoveUIData in oldLegalMoves) {
							legalMoveUIData.legalMove.v = new ReferenceData<MakrukMove> (null);
						}
					}
				} else {
		...
		public override void onAddCallBack<T> (T data)
		{
			...
			// Parent
			{
				...
				if (data is ShowUI.UIData) {
					// ShowUI.UIData show = data as ShowUI.UIData;
					dirty = true;
					return;
				}
			}
				...
				if (this.data.moveOrChoose.v == null) {
					...
					bool isActive = false;
					{
						...
						if (useRuleInputUIData != null) {
							makruk = useRuleInputUIData.makruk.v.data;
							if (makruk != null) {
						...
						if (x >= 0 && x < 8 && y >= 0 && y < 8) {
							...
							bool isSelectSameColor = false;
							{
								...
								case Common.Piece.PIECE_NB:
									break;
								default:
									Debug.LogError ("unknown piece type: " + makruk.piece_on (square) + "; " + this);
									break;
								}
							}
							if (isSelectSameColor) {
								...
								bool haveMoveSameClick = false;
								{
									...
									if (show != null && clickDestUIData!=null) {
										...
										for (int i = 0; i < show.legalMoves.vs.Count; i++) {
											MakrukMove check = show.legalMoves.vs [i];
											if (MakrukMove.IsClickCorrectPosition (check.move.v, clickDestUIData.x.v, 
