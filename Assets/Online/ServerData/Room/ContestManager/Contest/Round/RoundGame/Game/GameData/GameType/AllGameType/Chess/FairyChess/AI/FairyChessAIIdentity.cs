...
{
	public class FairyChessAIIdentity : DataIdentity
	{
		...
		private NetData<FairyChessAI> netData = new NetData<FairyChessAI>();
		...
		public override void onAddCallBack<T> (T data)
		{
			if (data is FairyChessAI) {
				...
				// Set property
				{
					this.serialize (this.searchInfor, fairyChessAI.makeSearchInforms ());
					this.depth = fairyChessAI.depth.v;
					this.skillLevel = fairyChessAI.skillLevel.v;
					this.duration = fairyChessAI.duration.v;
				}
				// Observer
				{
					...
					if (observer != null) {
						...
						observer.setCheckChangeData (fairyChessAI);
					} else {
		...
		public void requestChangeDepth(uint userId, int newDepth)
		{
			...
			if (clientConnect != null) {
				clientConnect.CmdFairyChessAIChangeDepth (this.netId, userId, newDepth);
			} else {
		...
		public void requestChangeSkillLevel(uint userId, int newSkillLevel)
		{
			ClientConnectIdentity clientConnect = ClientConnectIdentity.findYourClientConnectIdentity (this.netData.clientData);
			if (clientConnect != null) {
				clientConnect.CmdFairyChessAIChangeSkillLevel (this.netId, userId, newSkillLevel);
			} else {
				Debug.LogError ("Cannot find clientConnect: " + this);
			}
		}
		...
		public void requestChangeDuration(uint userId, long newDuration)
		{
			ClientConnectIdentity clientConnect = ClientConnectIdentity.findYourClientConnectIdentity (this.netData.clientData);
			if (clientConnect != null) {
				clientConnect.CmdFairyChessAIChangeDuration (this.netId, userId, newDuration);
			} else {
				Debug.LogError ("Cannot find clientConnect: " + this);
			}
		}
