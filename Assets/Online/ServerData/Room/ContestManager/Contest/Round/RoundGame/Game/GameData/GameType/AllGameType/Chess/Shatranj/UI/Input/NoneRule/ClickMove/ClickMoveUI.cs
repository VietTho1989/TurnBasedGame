				...
				if (this.data != null) {
					...
					Shatranj shatranj = null;
					{
						...
						if (noneRuleInputUIData != null) {
							shatranj = noneRuleInputUIData.shatranj.v.data;
						} else {
					...
					if (shatranj != null) {
						// clickPiece?
						{
							...
							bool isClickPiece = false;
							{
								...
								Common.Piece piece = shatranj.piece_on (square);
		...
		public override void onAddCallBack<T> (T data)
		{
			...
			// Parent
			{
				if (data is NoneRuleInputUI.UIData) {
					...
					// Child
					{
						noneRuleInputUIData.shatranj.allAddCallBack (this);
					}
					dirty = true;
					return;
				}
		...
		public override void onRemoveCallBack<T> (T data, bool isHide)
		{
			...
			// Parent
			{
				if (data is NoneRuleInputUI.UIData) {
					...
					// Child
					{
						noneRuleInputUIData.shatranj.allRemoveCallBack (this);
					}
					return;
				}
				// Child
		...
		public override void onUpdateSync<T> (WrapProperty wrapProperty, List<Sync<T>> syncs)
		{
			...
			// Parent
			{
				if (wrapProperty.p is NoneRuleInputUI.UIData) {
					...
					case NoneRuleInputUI.UIData.Property.shatranj:
						{
					...
					if (x >= 0 && x < 8 && y >= 0 && y < 8) {
						if (this.data.x.v == x && this.data.y.v == y) {
							...
							if (clientInput != null) {
								...
								{
									shatranjCustomMove.fromX.v = this.data.x.v;
									shatranjCustomMove.fromY.v = this.data.y.v;
									shatranjCustomMove.destX.v = x;
									shatranjCustomMove.destY.v = y;
								}
								clientInput.makeSend (shatranjCustomMove);
							} else {
