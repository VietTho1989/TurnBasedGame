            ...
            if (dirty)
            {
                ...
                if (this.data != null)
                {
                    ...
                    if (editChessAI != null)
                    {
                        ...
                        {
                            // different
                            RequestChange.ShowDifferentTitle(lbTitle, editChessAI);
                            // get server state
                            Server.State.Type serverState = RequestChange.GetServerState(editChessAI);
                            // set origin
                            {
                                RequestChange.RefreshUI(this.data.depth.v, editChessAI, serverState, needReset, editData => editData.depth.v);
                                RequestChange.RefreshUI(this.data.skillLevel.v, editChessAI, serverState, needReset, editData => editData.skillLevel.v);
                                RequestChange.RefreshUI(this.data.duration.v, editChessAI, serverState, needReset, editData => editData.duration.v);
                            }
                            needReset = false;
                        }
                    }
