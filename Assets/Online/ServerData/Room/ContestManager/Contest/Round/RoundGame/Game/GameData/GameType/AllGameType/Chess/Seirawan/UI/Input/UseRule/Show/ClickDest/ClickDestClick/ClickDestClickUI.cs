				...
				if (this.data != null) {
					...
					// Legal moves
					{
						...
						// Update
						{
								...
								if (show != null) {
									for (int i = 0; i < show.legalMoves.vs.Count; i++) {
										...
										bool isCorrect = false;
										{
											...
											int destX = 0;
											int destY = 0;
											SeirawanMove.GetClickPosition (legalMove.move.v, out fromX, out fromY, out destX, out destY);
							...
							// Make UI
							{
								for (int legalMoveIndex = 0; legalMoveIndex < legalMoves.Count; legalMoveIndex++) {
									SeirawanMove legalMove = legalMoves [legalMoveIndex];
									...
									if (legalMoveUIData != null) {
										// legalMove
										legalMoveUIData.legalMove.v = new ReferenceData<SeirawanMove> (legalMove);
									} else {
						...
						foreach (LegalMoveUI.UIData legalMoveUIData in oldLegalMoves) {
							legalMoveUIData.legalMove.v = new ReferenceData<SeirawanMove> (null);
						}
					}
				} else {
				...
				if (this.data.moveOrChoose.v == null) {
					...
					bool isActive = false;
					{
						...
						if (useRuleInputUIData != null) {
							seirawan = useRuleInputUIData.seirawan.v.data;
							if (seirawan != null) {
					...
					if (isActive) {
						if (x >= 0 && x < 8 && y >= 0 && y < 8) {
							...
							bool isSelectSameColor = false;
							{
								...
								switch (seirawan.piece_on (square)) {
								...
								case Common.Piece.PIECE_NB:
									break;
								default:
									Debug.LogError ("unknown piece type: " + seirawan.piece_on (square) + "; " + this);
									break;
								}
							}
							if (isSelectSameColor) {
								...
								bool haveMoveSameClick = false;
								{
									...
									if (show != null && clickDestUIData!=null) {
										...
										for (int i = 0; i < show.legalMoves.vs.Count; i++) {
											SeirawanMove check = show.legalMoves.vs [i];
											if (SeirawanMove.IsClickCorrectPosition (check.move.v, clickDestUIData.x.v, 
