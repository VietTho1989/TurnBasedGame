...
namespace Seirawan.UseRule
{
	public class LegalMoveUI : UIBehavior<LegalMoveUI.UIData>
	{
		...
		{
			public VP<ReferenceData<SeirawanMove>> legalMove;
			...
			public UIData() : base()
			{
				this.legalMove = new VP<ReferenceData<SeirawanMove>>(this, (byte)Property.legalMove, new ReferenceData<SeirawanMove>(null));
			}
		...
		public override void refresh ()
		{
			if (dirty) {
				...
				if (this.data != null) {
					...
					// Update
					{
						SeirawanMove legalMove = this.data.legalMove.v.data;
						if (legalMove != null) {
							// position
							{
								int fromX = 0;
							...
							// imgType
							if (imgType != null) {
								SeirawanMove.Move move = new SeirawanMove.Move (legalMove.move.v);
								switch (move.type) {
		...
		public override void onAddCallBack<T> (T data)
		{
			...
			if (data is SeirawanMove) {
				// SeirawanMove legalMove = data as SeirawanMove;
				{
