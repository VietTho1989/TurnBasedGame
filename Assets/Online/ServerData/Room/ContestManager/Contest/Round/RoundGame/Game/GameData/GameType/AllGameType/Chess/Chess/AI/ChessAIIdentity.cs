...
namespace Chess
{
	public class ChessAIIdentity : DataIdentity
	{
		...
		private NetData<ChessAI> netData = new NetData<ChessAI>();
		...
		public override void onAddCallBack<T> (T data)
		{
			if (data is ChessAI) {
				...
				// Set property
				{
					this.serialize (this.searchInfor, chessAI.makeSearchInforms ());
					this.depth = chessAI.depth.v;
					this.skillLevel = chessAI.skillLevel.v;
					this.duration = chessAI.duration.v;
				}
				// Observer
				{
					...
					if (observer != null) {
						...
						observer.setCheckChangeData (chessAI);
					} else {
		...
		public void requestChangeDepth(uint userId, int newDepth)
		{
			...
			if (clientConnect != null) {
				clientConnect.CmdChessAIChangeDepth (this.netId, userId, newDepth);
			} else {
		...
		public void requestChangeSkillLevel(uint userId, int newSkillLevel)
		{
			ClientConnectIdentity clientConnect = ClientConnectIdentity.findYourClientConnectIdentity (this.netData.clientData);
			if (clientConnect != null) {
				clientConnect.CmdChessAIChangeSkillLevel (this.netId, userId, newSkillLevel);
			} else {
				Debug.LogError ("Cannot find clientConnect: " + this);
			}
		}
		...
		public void requestChangeDuration(uint userId, long newDuration)
		{
			ClientConnectIdentity clientConnect = ClientConnectIdentity.findYourClientConnectIdentity (this.netData.clientData);
			if (clientConnect != null) {
				clientConnect.CmdChessAIChangeDuration (this.netId, userId, newDuration);
			} else {
				Debug.LogError ("Cannot find clientConnect: " + this);
			}
		}
