...
namespace Sudoku
{
	public class SudokuSolve : GameMove
	{
		...
		public override GameMove getForHintMove (GameType gameType)
		{
			if (gameType is Sudoku) {
				Sudoku sudoku = gameType as Sudoku;
				// get ai move
				{
					...
					List<AIHaveSolve.BoardAndOrder> boardAndOrders = new List<AIHaveSolve.BoardAndOrder>();
					{
						while (allBoards.Count >= 81) {
							AIHaveSolve.BoardAndOrder boardAndOrder = new AIHaveSolve.BoardAndOrder ();
							{
					...
					if (sudoku != null) {
						// remove board and order cannot solve
						{
							...
							for (int i = boardAndOrders.Count - 1; i >= 0; i--) {
								AIHaveSolve.BoardAndOrder boardAndOrder = boardAndOrders [i];
								// check correct
								bool isCorrect = true;
								{
									for (int y = 0; y < 9; y++)
						...
						if (boardAndOrders.Count > 0) {
							...
							foreach (AIHaveSolve.BoardAndOrder boardAndOrder in boardAndOrders) {
								int correctCount = 0;
								{
									for (int y = 0; y < 9; y++)
										for (int x = 0; x < 9; x++) {
							...
							if (boardAndOrders.Count > 0) {
								AIHaveSolve.BoardAndOrder boardAndOrder = boardAndOrders[0];
								{
