...
namespace Seirawan
{
	public class SeirawanAI : Computer.AI
	{
		#region Property
			...
			if (needRequest.canRequest) {
				if (!needRequest.needIdentity) {
					...
					if (DataIdentity.clientMap.TryGetValue (this, out dataIdentity)) {
						if (dataIdentity is SeirawanAIIdentity) {
							SeirawanAIIdentity seirawanAIIdentity = dataIdentity as SeirawanAIIdentity;
							seirawanAIIdentity.requestChangeDepth (userId, newDepth);
						} else {
			...
			if (needRequest.canRequest) {
				if (!needRequest.needIdentity) {
					...
					if (DataIdentity.clientMap.TryGetValue (this, out dataIdentity)) {
						if (dataIdentity is SeirawanAIIdentity) {
							SeirawanAIIdentity seirawanAIIdentity = dataIdentity as SeirawanAIIdentity;
							seirawanAIIdentity.requestChangeSkillLevel (userId, newSkillLevel);
						} else {
							Debug.LogError ("Why isn't correct identity");
						}
					} else {
			...
			if (needRequest.canRequest) {
				if (!needRequest.needIdentity) {
					...
					if (DataIdentity.clientMap.TryGetValue (this, out dataIdentity)) {
						if (dataIdentity is SeirawanAIIdentity) {
							SeirawanAIIdentity seirawanAIIdentity = dataIdentity as SeirawanAIIdentity;
							seirawanAIIdentity.requestChangeDuration (userId, newDuration);
						} else {
							Debug.LogError ("Why isn't correct identity");
						}
					} else {
