...
namespace Chess
{
    public class Common
    {
        ...
        public enum Value : int
        {
            ...
            VALUE_MATE_IN_MAX_PLY = 32000 - 2 * MAX_PLY,// VALUE_MATE - 2 * MAX_PLY,
            VALUE_MATED_IN_MAX_PLY = -32000 + 2 * MAX_PLY,// -VALUE_MATE + 2 * MAX_PLY,
        ...
        public static string moveToString(int m, bool chess960)
        {
            ...
            if (type_of(m) == MoveType.CASTLING && !chess960)
        ...
        public static string positionToString(Chess chess)
        {
            StringBuilder ss = new StringBuilder();
            {
                ...
                for (Rank y = Rank.RANK_8; y >= Rank.RANK_1; --y)
                {
                    for (File x = File.FILE_A; x <= File.FILE_H; ++x)
                    {
                        ...
                        int charIndex = 0;
                        {
                            charIndex = (int)chess.piece_on(make_square(x, y));
