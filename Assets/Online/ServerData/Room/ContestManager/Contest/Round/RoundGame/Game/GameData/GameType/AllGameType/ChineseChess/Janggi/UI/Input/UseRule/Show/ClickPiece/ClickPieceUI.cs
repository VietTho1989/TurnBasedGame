									...
									case GameMove.Type.JanggiMove:
										{
											...
											lastKeyX = janggiMove.toX.v;
											lastKeyY = janggiMove.toY.v;
										}
										break;
					...
					if (x >= 0 && x < 9 && y >= 0 && y < 10) {
						...
						uint pc = 0;
						{
							...
							if (index >= 0 && index < janggi.stones.vs.Count) {
								pc = janggi.stones.vs [index];
							}
						}
						if (pc != (uint)StoneHelper.Stones.Empty) {
							...
							if (isCorrectPiece) {
								...
								Debug.LogError ("not click correct piece: " + (StoneHelper.Stones)pc);
							}
						} else {
