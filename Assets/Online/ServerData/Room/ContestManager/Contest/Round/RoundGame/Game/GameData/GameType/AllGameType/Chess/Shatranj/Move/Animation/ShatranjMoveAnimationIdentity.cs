...
namespace Shatranj
{
    public class ShatranjMoveAnimationIdentity : DataIdentity
    {
        ...
        private NetData<ShatranjMoveAnimation> netData = new NetData<ShatranjMoveAnimation>();
        ...
        {
            if (data is ShatranjMoveAnimation)
            {
                ShatranjMoveAnimation shatranjMoveAnimation = data as ShatranjMoveAnimation;
                // Set new parent
                this.addTransformToParent();
                // Set property
                {
                    this.serialize(this.searchInfor, shatranjMoveAnimation.makeSearchInforms());
                    IdentityUtils.InitSync(this.board, shatranjMoveAnimation.board.vs);
                    this.move = shatranjMoveAnimation.move.v;
                    this.chess960 = shatranjMoveAnimation.chess960.v;
                    this.duration = shatranjMoveAnimation.duration.v;
                }
                // Observer
                {
                    ...
                    if (observer != null)
                    {
                        ...
                        observer.setCheckChangeData(shatranjMoveAnimation);
                    }
                    else
                    {
        ...
        public override void onUpdateSync<T>(WrapProperty wrapProperty, List<Sync<T>> syncs)
        {
            ...
            if (wrapProperty.p is ShatranjMoveAnimation)
            {
                switch ((ShatranjMoveAnimation.Property)wrapProperty.n)
                {
                    case ShatranjMoveAnimation.Property.board:
                        IdentityUtils.UpdateSyncList(this.board, syncs, GlobalCast<T>.CastingInt32);
                        break;
                    case ShatranjMoveAnimation.Property.move:
                        this.move = (System.Int32)wrapProperty.getValue();
                        break;
                    case ShatranjMoveAnimation.Property.chess960:
                        this.chess960 = (System.Boolean)wrapProperty.getValue();
                        break;
                    case ShatranjMoveAnimation.Property.duration:
