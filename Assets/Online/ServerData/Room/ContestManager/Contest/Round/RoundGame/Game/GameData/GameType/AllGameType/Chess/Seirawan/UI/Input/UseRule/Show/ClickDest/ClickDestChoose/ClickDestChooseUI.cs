...
namespace Seirawan.UseRule
{
    public class ClickDestChooseUI : UIBehavior<ClickDestChooseUI.UIData>, BtnChosenMoveUI.OnClick
    {
                ...
                if (this.data != null)
                {
                    List<SeirawanMove> seirawanMoves = new List<SeirawanMove>();
                    {
                        ...
                        if (showUIData != null && clickDestUIData != null)
                        {
                            ...
                            {
                                SeirawanMove legalMove = showUIData.legalMoves.vs[i];
                                if (SeirawanMove.IsClickCorrectPosition(legalMove.move.v, fromX, fromY, destX, destY))
                                {
                                    seirawanMoves.Add(legalMove);
                                }
                            }
                        }
                    }
                    ...
                    // btnChoseMoves
                    if (seirawanMoves.Count == 0)
                    {
                        Debug.LogError("why seirawanMoves count = 0: " + this);
                    ...
                    else if (seirawanMoves.Count == 1)
                    {
                        SeirawanMove seirawanMove = seirawanMoves[0];
                        // Send by clientInput
                        {
                            ClientInput clientInput = InputUI.UIData.findClientInput(this.data);
                            if (clientInput != null)
                            {
                                clientInput.makeSend(seirawanMove);
                            }
                            else
                            {
        ...
        public void onClickMove(SeirawanMove seirawanMove)
        {
            if (this.data != null)
            {
                ClientInput clientInput = InputUI.UIData.findClientInput(this.data);
