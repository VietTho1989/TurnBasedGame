...
namespace HEX
{
	public class UseRuleInputUI : UIBehavior<UseRuleInputUI.UIData>, IPointerDownHandler
	{
		...
		public GameObject switchSideContainer;
						...
						if (switchSideContainer != null) {
							...
							if (boardUIData != null) {
								...
								if (boardSize >= Hex.MIN_BOARD_SIZE && boardSize <= Hex.MAX_BOARD_SIZE) {
									...
									if (switchSideContainer != null) {
										if (hex.rounds () == 1) {
											switchSideContainer.SetActive (true);
											float boardWidth = boardSize;
											switchSideContainer.transform.localPosition = new Vector3 (-1 - boardWidth / 2.0f, (boardWidth / 2 - 1) - boardWidth / 2.0f + 0.5f, 0);
										} else {
											switchSideContainer.SetActive (false);
										}
									} else {
