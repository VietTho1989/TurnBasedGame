...
{
	public class RequestNewEliminationRoundStateAsk : RequestNewEliminationRound.State
	{
			...
			if (needRequest.canRequest) {
				if (!needRequest.needIdentity) {
					...
					if (DataIdentity.clientMap.TryGetValue (this, out dataIdentity)) {
						if (dataIdentity is RequestNewEliminationRoundStateAskIdentity) {
							RequestNewEliminationRoundStateAskIdentity requestNewEliminationRoundStateAskIdentity = dataIdentity as RequestNewEliminationRoundStateAskIdentity;
							requestNewEliminationRoundStateAskIdentity.requestAccept (userId);
						} else {
			...
			if (needRequest.canRequest) {
				if (!needRequest.needIdentity) {
					...
					if (DataIdentity.clientMap.TryGetValue (this, out dataIdentity)) {
						if (dataIdentity is RequestNewEliminationRoundStateAskIdentity) {
							RequestNewEliminationRoundStateAskIdentity requestNewEliminationRoundStateAskIdentity = dataIdentity as RequestNewEliminationRoundStateAskIdentity;
							requestNewEliminationRoundStateAskIdentity.requestCancel (userId);
						} else {
							Debug.LogError ("Why isn't correct identity");
						}
					} else {
		...
		public RequestNewEliminationRoundStateAsk() : base()
		{
