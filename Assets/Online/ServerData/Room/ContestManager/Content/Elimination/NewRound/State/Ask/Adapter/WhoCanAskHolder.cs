...
namespace GameManager.Match.Elimination
{
	public class WhoCanAskHolder : SriaHolderBehavior<WhoCanAskHolder.UIData>
	{
				...
				if (human != null) {
					...
					// tvAnswer
					{
						if (tvAnswer != null) {
							...
							{
								RequestNewEliminationRoundStateAsk requestNewEliminationRoundStateAsk = human.findDataInParent<RequestNewEliminationRoundStateAsk> ();
								if (requestNewEliminationRoundStateAsk != null) {
									alreadyAccept = requestNewEliminationRoundStateAsk.accepts.vs.Contains (human.playerId.v);
								} else {
		...
		private RequestNewEliminationRoundStateAsk requestNewEliminationRoundStateAsk = null;
			...
			// Child
			{
				// Human
				{
					if (data is Human) {
						...
						// Parent
						{
							DataUtils.addParentCallBack (human, this, ref this.requestNewEliminationRoundStateAsk);
						}
						// Child
						{
			...
			// Child
			{
				// Human
				{
					if (data is Human) {
						...
						// Parent
						{
							DataUtils.removeParentCallBack (human, this, ref this.requestNewEliminationRoundStateAsk);
						}
						// Child
						{
