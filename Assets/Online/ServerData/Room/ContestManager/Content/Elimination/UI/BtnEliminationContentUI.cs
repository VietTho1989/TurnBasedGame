...
{
    public class BtnEliminationContentUI : UIHaveTransformDataBehavior<BtnEliminationContentUI.UIData>
    {
        ...
        private static readonly TxtLanguage txtEliminationContent = new TxtLanguage("Elimination round");

        static BtnEliminationContentUI()
        {
            txtEliminationContent.add(Language.Type.vi, "Vòng loại");
        }
                    ...
                    if (eliminationContentUIData != null)
                    {
                        // tvEliminationContent
                        {
                            if (tvEliminationContent != null)
                            {
                                ...
                                tvEliminationContent.text = txtEliminationContent.get() + " " + (eliminationRoundIndex + 1) + "/" + eliminationRoundCount;
                            }
                            else
                            {
        ...
        public override void Awake()
        {
            ...
            // OnClick
            {
                UIUtils.SetButtonOnClick(btnEliminationContent, onClickBtnEliminationContent);
            }
        }
            ...
            bool isProcess = false;
            {
                if (e.isKey && e.type == EventType.KeyUp)
                {
                    switch (e.keyCode)
                    {
                        case KeyCode.C:
                            {
                                if (btnEliminationContent != null && btnEliminationContent.gameObject.activeInHierarchy && btnEliminationContent.interactable)
                                {
                                    this.onClickBtnEliminationContent();
                                    isProcess = true;
                                }
                                else
                                {
        ...
        public Button btnEliminationContent;
