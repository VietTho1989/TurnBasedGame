...
namespace GameManager.Match.RoundRobin
{
    public class WhoCanAskHolder : SriaHolderBehavior<WhoCanAskHolder.UIData>
    {
                ...
                if (human != null)
                {
                    ...
                    // ivAnswer
                    {
                        if (ivAnswer != null)
                        {
                            ...
                            {
                                RequestNewRoundRobinStateAsk requestNewRoundRobinStateAsk = human.findDataInParent<RequestNewRoundRobinStateAsk>();
                                if (requestNewRoundRobinStateAsk != null)
                                {
                                    alreadyAccept = requestNewRoundRobinStateAsk.accepts.vs.Contains(human.playerId.v);
                                }
                                else
                                {
        ...
        private RequestNewRoundRobinStateAsk requestNewRoundRobinStateAsk = null;
            ...
            // Child
            {
                // Human
                {
                    if (data is Human)
                    {
                        ...
                        // Parent
                        {
                            DataUtils.addParentCallBack(human, this, ref this.requestNewRoundRobinStateAsk);
                        }
                        // Child
                        {
            ...
            // Child
            {
                // Human
                {
                    if (data is Human)
                    {
                        ...
                        // Parent
                        {
                            DataUtils.removeParentCallBack(human, this, ref this.requestNewRoundRobinStateAsk);
                        }
                        // Child
                        {
