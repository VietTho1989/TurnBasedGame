...
{
	public class ChooseRoundContestTeamUI : UIBehavior<ChooseRoundContestTeamUI.UIData>
	{
		...
		static ChooseRoundContestTeamUI()
		{
			...
			txtTeamScore.add (Language.Type.vi, "Điểm số đội");
		}
			...
			if (dirty) {
				...
				if (this.data != null) {
					...
					if (roundContest != null) {
						...
						{
							if (tvTeamIndex != null) {
								tvTeamIndex.text = txtTeamIndex.get ("Team Index") + ": " + this.data.teamIndex.v;
							} else {
						...
						// tvTeamScore
						{
							if (tvTeamScore != null) {
								...
								tvTeamScore.text = txtTeamScore.get ("Team Score") + ": " + score;
							} else {
