...
namespace GameManager.Match.RoundRobin
{
	public class BtnRoundRobinUI : UIBehavior<BtnRoundRobinUI.UIData>
	{
		...
		public static readonly TxtLanguage txtRoundRobin = new TxtLanguage();

		static BtnRoundRobinUI()
		{
			txtRoundRobin.add (Language.Type.vi, "Trận đấu vòng tròn");
		}
					...
					if (roundRobinUIData != null) {
						// tvRoundRobin
						{
							if (tvRoundRobin != null) {
								...
								tvRoundRobin.text = txtRoundRobin.get ("RoundRobin Match") + " " + roundContestIndex + "/" + roundContestCount;
							} else {
