    ...
    public override void update()
    {
        if (dirty)
        {
            ...
            if (this.data != null)
            {
                ...
                // fastStart
                {
                    Debug.LogError("RoomUpdate: fast start: " + this.data.isFastStartRoom);
                    if (this.data.isFastStartRoom)
                    {
                        if (this.data.contestManagers.vs.Count == 1)
                        {
                            this.data.isFastStartRoom = false;
                            ContestManager contestManager = this.data.contestManagers.vs[0];
                            contestManager.isFastStart = true;
                            // Update
                            {
                                ContestManagerUpdate contestManagerUpdate = contestManager.findCallBack<ContestManagerUpdate>();
                                if (contestManagerUpdate != null)
                                {
                                    contestManagerUpdate.makeDirty();
                                }
                                else
                                {
                                    Debug.LogError("contestManagerUpdate null");
                                }
                            }
                        }
                    }
