...
public class RoomStateNormalFreezeUI : UIBehavior<RoomStateNormalFreezeUI.UIData>
{
    ...
    private static readonly TxtLanguage txtUnFreeze = new TxtLanguage("Unfreeze Room?");
    private static readonly TxtLanguage txtCancelUnFreeze = new TxtLanguage("Cancel Unfreeze Room?");
    private static readonly TxtLanguage txtUnFreezing = new TxtLanguage("Unfreezing Room");
    private static readonly TxtLanguage txtStateFreeze = new TxtLanguage("Room State Freeze");

    private static readonly TxtLanguage txtRequestError = new TxtLanguage("Send request unfreeze error");
                ...
                if (freeze != null)
                {
                                    ...
                                    case UIData.State.None:
                                        {
                                            ...
                                            tvFreeze.text = txtUnFreeze.get();
                                        }
                                        break;
                                    case UIData.State.Request:
                                        {
                                            btnFreeze.interactable = true;
                                            tvFreeze.text = txtCancelUnFreeze.get();
                                        }
                                        break;
                                    case UIData.State.Wait:
                                        {
                                            btnFreeze.interactable = false;
                                            tvFreeze.text = txtUnFreezing.get();
                                        }
                                        break;
                                    default:
                                        Debug.LogError("unknown state: " + this.data.state.v + "; " + this);
                    ...
                    else
                    {
                        ...
                        // btnFreeze, tvFreeze
                        {
                            ...
                            {
                                btnFreeze.interactable = false;
                                tvFreeze.text = txtStateFreeze.get();
                            }
                            else
                            {
                                Debug.LogError("btnFreeze, tvFreeze null: " + this);
    ...
    public override void Awake()
    {
        ...
        // OnClick
        {
            UIUtils.SetButtonOnClick(btnFreeze, onClickBtnFreeze);
        }
    }
        ...
        bool isProcess = false;
        {
            if (e.isKey && e.type == EventType.KeyUp)
            {
                switch (e.keyCode)
                {
                    case KeyCode.F:
                        {
                            if (btnFreeze != null && btnFreeze.gameObject.activeInHierarchy && btnFreeze.interactable)
                            {
                                this.onClickBtnFreeze();
                                isProcess = true;
                            }
                            else
                            {
