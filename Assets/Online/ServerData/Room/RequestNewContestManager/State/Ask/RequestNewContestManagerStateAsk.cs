...
{
	public class RequestNewContestManagerStateAsk : RequestNewContestManager.State
	{
			...
			if (needRequest.canRequest) {
				if (!needRequest.needIdentity) {
					...
					if (DataIdentity.clientMap.TryGetValue (this, out dataIdentity)) {
						if (dataIdentity is RequestNewContestManagerStateAskIdentity) {
							RequestNewContestManagerStateAskIdentity requestNewContestManagerStateAskIdentity = dataIdentity as RequestNewContestManagerStateAskIdentity;
							requestNewContestManagerStateAskIdentity.requestAccept(userId);
						} else {
			...
			if (needRequest.canRequest) {
				if (!needRequest.needIdentity) {
					...
					if (DataIdentity.clientMap.TryGetValue (this, out dataIdentity)) {
						if (dataIdentity is RequestNewContestManagerStateAskIdentity) {
							RequestNewContestManagerStateAskIdentity requestNewContestManagerStateAskIdentity = dataIdentity as RequestNewContestManagerStateAskIdentity;
							requestNewContestManagerStateAskIdentity.requestCancel(userId);
						} else {
							Debug.LogError ("Why isn't correct identity");
						}
					} else {
		...
		public RequestNewContestManagerStateAsk() : base()
		{
