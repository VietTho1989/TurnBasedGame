...
public class BtnSaveDataUI : UIBehavior<BtnSaveDataUI.UIData>
{
    ...
    private static readonly TxtLanguage txtSave = new TxtLanguage("Save Data");
    private static readonly TxtLanguage txtSaving = new TxtLanguage("Saving data...");
    private static readonly TxtLanguage txtNoFileName = new TxtLanguage("File name empty");
    ...
    public override void Awake()
    {
        ...
        // OnClick
        {
            UIUtils.SetButtonOnClick(btnSaveData, onClickBtnSaveData);
        }
    }
        ...
        bool isProcess = false;
        {
            if (e.isKey && e.type == EventType.KeyUp)
            {
                switch (e.keyCode)
                {
                    case KeyCode.S:
                        {
                            if (btnSaveData != null && btnSaveData.gameObject.activeInHierarchy && btnSaveData.interactable)
                            {
                                this.onClickBtnSaveData();
                                isProcess = true;
                            }
                            else
                            {
                                ...
                                if (saveUI != null)
                                {
                                    if (saveUI.edtName != null)
                                    {
                                        ...
                                        else
                                        {
                                            ...
                                            Toast.showMessage(txtNoFileName.get());
                                        }
                                    }
                                    else
                                    {
