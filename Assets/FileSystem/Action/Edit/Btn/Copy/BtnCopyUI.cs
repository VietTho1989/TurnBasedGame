...
namespace FileSystem
{
    public class BtnCopyUI : UIBehavior<BtnCopyUI.UIData>
    {
        ...
        private static readonly TxtLanguage txtCannotCopyNotSelect = new TxtLanguage("Copy");
        private static readonly TxtLanguage txtCopy = new TxtLanguage("Copy");
        private static readonly TxtLanguage txtAlreadySelectCopyCancel = new TxtLanguage("Cancel copy");
        private static readonly TxtLanguage txtCopying = new TxtLanguage("Copying");
        private static readonly TxtLanguage txtCancelCopy = new TxtLanguage("Copying, Cancel?");

        private static readonly TxtLanguage txtCopyingFile = new TxtLanguage("Copying");
        private static readonly TxtLanguage txtCancel = new TxtLanguage("cancel");

        private static readonly TxtLanguage txtCopySuccess = new TxtLanguage("Copy success");
        private static readonly TxtLanguage txtCopyFail = new TxtLanguage("Copy fail");

        private static readonly TxtLanguage txtCopyFile = new TxtLanguage("Copy");
        private static readonly TxtLanguage txtFail = new TxtLanguage("fail");

        private static readonly TxtLanguage txtCannotCopyOtherAction = new TxtLanguage("Can't copy");
                                    ...
                                    case Action.Type.None:
                                        {
                                            ...
                                            if (actionNone.selectFiles.vs.Count == 0)
                                            {
                                                ...
                                                tvCopy.text = txtCannotCopyNotSelect.get();
                                            }
                                            else
                                            {
                                                switch (actionNone.state.v)
                                                {
                                                    ...
                                                    case ActionNone.State.Copy:
                                                        {
                                                            btnCopy.interactable = true;
                                                            tvCopy.text = txtAlreadySelectCopyCancel.get();
                                                        }
                                                        break;
                                                    default:
                                    ...
                                    case Action.Type.Edit:
                                        {
                                                ...
                                                if (state != null)
                                                {
                                                    switch (state.getType())
                                                    {
                                                                ...
                                                                // set
                                                                {
                                                                    ...
                                                                    // txt
                                                                    {
                                                                        ...
                                                                        if (file != null)
                                                                        {
                                                                            ...
                                                                            tvCopy.text = txtCopyingFile.get() + " " + file.Name + " (" + percent + "), " + txtCancel.get() + "?";
                                                                        }
                                                                        else
                                                                        {
                                                                            tvCopy.text = txtCancelCopy.get();
                                                                        }
                                                                    }
                                                                }
                                                            }
                                                        ...
                                                        case ActionEdit.State.Type.Success:
                                                            {
                                                                btnCopy.interactable = false;
                                                                tvCopy.text = txtCopySuccess.get();
                                                            }
                                                            break;
                                                        case ActionEdit.State.Type.Fail:
                                                            {
                                                                ...
                                                                // Set
                                                                {
                                                                    ...
                                                                    // txt
                                                                    {
                                                                        ...
                                                                        if (failFile != null)
                                                                        {
                                                                            tvCopy.text = txtCopyFile.get() + " " + failFile.Name + " " + txtFail.get();
                                                                        }
                                                                        else
                                                                        {
                                                                            Debug.LogError("failFile null: " + this);
                                                                            tvCopy.text = txtCopyFail.get();
                                                                        }
                                                                    }
                                                                }
                                                            }
                                            ...
                                            {
                                                btnCopy.interactable = false;
                                                tvCopy.text = txtCannotCopyOtherAction.get();
                                            }
                                        }
                                        break;
                                    default:
            ...
            bool isProcess = false;
            {
                if (e.isKey && e.type == EventType.KeyUp)
                {
                    switch (e.keyCode)
                    {
                        case KeyCode.C:
                            {
                                if (btnCopy != null && btnCopy.gameObject.activeInHierarchy && btnCopy.interactable)
                                {
                                    this.onClickBtnCopy();
                                    isProcess = true;
                                }
