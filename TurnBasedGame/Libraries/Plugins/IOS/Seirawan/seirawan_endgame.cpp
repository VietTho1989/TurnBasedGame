...
namespace Seirawan
{
    ...
    namespace {
        ...
        const int32_t PushClose[8] = { 0, 120, 100, 80, 60, 40, 20, 10 };
    ...
    template<> Value Endgame<KEKR>::operator()(const Position& pos) const {
        
        // assert(verify_material(pos, strongSide, ElephantValueMg, 0));
        if(!verify_material(pos, strongSide, ElephantValueMg, 0)){
            printf("error, assert(verify_material(pos, strongSide, ElephantValueMg, 0))\n");
        }
        // assert(verify_material(pos, weakSide, RookValueMg, 0));
        if(!verify_material(pos, weakSide, RookValueMg, 0)){
            printf("error, assert(verify_material(pos, weakSide, RookValueMg, 0))\n");
    ...
    /// KH vs KR.
    template<> Value Endgame<KHKR>::operator()(const Position& pos) const {
        
        // assert(verify_material(pos, strongSide, HawkValueMg, 0));
        if(!verify_material(pos, strongSide, HawkValueMg, 0)){
            printf("error, assert(verify_material(pos, strongSide, HawkValueMg, 0))\n");
        }
        // assert(verify_material(pos, weakSide, RookValueMg, 0));
        if(!verify_material(pos, weakSide, RookValueMg, 0)){
            printf("error, assert(verify_material(pos, weakSide, RookValueMg, 0))\n");
