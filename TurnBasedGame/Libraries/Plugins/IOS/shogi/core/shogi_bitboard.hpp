...
#include "shogi_square.hpp"
#include "shogi_color.hpp"
...
namespace Shogi {
    ...
    public:
        ...
        u64 p(const int32_t index) const { return p_[index]; }
        void set(const int32_t index, const u64 val) { p_[index] = val; }
        ...
        Bitboard operator <<= (const int32_t i) {
#if defined (HAVE_SSE2) || defined (HAVE_SSE4)
        ...
        Bitboard operator | (const Bitboard& rhs) const { return Bitboard(*this) |= rhs; }
        Bitboard operator ^ (const Bitboard& rhs) const { return Bitboard(*this) ^= rhs; }
        Bitboard operator << (const int32_t i) const { return Bitboard(*this) <<= i; }
        Bitboard operator >> (const int32_t i) const { return Bitboard(*this) >>= i; }
        ...
        bool isSet(const Square sq) const {
            // TODO assert(isInSquare(sq));
            if(!isInSquare(sq)){
                printf("error, isSet: assert(isInSquare(sq))\n");
            }
        ...
        int32_t popCount() const { return (Crossover ? count1s(p(0)) + count1s(p(1)) : count1s(merge())); }
        ...
        void printTable(const int32_t part) const {
            for (Rank r = Rank1; r < RankNum; ++r) {
        ...
        static int32_t part(const Square sq) { return static_cast<int32_t>(SQ79 < sq); }
    ...
    extern const int32_t RookBlockBits[SquareNum];
    extern const int32_t BishopBlockBits[SquareNum];
    extern const int32_t RookShiftBits[SquareNum];
    extern const int32_t BishopShiftBits[SquareNum];
...
#endif
    extern int32_t RookAttackIndex[SquareNum];
    // メモリ節約の為、1次元配列にして無駄が無いようにしている。
    extern Bitboard BishopAttack[20224];
    extern int32_t BishopAttackIndex[SquareNum];
    ...
    inline u64 occupiedToIndex(const Bitboard& block, const u64 magic, const int32_t shiftBits) {
    ...
    inline Bitboard lanceAttack(const Color c, const Square sq, const Bitboard& occupied) {
        const int32_t part = Bitboard::part(sq);
        const int32_t index = (occupied.p(part) >> Slide[sq]) & 127;
