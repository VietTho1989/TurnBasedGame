...
#include "shogi_piece.hpp"
#include "shogi_common.hpp"
#include "shogi_hand.hpp"
#include "shogi_bitboard.hpp"
#include "shogi_pieceScore.hpp"
#include "shogi_evalList.hpp"
#include <stack>
#include <memory>
...
namespace Shogi {
    ...
    struct ChangedLists {
        ...
        int32_t listindex[2]; // 一手で動く駒は最大2つ。(動く駒、取られる駒)
        size_t size;
    };
    ...
    struct StateInfo {
        ...
        int32_t continuousCheck[ColorNum]; // Stockfish には無い。
        ...
        static int32_t getStateInfoByteSize()
        {
            int32_t stateInfoSize = 0;
            {
                ...
                stateInfoSize+=sizeof(int32_t);
                // int32_t continuousCheck[ColorNum]; // Stockfish には無い。
                stateInfoSize+=sizeof(int32_t)*(int)ColorNum;
                
                // Not copied when making a move (will be recomputed anyhow)
                // Key boardKey;
                stateInfoSize+=sizeof(u64);
                // Key handKey;
                stateInfoSize+=sizeof(u64);
                // Bitboard checkersBB; // 手番側の玉へ check している駒の Bitboard
                {
                    int32_t bitboardSize = 2*sizeof(u64);
                    stateInfoSize+=bitboardSize;
                }
                ...
                // Hand hand; // 手番側の持ち駒
                stateInfoSize+=sizeof(u32);
                // ChangedLists cl;
                {
                    stateInfoSize+=sizeof(ChangedLists);
                }
            }
            return stateInfoSize;
        }
        
        static int32_t convertToByteArray(StateInfo* stateInfo, uint8_t* &byteArray)
        {
            // find length of return
            int32_t length = StateInfo::getStateInfoByteSize();
            u8* ret = (uint8_t*)calloc(length, sizeof(uint8_t));
            {
                ...
                // Score material; // stocfish の npMaterial は 先手、後手の点数を配列で持っているけど、
                {
                    ...
                    if(pointerIndex+size<=length){
                        int32_t value = stateInfo->material;
                        memcpy(ret + pointerIndex, &value, size);
                        pointerIndex+=size;
                    }else{
                        printf("length error: material: %d, %d\n", pointerIndex, length);
                    }
                }
                // 特に分ける必要は無い気がする。
                // int32_t pliesFromNull;
                {
                    int32_t size = sizeof(int32_t);
                    if(pointerIndex+size<=length){
                        int32_t value = stateInfo->pliesFromNull;
                        memcpy(ret + pointerIndex, &value, size);
                        pointerIndex+=size;
                    }else{
                        printf("length error: pliesFromNull: %d, %d\n", pointerIndex, length);
                    }
                }
                // int32_t continuousCheck[ColorNum]; // Stockfish には無い。
                {
                    ...
                    for(int32_t i=0; i<ColorNum; i++){
                        if(pointerIndex+size<=length){
                            int32_t value = stateInfo->continuousCheck[i];
                            memcpy(ret + pointerIndex, &value, size);
                            pointerIndex+=size;
                        }else{
                            printf("length error: continuousCheck: %d, %d\n", pointerIndex, length);
                        }
                    }
                }
                ...
                // Key boardKey;
                {
                    ...
                    if(pointerIndex+size<=length){
                        u64 value = stateInfo->boardKey;
                        memcpy(ret + pointerIndex, &value, size);
                        pointerIndex+=size;
                    }else{
                        printf("length error: boardKey: %d, %d\n", pointerIndex, length);
                    }
                }
                // Key handKey;
                {
                    int32_t size = sizeof(u64);
                    if(pointerIndex+size<=length){
                        u64 value = stateInfo->handKey;
                        memcpy(ret + pointerIndex, &value, size);
                        pointerIndex+=size;
                    }else{
                        printf("length error: handKey: %d, %d\n", pointerIndex, length);
                    }
                }
                // Bitboard checkersBB; // 手番側の玉へ check している駒の Bitboard
                {
                    ...
                    if(pointerIndex+size<=length){
                        u64* value = stateInfo->checkersBB.getValue();
                        memcpy(ret + pointerIndex, value, size);
                        pointerIndex+=size;
                    }else{
                        printf("length error: checkersBB: %d, %d\n", pointerIndex, length);
                    }
                }
                ...
                // Piece capturedPiece;
                {
                    int32_t size = sizeof(int32_t);
                    if(pointerIndex+size<=length){
                        int32_t value = 0;// TODO stateInfo->capturedPiece;
                        memcpy(ret + pointerIndex, &value, size);
                        pointerIndex+=size;
                    }else{
                        printf("length error: capturedPiece: %d, %d\n", pointerIndex, length);
                    }
                }
                ...
                // Hand hand; // 手番側の持ち駒
                {
                    ...
                    if(pointerIndex+size<=length){
                        u32 value = stateInfo->hand.value_;
                        memcpy(ret + pointerIndex, &value, size);
                        pointerIndex+=size;
                    }else{
                ...
                // ChangedLists cl;
                {
                    int32_t size = sizeof(ChangedLists);
                    if(pointerIndex+size<=length){
                        ChangedLists value = stateInfo->cl;
                        memcpy(ret + pointerIndex, &value, size);
                        pointerIndex+=size;
                    }else{
                        printf("length error: ChangedLists: %d, %d %d\n", pointerIndex, length, size);
                    }
                }
            }
            byteArray = ret;
            ...
            while (pointerIndex < length) {
                ...
                switch (index) {
                        ...
                        // Score material; // stocfish の npMaterial は 先手、後手の点数を配列で持っているけど、
                    {
                        int32_t size = sizeof(int32_t);
                        if(pointerIndex+size<=length){
                            memcpy(&st->material, positionBytes + pointerIndex, size);
                            pointerIndex+=size;
                        }else{
                            printf("length error: material: %d, %d\n", pointerIndex, length);
                        ...
                        // int32_t continuousCheck[ColorNum]; // Stockfish には無い。
                    {
                        ...
                        for(int32_t i=0; i<ColorNum; i++){
                            if(pointerIndex+size<=length){
                                memcpy(&st->continuousCheck[i], positionBytes + pointerIndex, size);
                                pointerIndex+=size;
                            }else{
                                printf("length error: continuousCheck: %d, %d\n", pointerIndex, length);
                                isParseCorrect = false;
                        ...
                        // Key boardKey;
                    {
                        int32_t size = sizeof(u64);
                        if(pointerIndex+size<=length){
                            memcpy(&st->boardKey, positionBytes + pointerIndex, size);
                            pointerIndex+=size;
                        }else{
                            printf("length error: boardKey: %d, %d\n", pointerIndex, length);
                            isParseCorrect = false;
                        ...
                        // Key handKey;
                    {
                        int32_t size = sizeof(u64);
                        if(pointerIndex+size<=length){
                            memcpy(&st->handKey, positionBytes + pointerIndex, size);
                            pointerIndex+=size;
                        }else{
                            printf("length error: handKey: %d, %d\n", pointerIndex, length);
                            isParseCorrect = false;
                        ...
                        // Bitboard checkersBB; // 手番側の玉へ check している駒の Bitboard
                    {
                        int32_t size = 2*sizeof(u64);// bitboardSize
                        if(pointerIndex+size<=length){
                            memcpy(st->checkersBB.getValue(), positionBytes + pointerIndex, size);
                            pointerIndex+=size;
                        }else{
                            printf("length error: checkersBB: %d, %d\n", pointerIndex, length);
                            isParseCorrect = false;
                        ...
                        // Hand hand; // 手番側の持ち駒
                    {
                        int32_t size = sizeof(u32);
                        if(pointerIndex+size<=length){
                            memcpy(&st->hand.value_, positionBytes + pointerIndex, size);
                            pointerIndex+=size;
                        }else{
                            printf("length error: hand: %d, %d\n", pointerIndex, length);
                            isParseCorrect = false;
                        ...
                        // ChangedLists cl;
                    {
                        int32_t size = sizeof(ChangedLists);
                        if(pointerIndex+size<=length){
                            memcpy(&st->cl, positionBytes + pointerIndex, size);
                            pointerIndex+=size;
                        }else{
                            printf("length error: ChangedLists: %d, %d\n", pointerIndex, length);
                            isParseCorrect = false;
                        }
                    }
                        break;
                    default:
                    {
                        // printf("error, convertByteArrayToStateInfo unknown index: %d\n", index);
                        alreadyPassAll = true;
                    }
                        break;
                }
    ...
    public:
        ...
        u8 getBits(const int32_t numOfBits) {
            // assert(numOfBits <= 8);
            if(!(numOfBits <= 8)){
                printf("error, assert(numOfBits <= 8)\n");
            }
        ...
        void putBits(u8 val, const int32_t numOfBits) {
            // assert(numOfBits <= 8);
            if(!(numOfBits <= 8)){
                printf("error, assert(numOfBits <= 8)\n");
            }
        ...
        int32_t curr() const { return curr_; }
        
    private:
        u8* data_;
        int32_t curr_; // 1byte 中の bit の位置
    };
    ...
    public:
        ...
        FORCE_INLINE Square kingSquare(const Color c) const {
            ...
            if(!(kingSquare_[c] == bbOf(King, c).constFirstOneFromSQ11())){
                printf("error, assert(kingSquare_[c] == bbOf(King, c).constFirstOneFromSQ11())\n");
            }
        ...
        Score see(const Move move, const int32_t asymmThreshold = 0) const;
        ...
        void myToSFEN(const Ply ply, char* ss) const;
        void myToSFEN(char* ss) const { myToSFEN(gamePly(), ss); }
        
        HuffmanCodedPos toHuffmanCodedPos() const;
        
        s64 nodesSearched() const          { return nodes_; }
        void setNodesSearched(const s64 n) { nodes_ = n; }
        RepetitionType isDraw(const int32_t checkMaxPly = std::numeric_limits<int32_t>::max()) const;
        
        Thread* thisThread() const { return thisThread_; }
        
        void setStartPosPly(const Ply ply) { gamePly_ = ply; }
        
        static constexpr int32_t nlist() { return EvalList::ListSize; }
        EvalIndex list0(const int32_t index) const { return evalList_.list0[index]; }
        EvalIndex list1(const int32_t index) const { return evalList_.list1[index]; }
        int32_t squareHandToList(const Square sq) const { return evalList_.squareHandToList[sq]; }
        Square listToSquareHand(const int32_t i) const { return evalList_.listToSquareHand[i]; }
    ...
    private:
        ...
        void setHand(const HandPiece hp, const Color c, const int32_t num) { hand_[c].orEqual(num, hp); }
        void setHand(const Piece piece, const int32_t num) {
            const Color c = pieceToColor(piece);
            const PieceType pt = pieceToPieceType(piece);
        ...
        int32_t debugSetEvalList() const;
    ...
    public:
        ...
        static int32_t parse(Position* position, u8* positionBytes, int32_t length, int32_t start, bool canCorrect);
    };
    ...
    const int32_t MaxLegalMoves = 593 + 1;
