...
namespace Shogi {
    ...
    public:
        ...
        HandPiece handPieceDropped() const {
            // TODO assert(this->isDrop());
            if(!(this->isDrop())){
                printf("error, assert(this->isDrop())\n");
            }
    ...
    template <typename T, bool UseSentinel = false> inline void insertionSort(T first, T last) {
        if (UseSentinel){
            // TODO assert(std::max_element(first - 1, last) == first - 1); // 番兵が最大値となることを確認
            if(!(std::max_element(first - 1, last) == first - 1)){
                printf("error, assert(std::max_element(first - 1, last) == first - 1)\n");
            }
        }
        if (first != last) {
