...
#ifndef FAIRY_CHESS_BITBOARD_HPP_INCLUDED
#define FAIRY_CHESS_BITBOARD_HPP_INCLUDED
...
namespace FairyChess
{
    ...
    extern int32_t SquareDistance[SQUARE_NB][SQUARE_NB];
    ...
    inline Bitboard operator&(Bitboard b, Square s) {
        ...
        if(!(s >= SQ_A1 && s <= SQ_H8)){
            // printf("error, assert(s >= SQ_A1 && s <= SQ_H8)\n");
            s = SQ_A1;
        }
        return b & SquareBB[s];
    }
    ...
    template<typename T> inline int32_t distance(T x, T y) { return x < y ? y - x : x - y; }
    template<> inline int32_t distance<Square>(Square x, Square y) { return SquareDistance[x][y]; }
    
    template<typename T1, typename T2> inline int32_t distanceT(T2 x, T2 y);
    template<> inline int32_t distanceT<File>(Square x, Square y) { return distance(file_of(x), file_of(y)); }
    template<> inline int32_t distanceT<Rank>(Square x, Square y) { return distance(rank_of(x), rank_of(y)); }
    ...
    template<PieceType Pt> inline Bitboard attacks_bb(Square s, Bitboard occupied) {
        ...
        if(!(Pt == BISHOP || Pt == ROOK)){
            printf("error, assert(Pt == BISHOP || Pt == ROOK)\n");
        }
    ...
    inline int32_t popcount(Bitboard b) {
