...
#ifndef CHESS_STOCK_FISH_TYPES_H_INCLUDED
#define CHESS_STOCK_FISH_TYPES_H_INCLUDED
...
#  define pext(b, m) 0
    ...
    // TODO co ve chuyen max_ply ve thap hon se chinh xac, tuy tung core tren github co max_ply khac nhau, de cao hon se tran bo nho
    const int32_t MAX_PLY   = 32;
    
    // TODO tao move chi la 1 so nhu tren cung on
    ...
    /// care to avoid left-shifting a signed int32_t to avoid undefined behavior.
...
constexpr T operator*(int32_t i, T d) { return T(i * int(d)); }        \
constexpr T operator*(T d, int32_t i) { return T(int32_t(d) * i); }        \
constexpr T operator/(T d, int32_t i) { return T(int32_t(d) / i); }        \
constexpr int32_t operator/(T d1, T d2) { return int(d1) / int(d2); }  \
