...
namespace Shatranj
{
    ...
    int32_t Tablebases::MaxCardinality;
    ...
    namespace {
        ...
        inline Square operator^=(Square& s, int32_t i) { return s = Square(int32_t(s) ^ i); }
        inline Square operator^(Square s, int32_t i) { return Square(int32_t(s) ^ i); }
        
        // DTZ tables don't store valid scores for moves that reset the rule50 counter
        // like captures and pawn moves but we can easily recover the correct dtz of the
        // previous move if we know the position's WDL score.
        int32_t dtz_before_zeroing(WDLScore wdl) {
            return wdl == WDLWin         ?  1   :
        ...
        template <typename T> int32_t sign_of(T val) {
        ...
        const int32_t TBPIECES = 6;
        
        struct PairsData {
            int32_t flags;
            size_t sizeofBlock;            // Block size in bytes
            size_t span;                   // About every span values there is a SparseIndex[] entry
            int32_t blocksNum;                 // Number of blocks in the TB file
            int32_t maxSymLen;                 // Maximum length in bits of the Huffman symbols
            int32_t minSymLen;                 // Minimum length in bits of the Huffman symbols
            Sym* lowestSym;                // lowestSym[l] is the symbol of length l with the lowest value
            LR* btree;                     // btree[sym] stores the left and right symbols that expand sym
            uint16_t* blockLength;         // Number of stored positions (minus one) for each block: 1..65536
            int32_t blockLengthSize;           // Size of blockLength[] table: padded so it's bigger than blocksNum
            ...
            int32_t groupLen[TBPIECES+1];      // Number of pieces in a given group: KRKN -> (3, 1)
        };
        ...
        auto item(WDLPieceTable& e, int32_t stm, int32_t  ) -> decltype(e[stm])& { return e[stm]; }
        auto item(DTZPieceTable& e, int32_t    , int32_t  ) -> decltype(e)& { return e; }
        auto item(WDLPawnTable&  e, int32_t stm, int32_t f) -> decltype(e.file[stm][f])& { return e.file[stm][f]; }
        auto item(DTZPawnTable&  e, int32_t    , int32_t f) -> decltype(e.file[f])& { return e.file[f]; }
        
        template<typename E> struct Ret { typedef int32_t type; };
        template<> struct Ret<WDLEntry> { typedef WDLScore type; };

        int32_t MapPawns[SQUARE_NB];
        int32_t MapB1H1H7[SQUARE_NB];
        int32_t MapA1D1D4[SQUARE_NB];
        int32_t MapKK[10][SQUARE_NB]; // [MapA1D1D4][SQUARE_NB]
        
        // Comparison function to sort leading pawns in ascending MapPawns[] order
        bool pawns_comp(Square i, Square j) { return MapPawns[i] < MapPawns[j]; }
        int32_t off_A1H8(Square sq) { return int32_t(rank_of(sq)) - file_of(sq); }
        ...
        int32_t Binomial[6][SQUARE_NB];    // [k][n] k elements from a set of n elements
        int32_t LeadPawnIdx[5][SQUARE_NB]; // [leadPawnsCnt][SQUARE_NB]
        int32_t LeadPawnsSize[5][4];       // [leadPawnsCnt][FILE_A..FILE_D]
        
        enum { BigEndian, LittleEndian };
        
        template<typename T, int32_t Half = sizeof(T) / 2, int32_t End = sizeof(T) - 1>
        ...
        template<typename T, int32_t LE> T number(void* addr)
        {
        ...
        class HashTable {
            ...
            static const int32_t TBHASHBITS = 10;
            static const int32_t HSHMAX     = 5;
            ...
            void insert(Key key, WDLEntry* wdl, DTZEntry* dtz) {
                ...
                for (int32_t i = 0; i < HSHMAX; ++i, ++entry)
        ...
        public:
            template<typename E, int32_t I = std::is_same<E, WDLEntry>::value ? 0 : 1>
            E* get(Key key) {
                Entry* entry = hashTable[key >> (64 - TBHASHBITS)];
        ...
        int32_t decompress_pairs(PairsData* d, uint64_t idx) {
            ...
            int32_t offset     = number<uint16_t, LittleEndian>(&d->sparseIndex[k].offset);
            ...
            int32_t diff = idx % d->span - d->span / 2;
            ...
            while (true) {
                int32_t len = 0; // This is the symbol length - d->min_sym_len
        ...
        bool check_dtz_stm(DTZEntry* entry, int32_t stm, File f) {

            int32_t flags = entry->hasPawns ? entry->pawnTable.file[f].precomp->flags
        ...
        WDLScore map_score(WDLEntry*, File, int32_t value, WDLScore) { return WDLScore(value - 2); }

        int32_t map_score(DTZEntry* entry, File f, int32_t value, WDLScore wdl) {
            
            const int32_t WDLMap[] = { 1, 3, 0, 2, 0 };
        ...
        T do_probe_table(const Position& pos, Entry* entry, WDLScore wdl, ProbeState* result) {
            ...
            uint64_t idx;
            int32_t next = 0, size = 0, leadPawnsCnt = 0;
            PairsData* d;
            ...
            int32_t flipColor   = (symmetricBlackToMove || blackStronger) * 8;
            int32_t flipSquares = (symmetricBlackToMove || blackStronger) * 070;
            int32_t stm         = (symmetricBlackToMove || blackStronger) ^ pos.side_to_move();
            ...
            if (entry->hasPawns) {
                ...
                for (int32_t i = 1; i < leadPawnsCnt; ++i)
            ...
            for (int32_t i = 0; i < d->groupLen[0]; ++i) {
            ...
            if (entry->hasUniquePieces) {
                ...
                int32_t adjust2 = (squares[2] > squares[0]) + (squares[2] > squares[1]);
        ...
        encode_remaining:
            ...
            while (d->groupLen[++next])
            {
                ...
                for (int32_t i = 0; i < d->groupLen[next]; ++i)
                {
        ...
        template<typename T> void set_groups(T& e, PairsData* d, int32_t order[], File f) {

            int32_t n = 0, firstLen = e.hasPawns ? 0 : e.hasUniquePieces ? 3 : 2;
            d->groupLen[n] = 1;
            ...
            int32_t next = pp ? 2 : 1;
            int32_t freeSquares = 64 - d->groupLen[0] - (pp ? d->groupLen[1] : 0);
            uint64_t idx = 1;
        ...
        uint8_t* set_sizes(PairsData* d, uint8_t* data) {
            ...
            int32_t padding = number<uint8_t, LittleEndian>(data++);
            ...
            for (int32_t i = d->base64.size() - 2; i >= 0; --i) {
                d->base64[i] = (d->base64[i + 1] + number<Sym, LittleEndian>(&d->lowestSym[i])
                                - number<Sym, LittleEndian>(&d->lowestSym[i + 1])) / 2;
                
                // assert(d->base64[i] * 2 >= d->base64[i+1]);
                if(!(d->base64[i] * 2 >= d->base64[i+1])){
                    printf("error, assert(d->base64[i] * 2 >= d->base64[i+1])\n");
                }
            }
        ...
        template<typename T> uint8_t* set_dtz_map(WDLEntry&, T&, uint8_t*, File) { return nullptr; }
        
        template<typename T> uint8_t* set_dtz_map(DTZEntry&, T& p, uint8_t* data, File maxFile) {
            
            p.map = data;
            
            for (File f = FILE_A; f <= maxFile; ++f) {
                if (item(p, 0, f).precomp->flags & TBFlag::Mapped)
                    for (int32_t i = 0; i < 4; ++i) { // Sequence like 3,x,x,x,1,x,0,2,x,x
        ...
        template<typename Entry, typename T> void do_init(Entry& e, T& p, uint8_t* data) {
            ...
            if(!(e.hasPawns        == !!(*data & HasPawns))){
                printf("error, assert(e.hasPawns        == !!(*data & HasPawns))\n");
            }
            // assert((e.key != e.key2) == !!(*data & Split));
            if(!((e.key != e.key2) == !!(*data & Split))){
                printf("error, assert((e.key != e.key2) == !!(*data & Split))\n");
            }
            
            data++; // First byte stores flags
            
            const int32_t Sides = IsWDL && (e.key != e.key2) ? 2 : 1;
            const File MaxFile = e.hasPawns ? FILE_D : FILE_A;
            
            bool pp = e.hasPawns && e.pawnTable.pawnCount[1]; // Pawns on both sides
            
            // assert(!pp || e.pawnTable.pawnCount[0]);
            if(!(!pp || e.pawnTable.pawnCount[0])){
                printf("error, assert(!pp || e.pawnTable.pawnCount[0])\n");
            }
            ...
            for (File f = FILE_A; f <= MaxFile; ++f) {
                ...
                int32_t order[][2] = { { *data & 0xF, pp ? *(data + 1) & 0xF : 0xF },
        ...
        template<typename Entry> void* init(Entry& e, const Position& pos) {
        ...
        WDLScore search(bool CheckZeroingMoves, Position& pos, ProbeState* result) {
    ...
    void Tablebases::init(const std::string& paths) {
        ...
        for (int32_t n = 1; n < 64; n++) // Squares
            for (int32_t k = 0; k < 6 && k <= n; ++k) // Pieces
        ...
        int32_t availableSquares = 47; // Available squares when lead pawn is in a2
        
        // Init the tables for the encoding of leading pawns group: with 6-men TB we
        // can have up to 4 leading pawns (KPPPPK).
        for (int32_t leadPawnsCnt = 1; leadPawnsCnt <= 4; ++leadPawnsCnt)
            for (File f = FILE_A; f <= FILE_D; ++f)
            {
    ...
    int32_t Tablebases::probe_dtz(Position& pos, ProbeState* result) {
        ...
        *result = OK;
        ...
        int32_t dtz = probe_table<DTZEntry>(pos, result, wdl);
    ...
    static int32_t has_repeated(StateInfo *st)
    {
        while (1) {
            int32_t i = 4, e = min(st->rule50, st->pliesFromNull);
    ...
    bool Tablebases::root_probe(Position& pos, Search::RootMoves& rootMoves, Value& score)
    {
        ...
        if(!(rootMoves.size())){
            printf("error, assert(rootMoves.size())\n");
        }
        
        ProbeState result;
        int32_t dtz = probe_dtz(pos, &result);
        ...
        int32_t cnt50 = st.previous ? st.previous->rule50 : 0;
