...
#ifndef shatranj_bitboard_hpp
#define shatranj_bitboard_hpp
...
namespace Shatranj
{
    ...
    template<typename T> inline int32_t distance(T x, T y)
    {
        return x < y ? y - x : x - y;
    }
    
    template<> inline int32_t distance<Square>(Square x, Square y) {
        return SquareDistance[x][y];
    }
    
    template<typename T1, typename T2> inline int32_t distanceT(T2 x, T2 y);
    
    template<> inline int32_t distanceT<File>(Square x, Square y)
    {
        return distance(file_of(x), file_of(y));
    }
    
    template<> inline int32_t distanceT<Rank>(Square x, Square y) {
