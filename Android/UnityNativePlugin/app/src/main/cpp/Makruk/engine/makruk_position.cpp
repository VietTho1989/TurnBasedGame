...
namespace Makruk
{
    ...
    void Position::do_move(Move m, StateInfo& newSt, bool givesCheck) {
        ...
        {
            Square capsq = to;
            int32_t old_counting_limit = count<ALL_PIECES>(~color_of(captured)) == 1 ? counting_limit() : 0;
    ...
    int32_t Position::parseByteArray(Position* position, uint8_t* bytes, int32_t length, int32_t start, bool canCorrect)
    {
        ...
        if(canCorrect){
            ...
            if(!position->myPosIsOk()){
                ...
                int32_t correctLength = makruk_makePositionByFen(fen, chess960, correctPositionBytes);
                // convert again
