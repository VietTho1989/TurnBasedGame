...
#include "../cchess/xiangqi_cchess.hpp"
#include "xiangqi_pgnfile.hpp"
...
namespace Xiangqi
{
    ...
    static bool GetLabel(char *szDestStr, const char *szLineStr, const char *szLabelName) {
        ...
        if (StrEqvSkip(szLineStr, szTempLabel)) {
            ...
            if (lpLabelEnd == NULL) {
                nValueLen = (int)(strlen(szLineStr) - 1);
            } else {
                nValueLen = (int)(lpLabelEnd - szLineStr);
            }
    ...
    bool PgnFileStruct::Read(const char *szFileName, bool bNoAdvert) {
        FILE *fp;
        int32_t nRemLevel, nRemLen;
        bool bReturned, bDetail, bEndFor;
        int32_t nNotation, nCounter, nStatus, mv;
                            ...
                            default:
                                if (nNotation > 0) {
                                    if ((*lpLineChar >= 'A' && *lpLineChar <= 'Z') || (*lpLineChar >= 'a' && *lpLineChar <= 'z') || *lpLineChar == '+' || *lpLineChar == '-' || *lpLineChar == '=') {
                                        if (nNotation == 1) {
                                            mv = File2Move((uint32_t)(*(long*)lpLineChar), pos);
                                        } else {
