...
#include "fairy_chess_material.hpp"
...
namespace FairyChess
{
    ...
    namespace {
        ...
        constexpr int32_t KingAttackWeights[PIECE_TYPE_NB] = { 0, 0, 77, 55, 44, 10 };
        
        // Penalties for enemy's safe checks
        constexpr int32_t QueenSafeCheck  = 780;
        constexpr int32_t RookSafeCheck   = 880;
        constexpr int32_t BishopSafeCheck = 435;
        constexpr int32_t KnightSafeCheck = 790;
...
#define S(mg, eg) make_score(mg, eg)
        ...
        constexpr int32_t PassedDanger[RANK_NB] = { 0, 0, 0, 3, 6, 12, 21 };
...
#undef S
        ...
        private:
            ...
            int32_t kingAttacksCount[COLOR_NB];
        };
        ...
        Score Evaluation<T>::king() const {
            ...
            if (kingAttackersCount[Them] > 1 - pos->count<QUEEN>(Them))
            {
                ...
                if (kingDanger > 0)
                {
                    int32_t mobilityDanger = mg_value(mobility[Them] - mobility[Us]);
        ...
        Score Evaluation<T>::passed() const {
            ...
            auto king_proximity = [&](Color c, Square s) {
                return pos->count<KING>(c) ? min(distance(pos->square<KING>(c), s), 5) : 5;
            };
            ...
            while (b)
            {
                ...
                int32_t r = relative_rank(Us, s);
                int32_t w = PassedDanger[r];
                ...
                if (w)
                {
                    ...
                    if (pos->empty(blockSq))
                    {
                        ...
                        int32_t k = !unsafeSquares ? 20 : !(unsafeSquares & blockSq) ? 9 : 0;
            ...
            {
                maxMg = max(maxMg, PieceValue[MG][pt]);
                maxEg = max(maxEg, PieceValue[EG][pt]);
            }
        ...
        Score Evaluation<T>::space() const {
            ...
            int32_t bonus = popcount(safe) + popcount(behind & safe);
        ...
        Score Evaluation<T>::variant() const {
            ...
            if (pos->capture_the_flag(Us))
            {
                ...
                while (target_squares)
                {
                    ...
                    int32_t dist =  distance(pos->square<KING>(Us), s)
            ...
            {
                int32_t remainingChecks = pos->max_check_count() - pos->checks_given(Us);
                // assert(remainingChecks > 0);
                if(!(remainingChecks > 0)){
                    printf("error, assert(remainingChecks > 0)\n");
                }
        ...
        template<Tracing T> Score Evaluation<T>::initiative(Value eg) const {

            int32_t outflanking = !pos->count<KING>(WHITE) || !pos->count<KING>(BLACK) ? 0
            :  distanceT<File>(pos->square<KING>(WHITE), pos->square<KING>(BLACK))
            ...
            int32_t complexity =   8 * outflanking
            +  8 * pe->pawn_asymmetry()
            + 12 * pos->count<PAWN>()
            ...
            int32_t v = ((eg > 0) - (eg < 0)) * max(complexity, -abs(eg));
        ...
        ScaleFactor Evaluation<T>::scale_factor(Value eg) const {
            ...
            int32_t sf = me->scale_factor(pos, strongSide);
            ...
            if (sf == SCALE_FACTOR_NORMAL)
            {
                if (pos->opposite_bishops())
                {
                    ...
                    sf = min(40 + 7 * pos->count<PAWN>(strongSide), sf);
            }
        ...
        template<Tracing T> Value Evaluation<T>::value() {
            ...
            if(pos->is_variant_end()){
                printf("error, assert(!pos->is_variant_end())\n");
            }
