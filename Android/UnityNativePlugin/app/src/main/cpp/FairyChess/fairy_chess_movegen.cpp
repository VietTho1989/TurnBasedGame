...
namespace FairyChess
{
    namespace {
        ...
        ExtMove* generate_castling(Position* pos, ExtMove* moveList, Color us) {
        ...
        ExtMove* make_promotions(Position* pos, ExtMove* moveList, Square to) {
        ...
        ExtMove* generate_drops(Position* pos, ExtMove* moveList, PieceType pt, Bitboard b) {
        ...
        ExtMove* generate_pawn_moves(Position* pos, ExtMove* moveList, Bitboard target) {
            ...
            if (Type == CAPTURES || Type == EVASIONS || Type == NON_EVASIONS)
            {
                ...
                if (pos->ep_square() != SQ_NONE)
                {
                    ...
                    if(rank_of(pos->ep_square()) != relative_rank(Us, RANK_6)){
                        printf("error, assert(rank_of(pos->ep_square()) == relative_rank(Us, RANK_6))\n");
                    }
        ...
        template<bool Checks> ExtMove* generate_moves(Position* pos, ExtMove* moveList, Color us, PieceType pt, Bitboard target) {
        ...
        template<Color Us, GenType Type>
        ExtMove* generate_all(Position* pos, ExtMove* moveList, Bitboard target) {
    ...
    template<GenType Type> ExtMove* generate(Position* pos, ExtMove* moveList) {
    ...
    template ExtMove* generate<CAPTURES>(Position*, ExtMove*);
    template ExtMove* generate<QUIETS>(Position*, ExtMove*);
    template ExtMove* generate<NON_EVASIONS>(Position*, ExtMove*);
    
    
    /// generate<QUIET_CHECKS> generates all pseudo-legal non-captures and knight
    /// underpromotions that give check. Returns a pointer to the end of the move list.
    template<> ExtMove* generate<QUIET_CHECKS>(Position* pos, ExtMove* moveList) {
    ...
    template<> ExtMove* generate<EVASIONS>(Position* pos, ExtMove* moveList) {
    ...
    template<> ExtMove* generate<LEGAL>(Position* pos, ExtMove* moveList) {
