...
#define MAX_BYOYOMI_TIME_RATIO 1.1
    ...
    bool time_parse(struct time_info *ti, char *s)
    {
        ...
        switch (s[0]) {
            case '=':
                ti->dim = TD_GAMES;
                ti->len.games = (int)strtol(++s, &end, 10);
                ti->len.games_max = 0;
                if (*end == ':')
                    ti->len.games_max = (int)strtol(end + 1, &end, 10);
                if (*end) return false;
                break;
            default:
                ti->dim = TD_WALLTIME;
                ...
                if (ti->period == TT_TOTAL) {
                ...
                } else {
                    {
                        ...
                        if(!(ti->period == TT_MOVE)){
                            printf("error, assert(ti->period == TT_MOVE)\n");
                            ti->period = TT_MOVE;
                        }
                    }
        ...
        if (main_time < 0) {
            ...
            if (byoyomi_time > 0) {
            ...
            } else {
                {
                    ...
                    if(!(!ti->len.t.byoyomi_periods && !ti->len.t.byoyomi_stones)){
                        printf("error, assert(!ti->len.t.byoyomi_periods && !ti->len.t.byoyomi_stones)\n");
                        ti->len.t.byoyomi_periods = 0;
                        ti->len.t.byoyomi_stones = 0;
                    }
                }
    ...
    {
        {
            ...
            if(!(ti->period != TT_NULL)){
                printf("error, assert(ti->period != TT_NULL)\n");
                ti->period = TT_TOTAL;
            }
        }
        ti->dim = TD_WALLTIME;
    ...
    {
        {
            ...
            if(!(ti->dim == TD_WALLTIME && ti->period != TT_NULL)){
                printf("error, assert(ti->dim == TD_WALLTIME && ti->period != TT_NULL)\n");
                ti->dim = TD_WALLTIME;
                ti->period = TT_TOTAL;
            }
        }
        ...
        if (ti->period == TT_TOTAL) {
            ...
            if (ti->len.t.byoyomi_time <= 0) {
                ...
                printf("*** LOST ON TIME internally! (%0.2f, spent %0.2fs on last move)\n", ti->len.t.main_time, interval);
...
/*#if _POSIX_TIMERS > 0
    ...
    bool time_in_byoyomi(struct time_info *ti) {
        {
            ...
            if(!(ti->dim == TD_WALLTIME)){
                printf("error, time_in_byoyomi(struct time_info *ti)\n");
                ti->dim = TD_WALLTIME;
            }
        }
            ...
            return false; // there is no byoyomi!
        {
            ...
            if(!(ti->len.t.byoyomi_stones > 0)){
                printf("error, assert(ti->len.t.byoyomi_stones > 0)\n");
                ti->len.t.byoyomi_stones = 1;
            }
        }
    ...
    // bo static
    int32_t time_stop_set_remaining(struct time_info *ti, struct board *b, double net_lag, struct time_stop *stop)
    {
        int32_t moves_left = board_estimated_moves_left(b);
        ...
        if (real_move_time > 0) {
            int32_t main_moves = stop->worst.time / real_move_time;
    ...
    // bo static
    void time_stop_phase_adjust(struct board *b, int32_t fuseki_end, int32_t yose_start, struct time_stop *stop)
    {
        int32_t bsize = (board_size(b)-2)*(board_size(b)-2);
        ...
        if (b->moves >= yose_start)
            return;
        int32_t moves_to_yose = (yose_start - b->moves) / 2;
        // ^- /2 because we only consider the moves we have to play ourselves
        int32_t left_at_yose_start = board_estimated_moves_left(b) - moves_to_yose;
        ...
        if (b->moves < fuseki_end) {
            {
                // assert(fuseki_end > 0);
                if(!(fuseki_end > 0)){
                    printf("error, assert(fuseki_end > 0)\n");
                    fuseki_end = 1;
                }
            }
            ...
            {
                // assert(b->moves < yose_start);
                if(!(b->moves < yose_start)){
                    printf("error, assert(b->moves < yose_start)\n");
                }
            }
    ...
    void lag_adjust(double *time, double net_lag)
    {
    ...
    void time_stop_conditions(struct time_info *ti, struct board *b, int32_t fuseki_end, int32_t yose_start, floating_t max_maintime_ratio, struct time_stop *stop)
    {
            ...
            if (ti->len.games_max) {
                {
                    ...
                    if(!(ti->len.games_max > ti->len.games)){
                        printf("error, assert(ti->len.games_max > ti->len.games)\n");
                    }
                }
        ...
        
        {
            // assert(ti->dim == TD_WALLTIME);
            if(!(ti->dim == TD_WALLTIME)){
                printf("error, assert(ti->dim == TD_WALLTIME)\n");
                ti->dim = TD_WALLTIME;
            }
        }
        ...
        if (ti->period == TT_MOVE) {
        ...
        } else {
            {
                ...
                if(!(ti->period == TT_TOTAL)){
                    printf("error, assert(ti->period == TT_TOTAL)\n");
                    ti->period = TT_TOTAL;
                }
            }
            /* We are in main time. */
            
            {
                ...
                if(!(ti->len.t.main_time > 0)){
                    printf("error, assert(ti->len.t.main_time > 0)\n");
                }
            }
            /* Set worst.time to all available remaining time, to be spread
             * over returned number of moves. */
            int32_t moves_left = time_stop_set_remaining(ti, b, net_lag, stop);
            ...
            stop->desired.time = stop->worst.time / moves_left;
            {
                ...
                if(!(stop->desired.time > 0 && stop->worst.time > 0)){
                    printf("error, assert(stop->desired.time > 0 && stop->worst.time > 0)\n");
                }
            }
            {
                // assert(stop->desired.time <= stop->worst.time + 0.001);
                if(!(stop->desired.time <= stop->worst.time + 0.001)){
                    printf("error, assert(stop->desired.time <= stop->worst.time + 0.001)\n");
                }
            }
        ...
        if (DEBUGL(1)){
            /*TODO printf("desired %0.2f, worst %0.2f, clock [%d] %0.2f + %0.2f/%d*%d, lag %0.2f\n", stop->desired.time, stop->worst.time, ti->dim, ti->len.t.main_time, ti->len.t.byoyomi_time, ti->len.t.byoyomi_stones, ti->len.t.byoyomi_periods, net_lag);*/
        }
