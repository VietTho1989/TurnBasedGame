...
namespace weiqi
{
    ...
    // bo static
    void uct_pondering_start(struct uct *u, struct board *b0, struct tree *t, enum stone color);
    
    /* Maximal simulation length. */
#define MC_GAMELEN	MAX_GAMELEN
    
    // bo static
    void setup_state(struct uct *u, struct board *b, enum stone color)
    {
        u->t = tree_init(b, color, u->fast_alloc ? u->max_tree_size : 0, u->max_pruned_size, u->pruning_threshold, u->local_tree_aging, u->stats_hbits);
        if (u->initial_extra_komi)
            u->t->extra_komi = u->initial_extra_komi;
        if (u->force_seed)
            fast_srandom(u->force_seed);
        if (UDEBUGL(3))
            printf("Fresh board with random seed %llu\n", fast_getseed());
        if (!u->no_tbook && b->moves == 0) {
            if (color == S_BLACK) {
                tree_load(u->t, b);
            } else if (DEBUGL(0)) {
                printf("Warning: First move appears to be white\n");
            }
        }
    }
    
    // bo static
    void reset_state(struct uct *u)
    {
        {
            // assert(u->t);
            if(!u->t){
                printf("error, assert(u->t)\n");
            }
        }
    ...
    // bo static
    void setup_dynkomi(struct uct *u, struct board *b, enum stone to_play)
    {
    ...
    void uct_prepare_move(struct uct *u, struct board *b, enum stone color)
    {
        if (u->t) {
            /* Verify that we have sane state. */
            {
                // assert(b->es == u);
                if(!(b->es == u)){
                    printf("error, assert(b->es == u)\n");
                }
            }
            {
                // assert(u->t && b->moves);
                if(!(u->t && b->moves)){
                    printf("error, assert(u->t && b->moves)\n");
                }
            }
            if (color != stone_other(u->t->root_color)) {
                printf("die, Fatal: Non-alternating play detected %d %d\n", color, u->t->root_color);
            }
    ...
    // bo static
    void get_dead_groups(struct uct *u, struct board *b, struct move_queue *dead, struct move_queue *unclear)
    {
        // enum gj_state gs_array[board_size2(b)];
        enum gj_state* gs_array = new enum gj_state[board_size2(b)];
    ...
    char passIsSafeStr1[] = "score est and official score don't agree";
    char passIsSafeStr2[] = "too many dames";
    char passIsSafeStr3[] = "losing on official score";
    
    // bo static
    bool pass_is_safe(struct uct *u, struct board *b, enum stone color, struct move_queue *mq, float ownermap_score, bool pass_all_alive, char **msg)
    {
        int32_t dame;
        floating_t score = board_official_score_and_dame(b, mq, &dame);
        if (color == S_BLACK)  score = -score;
        //printf("pass_is_safe():  %d score %f   dame: %i\n", color, score, dame);
    ...
    char str1[] = "unclear groups";
    char str2[] = "need to remove opponent dead groups first";
    char str3[] = "unclear point, clarify first";
    char str4[] = "losing on score estimate";
    ...
    // bo static
    void uct_board_print(struct uct* u, struct board *b, FILE *f)
    {
        board_print_ownermap(b, f, (u ? &u->ownermap : NULL));
    }
    
    // bo static
    struct board_ownermap* uct_ownermap(struct uct* u, struct board *b)
    {
    ...
    // bo static
    char* uct_notify_play(struct uct* u, struct board *b, struct move *m, char *enginearg)
    {
        if (!u->t) {
        ...
        /* Promote node of the appropriate move to the tree root. */
        {
            // assert(u->t->root);
            if(!u->t->root){
                printf("error, assert(u->t->root)\n");
            }
        }
        if (u->t->untrustworthy_tree | !tree_promote_at(u->t, b, m->coord)) {
            if (UDEBUGL(3)) {
                if (u->t->untrustworthy_tree)
                    printf("Not promoting move node in untrustworthy tree.\n");
                else
                    printf("Warning: Cannot promote move node! Several play commands in row?\n");
            }
    ...
    // bo static
    char* uct_undo(struct uct* u, struct board *b)
    {
        if (!u->t) return NULL;
        uct_pondering_stop(u);
        u->initial_extra_komi = u->t->extra_komi;
    ...
    {
        {
            ...
            if(using_dcnn(b0)){
                printf("error, assert(!using_dcnn(b0))\n");
            }
        }
        if (UDEBUGL(1)){
            char strStone[20];
            {
                stone2str(strStone, stone_other(color));
            }
            printf("Starting to ponder with color %s\n", strStone);
        }
        u->pondering = true;
        
        /* We need a local board copy to ponder upon. */
        // TODO test callloc
        struct board* b = (struct board*)calloc(1, sizeof(struct board));//malloc2(sizeof(*b));
        board_copy(b, b0);
        
        /* *b0 did not have the genmove'd move played yet. */
        struct move m = { node_coord(t->root), t->root_color };
        int32_t res = board_play(b, &m);
        {
            // assert(res >= 0);
            if(!(res >= 0)){
                printf("error, assert(res >= 0)\n");
            }
        }
        ...
        printf("uct_pondering_start: uct_search_start\n");
    ...
    int32_t uct_search(struct uct *u, struct board *b, struct time_info *ti, enum stone color, struct tree *t, bool print_progress)
    {
        ...
        if (UDEBUGL(2))
        /* TODO printf("(avg score %f/%d; dynkomi's %f/%d value %f/%d)\n", t->avg_score.value, t->avg_score.playouts, u->dynkomi->score.value, u->dynkomi->score.playouts, u->dynkomi->value.value, u->dynkomi->value.playouts);*/
            if (print_progress)
        ...
        if (u->debug_after.playouts > 0) {
            ...
            printf("--8<-- UCT debug post-run begin (%d:%d) --8<--\n", u->debug_after.level, u->debug_after.playouts);

            int32_t debug_level_save = debug_level;
            int32_t u_debug_level_save = u->debug_level;
            int32_t p_debug_level_save = u->playout->debug_level;
            ...
            printf("--8<-- UCT debug post-run finished --8<--\n");
        }
    ...
    void uct_genmove_setup(struct uct *u, struct board *b, enum stone color)
    {
        if (b->superko_violation) {
            printf("!!! WARNING: SUPERKO VIOLATION OCCURED BEFORE THIS MOVE\n");
            printf("Maybe you play with situational instead of positional superko?\n");
            printf("I'm going to ignore the violation, but note that I may miss\n");
            printf("some moves valid under this ruleset because of this.\n");
        ...
        if (u->territory_scoring && (((int)floor(b->komi) + board_size(b)) & 1)) {
            ...
            if (UDEBUGL(0))
                printf("Setting komi to %.1f assuming Japanese rules\n", b->komi);
        }
    }
    
    // tam bo static
    struct tree_node* genmove(struct uct* u, struct board *b, struct time_info *ti, enum stone color, bool pass_all_alive, coord_t *best_coord)
    {
        reset_dcnn_time();
        ...
        int32_t base_playouts = u->t->root->u.playouts;
        int32_t played_games = uct_search(u, b, ti, color, u->t, false);
    ...
    {
        coord_t best_coord;
        struct tree_node *best = genmove(u, b, ti, color, pass_all_alive, &best_coord);
    ...
    {
        struct tree_node** best_d = new struct tree_node*[nbest];
        
        for (int32_t i = 0; i < nbest; i++)  best_c[i] = pass;
        for (int32_t i = 0; i < nbest; i++)  best_r[i] = 0;
        ...
        if (winrates)  /* Get winrates */
            for (int32_t i = 0; i < nbest && best_c[i] != pass; i++)
                best_r[i] = tree_node_get_value(t, 1, best_d[i]->u.value);
        
        // TODO can phai can than
        delete [] best_d;
    }
    
    // TODO de static chac van on
    static void log_nthreads(struct uct *u)
    {
        static int32_t logged = 0;
        if (DEBUGL(0) && !logged++)  printf("Threads: %i\n", u->threads);
    }
    
    // bo static
    size_t default_max_tree_size()
    {
        // TODO test
        // return 0;
        /*int32_t mult = (sizeof(void*) == 4 ? 1 : 2);
        return (size_t)300 * mult * 1048576;*/
        size_t ret = sizeof(tree_node)*DefaultTreeNodeCount;
        // printf("size of tree node: %zu, %zu\n", sizeof(tree_node), ret);
        return ret;
    }
    
    struct uct* uct_state_init(char *arg, struct board *b)
    {
        struct uct* u = (struct uct*)calloc2(1, sizeof(struct uct));
        bool pat_setup = false;
        
        u->debug_level = debug_level;
        u->reportfreq = 1000;
        u->gamelen = MC_GAMELEN;
        u->resign_threshold = 0.1f;// 0.2;
            ...
            while (*next) {
                ...
                if (!strcasecmp(optname, "debug")) {
                    ...
                    if (!strcasecmp(optval, "text")) {
                        ...
                        printf("die, UCT: Invalid reporting format %s\n", optval);
                    }
                    ...
                    if (!strcasecmp(optval, "ucb1")) {
                        ...
                        *p = policy_ucb1amaf_init(u, policyarg, b);
                        ...
                        printf("die, UCT: Invalid tree policy %s\n", optval);
                    }
                    ...
                    if (!strcasecmp(optval, "moggy")) {
                        ...
                        printf("die, UCT: Invalid playout policy %s\n", optval);
                    }
                    ...
                    if (!strcasecmp(optval, "tree")) {
                        ...
                        printf("die, UCT: Invalid thread model %s\n", optval);
                    }
                    ...
                    if (!strcasecmp(optval, "none")) {
                        ...
                        printf("die, UCT: Invalid dynkomi mode %s\n", optval);
                    }
                    ...
                    if (u->tenuki_d > TREE_NODE_D_MAX + 1) {
                        printf("die, uct: tenuki_d must not be larger than TREE_NODE_D_MAX+1 %d\n", TREE_NODE_D_MAX + 1);
                    }
                     ...
                     * inverses) are averaged. */
                        ...
                        printf("die, uct: unknown local_tree_eval %s\n", optval);
                    }
                    ...
                    printf("die, uct: Invalid engine argument %s or missing value\n", optname);
            }
        }
        ...
        if (!!u->random_policy_chance ^ !!u->random_policy)
            printf("die, uct: Only one of random_policy and random_policy_chance is set\n");
        ...
        if (u->slave) {
            ...
            if (!u->shared_nodes) u->shared_nodes = DEFAULT_SHARED_NODES;
            {
                ...
                if(!(u->shared_levels * board_bits2(b) <= 8 * (int)sizeof(path_t))){
                    printf("error, assert(u->shared_levels * board_bits2(b) <= 8 * (int)sizeof(path_t))\n");
                }
            }
        }
        ...
        if (u->pondering_opt && using_dcnn(b)) {
            printf("warning, Can't use pondering with dcnn right now, pondering turned off.\n");
            u->pondering_opt = false;
        }
    ...
    // bo static
    void print_dead_groups(struct uct *u, struct board *b, struct move_queue *mq)
    {
        {
            char strStone[20];
            {
                stone2str(strStone, u->my_color);
            }
            printf("dead groups (playing %s)\n", (u->my_color ? strStone : "???"));
        }
        if (!mq->moves)
            printf("  none\n");
        for (uint32_t i = 0; i < mq->moves; i++) {
