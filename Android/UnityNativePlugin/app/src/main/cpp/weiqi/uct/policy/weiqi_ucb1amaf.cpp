...
namespace weiqi
{
    ...
    struct ucb1_policy_amaf {
        ...
        int32_t virtual_win;
        int32_t root_virtual_win;
        int32_t vwin_min_playouts;
    ...
    // bo static
    inline floating_t fast_sqrt(uint32_t x)
    {
        // bo static
        const floating_t table[] = {
...
#define URAVE_DEBUG if (0)
    // bo static
    inline floating_t ucb1rave_evaluate(struct uct_policy *p, struct tree *tree, struct uct_descent *descent, int32_t parity)
    {
        struct ucb1_policy_amaf* b = (struct ucb1_policy_amaf*)p->data;
        ...
        if (p->uct->virtual_loss) {
            ...
            struct move_stats c;
            {
                c.value = static_cast<float>(parity > 0 ? 0. : 1.);
                c.playouts = static_cast<int>(node->descents * vloss_coeff);
            }
            stats_merge(&n, &c);
        }
        ...
        /* Local tree heuristics. */
        {
            ...
            if(!(!lnode || lnode->parent)){
                printf("error, assert(!lnode || lnode->parent)\n");
            }
        }
        if (p->uct->local_tree && b->ltree_rave > 0 && lnode
            ...
            URAVE_DEBUG{
                char strCoord1[256];
                {
                    coord2sstr(strCoord1, node_coord(lnode), tree->board);
                }
                char strCoord2[256];
                {
                    coord2sstr(strCoord2, node_coord(node), tree->board);
                }
                printf("[ltree] adding [%s] %f%%%d to [%s] RAVE %f%%%d\n", strCoord1, l.value, l.playouts, strCoord2, r.value, r.playouts);
            }
            stats_merge(&r, &l);
        }
        ...
        if (b->crit_rave > 0 && (b->crit_plthres_coef > 0
            ...
            if (b->crit_negative || crit > 0) {
                ...
                struct move_stats c;
                {
                    c.value = tree_node_get_value(tree, parity, val);
                    c.playouts = static_cast<int>(crit * r.playouts * b->crit_rave);
                }
                URAVE_DEBUG{
                    char strCoord[256];
                    {
                    ...
                    printf("[crit] adding %f%%%d to [%s] RAVE %f%%%d\n", c.value, c.playouts, strCoord, r.value, r.playouts);
                }
                stats_merge(&r, &c);
            }
        }
        ...
        if (n.playouts) {
            if (r.playouts) {
                ...
                URAVE_DEBUG{
                    ...
                    printf("\t%s value = %f * %f + (1 - %f) * %f (prior %f)\n", strCoord, beta, r.value, beta, n.value, node->prior.value);
                }
            } else {
                value = n.value;
                URAVE_DEBUG{
                    ...
                    printf("\t%s value = %f (prior %f)\n", strCoord, n.value, node->prior.value);
                }
            }
        } else if (r.playouts) {
            value = r.value;
            URAVE_DEBUG{
                ...
                printf("\t%s value = rave %f (prior %f)\n", strCoord, r.value, node->prior.value);
            }
        }
    ...
    void ucb1rave_descend(struct uct_policy *p, struct tree *tree, struct uct_descent *descent, int32_t parity, bool allow_pass)
    {
        struct ucb1_policy_amaf* b = (struct ucb1_policy_amaf*)p->data;
     ...
     *   W connects the ko */
    // bo static
    inline int32_t ko_length(bool *ko_capture_map, int32_t map_length)
    {
    ...
    void ucb1amaf_update(struct uct_policy *p, struct tree *tree, struct tree_node *node, enum stone node_color, enum stone player_color, struct playout_amafmap *map, struct board *final_board, floating_t result)
    {
         ...
         */
        int32_t* first_map = new int32_t[board_size2(final_board)+1];
        
        int32_t *first_move = &first_map[1]; // +1 for pass
        
#if 0
        struct board bb; bb.size = 9+2;
        for (struct tree_node *ni = node; ni; ni = ni->parent)
            printf("%s ", coord2sstr(node_coord(ni), &bb));
        printf("[color %d] update result %d (color %d)\n", node_color, result, player_color);
#endif
        
        /* Initialize first_move */
        for (int32_t i = pass; i < board_size2(final_board); i++) first_move[i] = INT_MAX;
        int32_t move;
        {
            // assert(map->gamelen > 0);
            if(!(map->gamelen > 0)){
                printf("error, assert(map->gamelen > 0)\n");
            }
        }
        for (move = map->gamelen - 1; move >= map->game_baselen; move--){
            first_move[map->game[move]] = move;
        }
        // printf("ucb1amaf_update: %d, %d, %d\n", map->gamelen, map->game_baselen, move);
        while (node) {
            ...
            int32_t max_threat_dist = b->threat_rave <= 0 ? ko_length(ko_capture_map, map->gamelen - (move+1)) : -1;
             ...
             * matter only at a point when AMAF doesn't help much. */
            {
                ...
                if(!(map->game_baselen >= 0)){
                    printf("error, assert(map->game_baselen >= 0)\n");
                }
            }
            for (struct tree_node *ni = node->children; ni; ni = ni->sibling) {
                ...
                int32_t first = first_move[node_coord(ni)];
                if (first == INT_MAX) continue;
                {
                    // assert(first > move && first < map->gamelen);
                    if(!(first > move && first < map->gamelen)){
                        printf("error, assert(first > move && first < map->gamelen)\n");
                    }
                }
                int32_t distance = first - (move + 1);
...
#if 0
                struct board bb; bb.size = 9+2;
                printf("* %s<%"PRIhash"> -> %s<%"PRIhash"> [%d/%f => %d/%f]\n", coord2sstr(node_coord(node), &bb), node->hash, coord2sstr(node_coord(ni), &bb), ni->hash, player_color, result, move, res);
#endif
            }
            if (node->parent) {
                {
                    // assert(move >= 0 && map->game[move] == node_coord(node) && first_move[node_coord(node)] > move);
                    if(!(move >= 0 && map->game[move] == node_coord(node) && first_move[node_coord(node)] > move)){
                        printf("error, assert(move >= 0 (%d) && map->game[move] (%d) == node_coord(node) (%d) && first_move[node_coord(node)] (%d) > move): %ld\n", move, map->game[move], node_coord(node), first_move[node_coord(node)], (node - tree->nodes)/sizeof(tree_node));
                        // TODO test
                        /*node->coord = map->game[move];
                        move = 1;*/
                    }else{
                        // printf("correct, assert(move >= 0 (%d) && map->game[move] (%d) == node_coord(node) (%d) && first_move[node_coord(node)] (%d) > move): %ld\n", move, map->game[move], node_coord(node), first_move[node_coord(node)], (node - tree->nodes)/sizeof(tree_node));
                    }
                }
    ...
    {
        struct uct_policy* p = (struct uct_policy*)calloc2(1, sizeof(*p));
        struct ucb1_policy_amaf* b = (struct ucb1_policy_amaf*)calloc2(1, sizeof(*b));
        p->uct = u;
        p->data = b;
        ...
        if (arg) {
            ...
            while (*next) {
                ...
                if (!strcasecmp(optname, "explore_p")) {
                    ...
                    printf("die, ucb1amaf: Invalid policy argument %s or missing value\n", optname);
            }
        }
