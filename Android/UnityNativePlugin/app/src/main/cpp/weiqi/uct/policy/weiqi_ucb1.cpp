...
namespace weiqi
{
    ...
    void ucb1_descend(struct uct_policy *p, struct tree *tree, struct uct_descent *descent, int32_t parity, bool allow_pass)
    {
        ...
        struct ucb1_policy* b = (struct ucb1_policy*)p->data;
        ...
        uctd_try_node_children(tree, descent, allow_pass, parity, p->uct->tenuki_d, di, urgency) {
            ...
            int32_t uct_playouts = ni->u.playouts + ni->prior.playouts + ni->descents;
    ...
    void ucb1_update(struct uct_policy *p, struct tree *tree, struct tree_node *node, enum stone node_color, enum stone player_color, struct playout_amafmap *map, struct board *final_board, floating_t result)
    {
    ...
    void ucb1_done(struct uct_policy *p)
    {
        free(p->data);
        free(p);
    }
    ...
    struct uct_policy* policy_ucb1_init(struct uct *u, char *arg)
    {
        ...
        struct ucb1_policy* b = (struct ucb1_policy*)calloc2(1, sizeof(*b));
        p->uct = u;
        p->data = b;
        p->done = ucb1_done;
        ...
        if (arg) {
            ...
            while (*next) {
                ...
                if (!strcasecmp(optname, "explore_p") && optval) {
                    ...
                    printf("die, ucb1: Invalid policy argument %s or missing value\n", optname);
                }
            }
        }
