...
namespace weiqi
{
    ...
    // bo static
    struct tree_node* tree_alloc_node(struct tree *t, int32_t count, bool fast_alloc)
    {
        // printf("tree_alloc_node\n");
        struct tree_node *n = NULL;
        size_t nsize = count * sizeof(*n);
#ifdef _MSC_VER
        size_t old_size = t->nodes_size;
        t->nodes_size+=nsize;
#else
        size_t old_size = __sync_fetch_and_add(&t->nodes_size, nsize);
#endif
        
        // TODO test ko cho fast_alloc het
        // fast_alloc = false;
        if (fast_alloc) {
            if (old_size + nsize >= t->max_tree_size){
                // printf("error, why tree_alloc_node max size: %zu\n", old_size);
                return NULL;
            }
            // printf("node size: %zu, %zu, %zu, %d\n", old_size, nsize, sizeof(struct tree_node), count);
            {
                // assert(t->nodes != NULL);
                if(!(t->nodes != NULL)){
                    printf("error, assert(t->nodes != NULL)\n");
                }
            }
            {
                // init
                {
                    int64_t index = old_size/sizeof(tree_node);
                    int64_t maxTreeNodeCount = t->max_tree_size/sizeof(tree_node);
                    if(index>=0 && index<maxTreeNodeCount){
                        n = &t->nodes[index];
                    }else{
                        printf("error, index: %d\n", count);
                    }
                    // printf("nsize: %zu, old size: %zu, max tree size: %zu, %zu, %d, %ld, %ld\n", nsize, old_size, t->max_tree_size, DefaultTreeNodeCount*sizeof(struct tree_node), count, index, maxTreeNodeCount);
                }
                if(n!=NULL){
                    memset(n, 0, nsize);
                }else{
            ...
            // printf("error, not fast alloc\n");
            // printf("tree_alloc_node: not fast alloc\n");
            n = (struct tree_node *)calloc2(count, sizeof(*n));
            // n = new struct tree_node[count];
        }
        return n;
    }
     ...
     * This function may be called by multiple threads in parallel. */
    // bo static
    void tree_setup_node(struct tree *t, struct tree_node *n, coord_t coord, int32_t depth)
    {
        // printf("tree set up coord: %d, %d\n", coord, t->hash);
        n->coord = coord;
        n->depth = depth;
        /* n->hash is used only for debugging. It is very likely (but not
         * guaranteed) to be unique. */
        hash_t h = n - (struct tree_node *)0;
        n->hash = (h << 32) + (t->hash++ & 0xffffffff);
        if (depth > t->max_depth){
            t->max_depth = depth;
            // printf("tree_setup_node: max_depth: %d\n", t->max_depth);
        }
    }
     ...
     * This function may be called by multiple threads in parallel. */
    // bo static
    struct tree_node* tree_init_node(struct tree *t, coord_t coord, int32_t depth, bool fast_alloc)
    {
        struct tree_node *n;
    ...
    {
        struct tree* t = (struct tree*)calloc2(1, sizeof(*t));
        t->board = board;
        t->max_tree_size = max_tree_size;
        t->max_pruned_size = max_pruned_size;
        t->pruning_threshold = pruning_threshold;
        if (max_tree_size > 0) {
            // int64_t treeNodeCount = max_tree_size/sizeof(tree_node);
            // t->nodes = new tree_node[treeNodeCount];
            t->nodes = (tree_node*)malloc(max_tree_size);
            if(!t->nodes){
                printf("error, why malloc t->nodes not success\n");
            }
        }
        ...
        t->hbits = hbits;
        if (hbits) t->htable = (struct tree_hash*)uct_htable_alloc(hbits);
        return t;
    }
    ...
    // bo static
    size_t tree_done_node(struct tree *t, struct tree_node *n)
    {
        ...
        if(n!=NULL){
            ...
            printf("error, why node null\n");
        }
        
#ifdef _MSC_VER
        size_t old_size = t->nodes_size;
        t->nodes_size-=sizeof(*n);
#else
    ...
    // bo static
    void* tree_done_node_worker(void *ctx_)
    {
        struct subtree_ctx* ctx = (struct subtree_ctx*)ctx_;
        char str[256];
        coord2str(str, node_coord(ctx->n), ctx->t->board);
        
        size_t tree_size = tree_done_node(ctx->t, ctx->n);
        if (!tree_size)
            free(ctx->t);
        if (DEBUGL(2)){
            printf("done freeing node at %s, tree size %llu\n", str, (uint64_t)tree_size);
        }
        free(ctx);
        return NULL;
    }
    ...
    // bo static
    void tree_done_node_detached(struct tree *t, struct tree_node *n)
    {
        // printf("tree_done_node_detached\n");
    ...
    void tree_done(struct tree *t)
    {
        ...
        if (t->nodes) {
            // printf("tree done, free tree nodes\n");
            free(t->nodes);
            // delete [] t->nodes;
            free(t);
    ...
    // bo static
    void tree_node_dump(struct tree *tree, struct tree_node *node, int32_t treeparity, int32_t l, int32_t thres)
    {
        for (int32_t i = 0; i < l; i++) fputc(' ', stderr);
        int32_t children = 0;
         ...
         * win probability of _us_, not the node color. */
        {
            char strCoord[256];
            {
                coord2sstr(strCoord, node_coord(node), tree->board);
            }
            printf("[%s] %.3f/%d [prior %.3f/%d amaf %.3f/%d crit %.3f vloss %d] h=%x c#=%d <%" PRIhash">\n", strCoord, tree_node_get_value(tree, treeparity, node->u.value), node->u.playouts, tree_node_get_value(tree, treeparity, node->prior.value), node->prior.playouts, tree_node_get_value(tree, treeparity, node->amaf.value), node->amaf.playouts, tree_node_criticality(tree, node), node->descents, node->hints, children, node->hash);
        }
        
        /* Print nodes sorted by #playouts. */
        
        struct tree_node *nbox[1000]; int32_t nboxl = 0;
        for (struct tree_node *ni = node->children; ni; ni = ni->sibling)
    ...
    {
        int32_t thres_abs = thres > 0 ? tree->root->u.playouts * thres : thres;
        {
            char strStone[20];
            {
                stone2str(strStone, tree->root_color);
            }
            printf("(UCT tree; root %s; extra komi %f; max depth %d)\n", strStone, tree->extra_komi, tree->max_depth - tree->root->depth);
        }
    ...
    // bo static
    void tree_book_name(char* buf, struct board *b)
    {
        // printf("tree book name\n");
        // bo static
        // char buf[256];
        if (b->handicap > 0) {
    ...
    // bo static
    void tree_node_save(FILE *f, struct tree_node *node, int32_t thres)
    {
        ...
        fputc(1, f);
        // fwrite(((void *) node) + offsetof(struct tree_node, u), sizeof(struct tree_node) - offsetof(struct tree_node, u), 1, f);
        fwrite(node + offsetof(struct tree_node, u), sizeof(struct tree_node) - offsetof(struct tree_node, u), 1, f);
    ...
    void tree_node_load(FILE *f, struct tree_node *node, int32_t *num)
    {
        (*num)++;
        
        // checked_fread(((void *) node) + offsetof(struct tree_node, u), sizeof(struct tree_node) - offsetof(struct tree_node, u), 1, f);
        checked_fread(node + offsetof(struct tree_node, u), sizeof(struct tree_node) - offsetof(struct tree_node, u), 1, f);
...
#define MAX_PLAYOUTS	10000000
        ...
        while (fgetc(f)) {
            ni_prev = ni;
            ni = (struct tree_node*)calloc2(1, sizeof(*ni));
            if (!node->children)
                node->children = ni;
            else
                ni_prev->sibling = ni;
            ni->parent = node;
            printf("set node parent: %d, %d\n", ni->coord, node->coord);
    ...
    void tree_load(struct tree *tree, struct board *b)
    {
        ...
        printf("Loading opening tbook %s...\n", filename);
    ...
    // bo static
    struct tree_node* tree_prune(struct tree *dest, struct tree *src, struct tree_node *node, int32_t threshold, int32_t depth)
    {
        {
            // assert(dest->nodes && node);
            if(!(dest->nodes && node)){
                printf("error, assert(dest->nodes && node)\n");
            }
        }
        ...
        *n2 = *node;
        ...
        while (ni) {
            ...
            *prev2 = ni2;
            ...
            {
                ni2->parent = n2;
                // printf("tree_prune: set node parent: %d, %d\n", ni2->coord, n2->coord);
            }
            ni = ni->sibling;
        }
        if (!ni) {
...
#define SMALL_TREE_PLAYOUTS 5000
    ...
    {
        {
            ...
            if(!(tree->nodes && !node->parent && !node->sibling)){
                printf("error, assert(tree->nodes && !node->parent && !node->sibling)\n");
            }
        }
        // double start_time = time_now();
        // size_t orig_size = tree->nodes_size;
        
        struct tree *temp_tree = tree_init(tree->board, tree->root_color, tree->max_pruned_size, 0, 0, tree->ltree_aging, 0);
        ...
        int32_t threshold = (node->u.playouts - LARGE_TREE_PLAYOUTS) * DEEP_PLAYOUTS_THRESHOLD / LARGE_TREE_PLAYOUTS;
        ...
        /*if (DEBUGL(1)) {
         ...
         printf("tree pruned in %0.6g s, prev %0.3g s ago, dest depth %d wanted %d, size %llu->%llu/%llu, playouts %d\n", now - start_time, start_time - prev_time, temp_tree->max_depth, max_depth, (unsigned long long)orig_size, (unsigned long long)temp_tree->nodes_size, (unsigned long long)tree->max_pruned_size, new_node->u.playouts);
         prev_time = start_time;
         }*/
        if (temp_tree->nodes_size >= temp_tree->max_tree_size) {
            printf("error, temp tree overflow, max_tree_size %llu, pruning_threshold %llu\n", (uint64_t)tree->max_tree_size, (unsigned long long)tree->pruning_threshold);
        ...
        } else {
            {
                ...
                if(!(tree->nodes_size == temp_tree->nodes_size)){
                    printf("error, assert(tree->nodes_size == temp_tree->nodes_size)\n");
                }
            }
            {
                // assert(tree->max_depth == temp_tree->max_depth);
                if(!(tree->max_depth == temp_tree->max_depth)){
                    printf("error, assert(tree->max_depth == temp_tree->max_depth)\n");
                }
            }
        }
        tree_done(temp_tree);
    ...
    struct tree_node* tree_get_node(struct tree *t, struct tree_node *parent, coord_t c, bool create)
    {
        if (!parent->children || node_coord(parent->children) >= c) {
            ...
            {
                nn->parent = parent;
                printf("tree_get_node: set node parent: %d, %d\n", nn->coord, parent->coord);
            }
            nn->sibling = parent->children;
            parent->children = nn;
            return nn;
        }
            ...
            return ni->sibling;
        {
            ...
            if(!(node_coord(ni) < c)){
                printf("error, assert(node_coord(ni) < c)\n");
            }
        }
        if (!create)
            return NULL;
        ...
        {
            nn->parent = parent;
            printf("tree_get_node1: set node parent: %d, %d\n", nn->coord, parent->coord);
        }
        nn->sibling = ni->sibling; ni->sibling = nn;
        return nn;
    }
    ...
    struct tree_node* tree_lnode_for_node(struct tree *tree, struct tree_node *ni, struct tree_node *lni, int32_t tenuki_d)
    {
        ...
        if (ni->d >= tenuki_d) {
            /* Tenuki, pick a pass lsibling if available. */
            {
                ...
                if(!(lni->parent && lni->parent->children)){
                    printf("error, assert(lni->parent && lni->parent->children)\n");
                }
            }
    ...
    void tree_expand_node(struct tree *t, struct tree_node *node, struct board *b, enum stone color, struct uct *u, int32_t parity)
    {
        // printf("tree expand node: parent coord: %d, %p\n", node->coord, node);
        // printf("tree_expand_node: %p\n", b);
        /* Get a Common Fate Graph distance map from parent node. */
        int32_t* distances = new int32_t[board_size2(b)];
        ...
        struct prior_map map;
        {
            map.b = b;
            map.to_play = color;
            map.parity = tree_parity(t, parity);
            map.distances = distances;
        }
        // Include pass in the prior map.
        struct move_stats* map_prior = new struct move_stats[board_size2(b) + 1];
        map.prior = &map_prior[1];
        bool* map_consider = new bool[board_size2(b) + 1];
        map.consider = &map_consider[1];
        memset(map_prior, 0, sizeof(struct move_stats)*(board_size2(b) + 1));
        memset(map_consider, 0, sizeof(bool)*(board_size2(b) + 1));
        map.consider[pass] = true;
        int32_t child_count = 1; // for pass
        // printf("flen: %d\n", b->flen);
        foreach_free_point(b) {
            {
                // assert(board_at(b, c) == S_NONE);
                if(!(board_at(b, c) == S_NONE)){
                    printf("error, assert(board_at(b, c) == S_NONE)\n");
                }
            }
        ...
        {
            ni->parent = node;
            // printf("tree_expand_node: set node parent: %d, %d\n", ni->coord, node->coord);
        }
        ...
        if (UDEBUGL(6))
        {
            char strCoord[256];
            {
                coord2sstr(strCoord, node_coord(node), b);
            }
            printf("expanding %s within [%d,%d],[%d,%d] %d-%d\n", strCoord, b->symmetry.x1, b->symmetry.y1, b->symmetry.x2, b->symmetry.y2, b->symmetry.type, b->symmetry.d);
        }
        // printf("tree expand node: parent coord1: %d\n", node->coord);
        int32_t child = 1;
        for (int32_t j = b->symmetry.y1; j <= b->symmetry.y2; j++) {
            for (int32_t i = b->symmetry.x1; i <= b->symmetry.x2; i++) {
                if (b->symmetry.d) {
                    int32_t x = b->symmetry.type == SYM_DIAG_DOWN ? board_size(b) - 1 - i : i;
                    if (x > j) {
                        if (UDEBUGL(7))
                        {
                            printf("drop %d,%d\n", i, j);
                        }
                        // TODO bo symmetry tam
                        continue;
                    }
                }
                    ...
                    continue;
                {
                    ...
                    if(!(c != node_coord(node))){
                        printf("error, assert(c != node_coord(node))\n");
                    }
                }
                
                struct tree_node *nj = NULL;
                {
                    if(t->nodes){
                        // printf("make new tree node by + \n");
                        // TODO nj = first_child + child++;
                        nj = &first_child[child++];
                    }else{
                        nj = tree_alloc_node(t, 1, false);
                    }
                }
                tree_setup_node(t, nj, c, node->depth + 1);
                {
                    nj->parent = node;
                    // printf("tree_expand_node: set node parent1: %d, %d, %p\n", nj->coord, node->coord, node);
                }
                ni->sibling = nj;
                ni = nj;
    ...
    // bo static
    coord_t flip_coord(struct board *b, coord_t c, bool flip_horiz, bool flip_vert, int32_t flip_diag)
    {
    ...
    // bo static
    void tree_fix_node_symmetry(struct board *b, struct tree_node *node, bool flip_horiz, bool flip_vert, int32_t flip_diag)
    {
    ...
    // bo static
    void tree_fix_symmetry(struct tree *tree, struct board *b, coord_t c)
    {
        if (is_pass(c))
            return;
        ...
        board_statics* board_statics = &b->board_statics;
        int32_t cx = coord_x(board_statics, c, b);
        int32_t cy = coord_y(board_statics, c, b);
        ...
        if (s->d) {
            ...
            int32_t x = dir ^ flip_horiz ^ flip_vert ? board_size(b) - 1 - cx : cx;
        ...
        if (DEBUGL(4)) {
            char strCoord1[256];
            {
                coord2sstr(strCoord1, c, b);
            }
            char strCoord2[256];
            {
                coord2sstr(strCoord2, flip_coord(b, c, flip_horiz, flip_vert, flip_diag), b);
            }
            printf("%s [%d,%d -> %d,%d;%d,%d] will flip %d %d %d -> %s, sym %d (%d) -> %d (%d)\n", strCoord1, cx, cy, s->x1, s->y1, s->x2, s->y2, flip_horiz, flip_vert, flip_diag, strCoord2, s->type, s->d, b->symmetry.type, b->symmetry.d);
        }
    ...
    // bo static
    void tree_unlink_node(struct tree_node *node)
    {
        ...
        node->sibling = NULL;
        node->parent = NULL;
        // printf("tree_unlink_node: set node parent null: %d\n", node->coord);
    }
    ...
    // bo static
    struct tree_node* tree_age_node(struct tree *tree, struct tree_node *node)
    {
    ...
    {
        {
            ...
            if(!((*node)->parent == tree->root)){
                printf("error, assert((*node)->parent == tree->root)\n");
            }
        }
        tree_unlink_node(*node);
        if (!tree->nodes) {
