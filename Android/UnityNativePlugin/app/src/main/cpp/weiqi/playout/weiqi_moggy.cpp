...
#include "../tatics/weiqi_1lib.hpp"
#include "../tatics/weiqi_ladder.hpp"
#include "../tatics/weiqi_2lib.hpp"
#include "../tatics/weiqi_nlib.hpp"
#include "../tatics/weiqi_nakade.hpp"
#include "../uct/weiqi_prior.hpp"
#include "../tatics/weiqi_seki.hpp"
...
#define PAT3_N 15
    ...
    struct moggy_policy {
        uint32_t lcapturerate, atarirate, nlibrate, ladderrate, capturerate, patternrate, korate, josekirate, nakaderate, eyefixrate;
        uint32_t selfatarirate, eyefillrate, alwaysccaprate;
        uint32_t fillboardtries;
        int32_t koage;
        ...
        int32_t cap_stone_min, cap_stone_max;
        int32_t cap_stone_denom;
    ...
    // tam bo static
    char moggy_patterns_src[PAT3_N][11] = {
...
#define moggy_patterns_src_n sizeof(moggy_patterns_src) / sizeof(moggy_patterns_src[0])
    ...
    // bo static
    int32_t local_atari_check(struct playout_policy *p, struct board *b, struct move *m, struct move_queue *q)
    {
    ...
    // bo static
    void local_ladder_check(struct playout_policy *p, struct board *b, struct move *m, struct move_queue *q)
    {
    ...
    // bo static
    void local_2lib_check(struct playout_policy *p, struct board *b, struct move *m, struct move_queue *q)
    {
        struct moggy_policy* pp = (struct moggy_policy*)p->data;
    ...
    // bo static
    void local_2lib_capture_check(struct playout_policy *p, struct board *b, struct move *m, struct move_queue *q)
    {
        struct moggy_policy* pp = (struct moggy_policy*)p->data;
        group_t group = group_at(b, m->coord), group2 = 0;
        ...
        if (PLDEBUGL(5)){
            char str[] = "Local 2lib capture";
            mq_print(q, b, str);
        }
    }
    
    // bo static
    void local_nlib_check(struct playout_policy *p, struct board *b, struct move *m, struct move_queue *q)
    {
        struct moggy_policy* pp = (struct moggy_policy*)p->data;
        ...
        foreach_8neighbor(board_statics, b, m->coord) {
            group_t g = group_at(b, c);
    ...
    // bo static
    void eye_fix_check(struct playout_policy *p, struct board *b, struct move *m, enum stone to_play, struct move_queue *q)
    {
        ...
        int32_t size = board_size(b);
        int32_t nei8_clockwise[10] = { -size-1, 1, 1, size, size, -1, -1, -size, -size, 1 };
        
        /* This is sort of like a cross between foreach_diag_neighbor
         * and foreach_8neighbor. */
        coord_t c = m->coord;
        for (int32_t dni = 0; dni < 8; dni += 2) {
            ...
            if ((board_at(b, c0) == to_play || board_at(b, c0) == S_OFFBOARD)
                ...
                foreach_neighbor(b, c1, {
                    ...
                    foreach_diag_neighbor(board_statics, b, falsifying) {
    ...
    // bo static
    coord_t nakade_check(struct playout_policy *p, struct board *b, struct move *m, enum stone to_play)
    {
        coord_t empty = pass;
        foreach_neighbor(b, m->coord, {
            if (board_at(b, c) != S_NONE)
        ...
        });
        {
            ...
            if(is_pass(empty)){
                printf("error, assert(!is_pass(empty))\n");
            }
        }
    ...
    // bo static
    inline bool test_pattern3_here(struct playout_policy *p, struct board *b, struct move *m, bool middle_ladder, double *gamma)
    {
        struct moggy_policy* pp = (struct moggy_policy*)p->data;
    ...
    // bo static
    void apply_pattern_here(struct playout_policy *p, struct board *b, coord_t c, enum stone color, struct move_queue *q, fixp_t *gammas)
    {
        struct moggy_policy* pp = (struct moggy_policy*)p->data;
        struct move m2;
        {
    ...
    // bo static
    void apply_pattern(struct playout_policy *p, struct board *b, struct move *m, struct move *mm, struct move_queue *q, fixp_t *gammas)
    {
        ...
        if (mm) { /* Second move for pattern searching */
            foreach_8neighbor(board_statics, b, mm->coord) {
        ...
        if (PLDEBUGL(5)){
            char str[] = "Pattern";
            mq_gamma_print(q, gammas, b, str);
        }
    }
    
    // bo static
    void global_atari_check(struct playout_policy *p, struct board *b, enum stone to_play, struct move_queue *q)
    {
        if (b->clen == 0)
            return;
        ...
        int32_t g_base = fast_random(b->clen);
        for (int32_t g = g_base; g < b->clen; g++) {
            group_atari_check(pp->alwaysccaprate, b, group_at(b, group_base(b->c[g])), to_play, q, NULL, pp->middle_ladder, 1<<MQ_GATARI);
            if (q->moves > 0) {
                /* XXX: Try carrying on. */
                if (PLDEBUGL(5)){
    ...
    // bo static
    void joseki_check(struct playout_policy *p, struct board *b, enum stone to_play, struct move_queue *q)
    {
        struct moggy_policy* pp = (struct moggy_policy*)p->data;
        if (!pp->jdict)
            return;
        ...
        for (int32_t i = 0; i < 4; i++) {
            ...
            if (!cc) continue;
            for (; !is_pass(*cc); cc++) {
                if (coord_quadrant(board_statics, *cc, b) != i)
                    continue;
    ...
    // bo static
    coord_t fillboard_check(struct playout_policy *p, struct board *b)
    {
        struct moggy_policy* pp = (struct moggy_policy*)p->data;
        uint32_t fbtries = b->flen / 8;
        if (pp->fillboardtries < fbtries)
            fbtries = pp->fillboardtries;
        
        board_statics* board_statics = &b->board_statics;
        for (uint32_t i = 0; i < fbtries; i++) {
            coord_t coord = b->f[fast_random(b->flen)];
            if (immediate_liberty_count(b, coord) != 4)
                continue;
            foreach_diag_neighbor(board_statics, b, coord) {
                if (board_at(b, c) != S_NONE)
                    goto next_try;
            } foreach_diag_neighbor_end;
            return coord;
    ...
    // bo static
    coord_t playout_moggy_seqchoose(struct playout_policy *p, struct playout_setup *s, struct board *b, enum stone to_play)
    {
        struct moggy_policy* pp = (struct moggy_policy*)p->data;
        ...
        if (!is_pass(b->last_move.coord)) {
            ...
            if (ps->last_selfatari[other_color] &&
                ...
                struct move m;
                {
                    m.coord = ps->last_selfatari[other_color];
                    m.color = other_color;
                }
    ...
    void playout_moggy_setboard(struct playout_policy *playout_policy, struct board *b)
    {
        /*if (b->ps)
         return;*/
        // printf("playout_moggy_setboard\n");
        /*struct moggy_state* ps = new struct moggy_state;
         ps->last_selfatari[S_BLACK] = ps->last_selfatari[S_WHITE] = 0;
         b->ps = ps;*/
        memset(&b->ps, 0, sizeof(struct moggy_state));
        b->ps.last_selfatari[S_BLACK] = b->ps.last_selfatari[S_WHITE] = 0;
    }
    
    // bo static
    void playout_moggy_assess_group(struct playout_policy *p, struct prior_map *map, group_t g, int32_t games)
    {
        struct moggy_policy* pp = (struct moggy_policy*)p->data;
        ...
        if (PLDEBUGL(5)) {
            {
                char strCoord[256];
                {
                    coord2sstr(strCoord, g, b);
                }
                printf("ASSESS of group %s:\n", strCoord);
            }
            board_print(b, stderr);
        }
        ...
        while (q.moves--) {
            ...
            if (pp->cap_stone_denom > 0) {
                int32_t stones = group_stone_count(b, g, pp->cap_stone_max) - (pp->cap_stone_min-1);
    ...
    // bo static
    void playout_moggy_assess_one(struct playout_policy *p, struct prior_map *map, coord_t coord, int32_t games)
    {
        struct moggy_policy* pp = (struct moggy_policy*)p->data;
        struct board *b = map->b;
        ...
        if (pp->selfatarirate) {
            if (!board_playing_ko_threat(b) && is_bad_selfatari(b, map->to_play, coord)) {
                ...
                if (PLDEBUGL(5)){
                    ...
                    printf("1.0: self-atari redirect %s\n", strCoord);
                }
    ...
    // bo static
    void playout_moggy_assess(struct playout_policy* p, struct prior_map *map, int32_t games)
    {
        struct moggy_policy* pp = (struct moggy_policy*)p->data;
...
#define permit_move(c)  playout_permit(p, b, c, m->color)
    ...
    // bo static
    bool playout_moggy_permit(struct playout_policy *p, struct board *b, struct move *m, bool alt)
    {
        struct moggy_policy* pp = (struct moggy_policy*)p->data;
        struct moggy_state* ps = &b->ps;
        ...
        int32_t bad_selfatari = (pp->selfatarirate > fast_random(100) ?
        ...
        if (bad_selfatari) {
            if (PLDEBUGL(5)){
                char strStone[20];
                {
                    stone2str(strStone, m->color);
                }
                char strCoord[256];
                {
                    coord2sstr(strCoord, m->coord, b);
                }
                printf("__ Prohibiting self-atari %s %s\n", strStone, strCoord);
            }
            if (alt && pp->selfatari_other) {
                ...
                if (PLDEBUGL(5)){
                    ...
                    printf("___ Redirecting to other lib %s\n", strCoord);
                }
                m->coord = c;
                return true;
            }
        ...
        if (!alt || fast_random(100) >= pp->eyefillrate) {
            if (PLDEBUGL(5))
                printf("skipping eyefill test\n");
            // TODO goto eyefill_skip;
            {
                    ...
                    case 1: /* Capture! */
                        ...
                        if (permit_move(c)) {
                            if (PLDEBUGL(5)){
                                ...
                                printf("___ Redirecting to capture %s\n", strCoord);
                            }
                            m->coord = c;
                            return true;
                        }
     ...
     * based on their tags. */
    // bo static
    coord_t mq_tagged_choose(struct playout_policy *p, struct board *b, enum stone to_play, struct move_queue *q)
    {
        struct moggy_policy* pp = (struct moggy_policy*)p->data;
        ...
        for (uint32_t i = 0; i < q->moves; i++) {
            for (uint32_t j = i + 1; j < q->moves; j++) {
        ...
        fixp_t total = 0;
        fixp_t* pd = new fixp_t[q->moves];
        for (uint32_t i = 0; i < q->moves; i++) {
            double val = 1.0;
            {
                // assert(q->tag[i] != 0);
                if(!(q->tag[i] != 0)){
                    printf("error, assert(q->tag[i] != 0)\n");
                }
            }
            for (int32_t j = 0; j < MQ_MAX; j++)
                if (q->tag[i] & (1<<j)) {
        ...
        if (PLDEBUGL(5)) {
            printf("Pick (total %.3f stab %.3f): ", fixp_to_double(total), fixp_to_double(stab));
            for (uint32_t i = 0; i < q->moves; i++) {
                char strCoord[256];
                {
                    coord2sstr(strCoord, q->move[i], b);
                }
                printf("%s(%x:%.3f) ", strCoord, q->tag[i], fixp_to_double(pd[i]));
            }
            printf("\n");
        }
        for (uint32_t i = 0; i < q->moves; i++) {
        ...
        /* Tenuki. */
        {
            ...
            if(!(stab < double_to_fixp(pp->tenuki_prob))){
                printf("error, assert(stab < double_to_fixp(pp->tenuki_prob))\n");
            }
        }
        delete [] pd;
        return pass;
    ...
    // bo static
    coord_t playout_moggy_fullchoose(struct playout_policy *p, struct playout_setup *s, struct board *b, enum stone to_play)
    {
        struct moggy_policy* pp = (struct moggy_policy*)p->data;
        struct move_queue q;
        q.moves = 0;
    ...
    struct playout_policy* playout_moggy_init(char *arg, struct board *b, struct joseki_dict *jdict)
    {
        ...
        struct moggy_policy* pp = (struct moggy_policy*)calloc2(1, sizeof(*pp));
        p->data = pp;
        ...
        int32_t rate = board_large(b) ? 80 : 90;
        ...
        if (arg) {
            ...
            while (*next) {
                ...
                if (!strcasecmp(optname, "debug") && optval) {
                    ...
                    for (int32_t i = 0; *optval && i < MQ_MAX; i++) {
                    ...
                    for (int32_t i = 0; *optval && i < PAT3_N; i++) {
                    ...
                    printf("playout-moggy: Invalid policy argument %s or missing value\n", optname);
                    exit(1);
                }
            }
        }
