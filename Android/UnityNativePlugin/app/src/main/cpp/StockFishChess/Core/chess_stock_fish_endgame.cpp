...
#include "chess_stock_fish_endgame.hpp"
...
namespace StockFishChess
{
    namespace {
        ...
        const int32_t PushToCorners[SQUARE_NB] = {
    ...
    template<> Value Endgame<KPK>::operator()(const Position& pos) const {
        ...
        // assert(verify_material(pos, weakSide, VALUE_ZERO, 0));
        if(!(verify_material(pos, weakSide, VALUE_ZERO, 0))){
            printf("error, verify_material(pos, weakSide, VALUE_ZERO, 0)\n");
        }
    ...
    template<> ScaleFactor Endgame<KBPsK>::operator()(const Position& pos) const {
        ...
        if(!(pos.non_pawn_material(strongSide) == BishopValueMg)){
            printf("error, assert(pos.non_pawn_material(strongSide) == BishopValueMg)\n");
        }
        // assert(pos.count<PAWN>(strongSide) >= 1);
        if(!(pos.count<PAWN>(strongSide) >= 1)){
            printf("error, assert(pos.count<PAWN>(strongSide) >= 1)\n");
        ...
        if(    (pawnsFile == FILE_B || pawnsFile == FILE_G)
           ...
           && pos.count<PAWN>(weakSide) >= 1) {
            ...
            {
                int32_t strongKingDist = distance(weakPawnSq, strongKingSq);
                int32_t weakKingDist = distance(weakPawnSq, weakKingSq);
    ...
    template<> ScaleFactor Endgame<KQKRPs>::operator()(const Position& pos) const {
        ...
        if(!(pos.count<ROOK>(weakSide) == 1)){
            printf("error, assert(pos.count<ROOK>(weakSide) == 1)\n");
        }
        // assert(pos.count<PAWN>(weakSide) >= 1);
        if(!(pos.count<PAWN>(weakSide) >= 1)){
            printf("error, assert(pos.count<PAWN>(weakSide) >= 1)\n");
        }
    ...
    template<> ScaleFactor Endgame<KRPKB>::operator()(const Position& pos) const {
        ...
        if (pos.pieces(PAWN) & (FileABB | FileHBB)) {
            ...
            if (rk == RANK_5 && !opposite_colors(bsq, psq)) {
                int32_t d = distance(psq + 3 * push, ksq);
    ...
    template<> ScaleFactor Endgame<KPsK>::operator()(const Position& pos) const {
        ...
        if(!(pos.non_pawn_material(strongSide) == VALUE_ZERO)){
            printf("error, assert(pos.non_pawn_material(strongSide) == VALUE_ZERO)\n");
        }
        // assert(pos.count<PAWN>(strongSide) >= 2);
        if(!(pos.count<PAWN>(strongSide) >= 2)){
            printf("error, assert(pos.count<PAWN>(strongSide) >= 2)\n");
        }
        // assert(verify_material(pos, weakSide, VALUE_ZERO, 0));
        if(!(verify_material(pos, weakSide, VALUE_ZERO, 0))){
            printf("error, assert(verify_material(pos, weakSide, VALUE_ZERO, 0))\n");
        ...
        if (   !(pawns & ~forward_ranks_bb(weakSide, ksq))
            ...
            &&  distanceT<File>(ksq, lsb(pawns)) <= 1)
            return SCALE_FACTOR_DRAW;
    ...
    template<> ScaleFactor Endgame<KNPK>::operator()(const Position& pos) const {
        
        // assert(verify_material(pos, strongSide, KnightValueMg, 1));
        if(!(verify_material(pos, strongSide, KnightValueMg, 1))){
            printf("error, assert(verify_material(pos, strongSide, KnightValueMg, 1))\n");
        }
        // assert(verify_material(pos, weakSide, VALUE_ZERO, 0));
        if(!(verify_material(pos, weakSide, VALUE_ZERO, 0))){
            printf("error, assert(verify_material(pos, weakSide, VALUE_ZERO, 0))\n");
    ...
    /*template struct Endgame<KNNK, Value>;
    template struct Endgame<KXK, Value>;
    template struct Endgame<KBNK, Value>;
    template struct Endgame<KPK, Value>;
    template struct Endgame<KRKP, Value>;
    template struct Endgame<KRKB, Value>;
    template struct Endgame<KRKN, Value>;
    template struct Endgame<KQKP, Value>;
    template struct Endgame<KQKR, Value>;
    template struct Endgame<KBPsK, Value>;
    template struct Endgame<KQKRPs, Value>;
    template struct Endgame<KRPKR, Value>;
    template struct Endgame<KRPKB, Value>;
    template struct Endgame<KRPPKRP, Value>;
    template struct Endgame<KPsK, Value>;
    template struct Endgame<KBPKB, Value>;
    template struct Endgame<KBPPKB, Value>;
    template struct Endgame<KBPKN, Value>;
    template struct Endgame<KNPK, Value>;
    template struct Endgame<KNPKB, Value>;
    template struct Endgame<KPKP, Value>;*/
    
    /*template struct Endgame<KNNK, ScaleFactor>;
    template struct Endgame<KXK, ScaleFactor>;
    template struct Endgame<KBNK, ScaleFactor>;
    template struct Endgame<KPK, ScaleFactor>;
    template struct Endgame<KRKP, ScaleFactor>;
    template struct Endgame<KRKB, ScaleFactor>;
    template struct Endgame<KRKN, ScaleFactor>;
    template struct Endgame<KQKP, ScaleFactor>;
    template struct Endgame<KQKR, ScaleFactor>;
    template struct Endgame<KBPsK, ScaleFactor>;
    template struct Endgame<KQKRPs, ScaleFactor>;
    template struct Endgame<KRPKR, ScaleFactor>;
    template struct Endgame<KRPKB, ScaleFactor>;
    template struct Endgame<KRPPKRP, ScaleFactor>;
    template struct Endgame<KPsK, ScaleFactor>;
    template struct Endgame<KBPKB, ScaleFactor>;
    template struct Endgame<KBPPKB, ScaleFactor>;
    template struct Endgame<KBPKN, ScaleFactor>;
    template struct Endgame<KNPK, ScaleFactor>;
    template struct Endgame<KNPKB, ScaleFactor>;
    template struct Endgame<KPKP, ScaleFactor>;*/
}
