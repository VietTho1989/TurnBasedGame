...
#ifndef CHESS_STOCK_FISH_MOVEPICK_H_INCLUDED
#define CHESS_STOCK_FISH_MOVEPICK_H_INCLUDED
...
namespace StockFishChess
{
    ...
    template<int32_t Size1, int32_t Size2, int32_t Size3, typename T = int16_t> struct StatCubes : public std::array<std::array<std::array<T, Size3>, Size2>, Size1> {
        ...
        void update(T& entry, int32_t bonus, const int32_t D, const int32_t W) {
            ...
            if(!(abs(entry) <= W * D)){
                printf("error, stockfish assert(abs(entry) <= W * D)\n");
            }
        }
    };
