...
namespace Shogi {
    ...
    inline int32_t abs(const File f) { return std::abs(static_cast<int32_t>(f)); }
    ...
    inline int32_t abs(const Rank r) { return std::abs(static_cast<int32_t>(r)); }
    ...
    // inline bool isInFile(const File f) { return f < FileNum; }
    
    inline bool isInRank(const Rank r) { return (0 <= r) && (r < RankNum); }
    // inline bool isInRank(const Rank r) { return r < RankNum; }
    
    // s が Square の中に含まれているか判定
    inline bool isInSquare(const Square s) { return (0 <= s) && (s < SquareNum); }
    // inline bool isInSquare(const Square s) { return s < SquareNum; }
    ...
    inline Rank makeRank(const Square s) {
        // assert(isInSquare(s));
        if(!(isInSquare(s))){
            printf("error, assert(isInSquare(s))\n");
        }
    ...
    extern int32_t SquareDistance[SquareNum][SquareNum];
    inline int32_t squareDistance(const Square sq1, const Square sq2) { return SquareDistance[sq1][sq2]; }
    ...
    inline bool isAligned(const Square from, const Square to, const Square ksq) {
        ...
        if (FROM_KSQ_NEVER_BE_DIRECMISC) {
            ...
            if(!(direc != DirecMisc)){
                printf("error, assert(direc != DirecMisc)\n");
            }
    ...
    inline void squareToStringUSI(char* ret, const Square sq) {
        ret[0] = 0;
        const Rank r = makeRank(sq);
        const File f = makeFile(sq);
        sprintf(ret, "%s%c%c", ret, fileToCharUSI(f), rankToCharUSI(r));
    }
    ...
    inline constexpr Square inverseIfWhite(const Color c, const Square sq) { return (Square) (c == Black ? sq : inverse(sq)); }
