...
namespace Shogi
{
    ...
    namespace {
        EvalSum doapc(const Position& pos, const EvalIndex index[2]) {
            ...
            EvalSum sum;
            sum.p[2][0] = Evaluator::allocated ? Evaluator::KKP[sq_bk][sq_wk][index[0]][0] : 0;
            sum.p[2][1] = Evaluator::allocated ? Evaluator::KKP[sq_bk][sq_wk][index[0]][1] : 0;
            const auto* pkppb = Evaluator::allocated ? Evaluator::KPP[sq_bk         ][index[0]] : NULL;
            const auto* pkppw = Evaluator::allocated ? Evaluator::KPP[inverse(sq_wk)][index[1]] : NULL;
            ...
            if(Evaluator::allocated){
                ...
                // printf("error, doapc: evaluator not allocated\n");
            }
#endif
        ...
        std::array<s32, 2> doablack(const Position& pos, const EvalIndex index[2]) {
            ...
            if(Evaluator::allocated){
                ...
                // printf("error, doablack: evalutator not allocated\n");
        ...
        std::array<s32, 2> doawhite(const Position& pos, const EvalIndex index[2]) {
            ...
            if(Evaluator::allocated){
                ...
                // printf("error, doawhite: evalutator not allocated\n");
                std::array<s32, 2> sum = {{0, 0}};
                return sum;
            }
        }
        ...
        Score inaniwaScoreBody(const Position& pos) {
            ...
            if (pos.csearcher()->inaniwaFlag == InaniwaIsBlack) {
                ...
                // TODO assert(pos.csearcher()->inaniwaFlag == InaniwaIsWhite);
                if(!(pos.csearcher()->inaniwaFlag == InaniwaIsWhite)){
                    printf("error, inaniwaScoreBody: assert(pos.csearcher()->inaniwaFlag == InaniwaIsWhite)\n");
                }
        ...
        bool calcDifference(Position& pos, SearchStack* ss) {
            ...
            // TODO assert(lastMove != Move::moveNull());
            if(!(lastMove != Move::moveNull())){
                printf("error, calcDifference: lastMove != Move::moveNull()\n");
            }
            ...
            if (lastMove.pieceTypeFrom() == King) {
                ...
                if (pos.turn() == Black) {
                    const auto* ppkppw = Evaluator::allocated ? Evaluator::KPP[inverse(sq_wk)] : NULL;
                    ...
                    if(Evaluator::allocated){
                        ...
                        // printf("error, calcDifference: evaluator not allocated\n");
                    }
                    
                    if (pos.cl().size == 2) {
                        const int32_t listIndex_cap = pos.cl().listindex[1];
                    ...
                    const auto* ppkppb = Evaluator::allocated ? Evaluator::KPP[sq_bk         ] : NULL;
                ...
                const int32_t listIndex = pos.cl().listindex[0];
                ...
                if (pos.cl().size == 1) {
                    ...
                    if(!(pos.cl().size == 2)){
                        printf("error, assert(pos.cl().size == 2)\n");
                    }
        ...
        int32_t make_list_unUseDiff(const Position& pos, int32_t list0[EvalList::ListSize], int32_t list1[EvalList::ListSize], int32_t nlist) {
            auto func = [&](const Bitboard& posBB, const int32_t f_pt, const int32_t e_pt) {
                Square sq;
                Bitboard bb;
        ...
        void evaluateBody(Position& pos, SearchStack* ss) {
            if (calcDifference(pos, ss)) {
                if(Evaluator::allocated){
                    ...
                    // printf("error, evaluateBody: evaluator not allocated\n");
                }
                return;
            }
            ...
            // TODO assert(evaluateUnUseDiff(pos) == sum.sum(pos.turn()));
            if(!(evaluateUnUseDiff(pos) == sum.sum(pos.turn()))){
                printf("error, assert(evaluateUnUseDiff(pos) == sum.sum(pos.turn()))\n");
            }
        }
    }
    ...
    Score evaluateUnUseDiff(const Position& pos) {
        // printf("evaluateUnUseDiff\n");
        int32_t list0[EvalList::ListSize];
        int32_t list1[EvalList::ListSize];
        ...
        auto func = [&](const Hand hand, const HandPiece hp, const int32_t list0_index, const int32_t list1_index) {
...
#endif
        Score ret = static_cast<Score>(score.sum(pos.turn()));
        // printf("evaluateUnUseDiff return: %d\n", ret);
        return ret;
    }
    ...
    Score evaluate(Position& pos, SearchStack* ss) {
        if (ss->staticEvalRaw.p[0][0] != ScoreNotEvaluated) {
            ...
            if(!(score == evaluateUnUseDiff(pos))){
                // printf("error, assert(score == evaluateUnUseDiff(pos))\n");
            }
        ...
        if (entry.key == keyExcludeTurn) {
            ...
            // TODO assert(static_cast<Score>(ss->staticEvalRaw.sum(pos.turn())) == evaluateUnUseDiff(pos));
            if(!(static_cast<Score>(ss->staticEvalRaw.sum(pos.turn())) == evaluateUnUseDiff(pos))){
                // printf("error, assert(static_cast<Score>(ss->staticEvalRaw.sum(pos.turn())) == evaluateUnUseDiff(pos))\n");
            }
        ...
        *g_evalTable[keyExcludeTurn] = ss->staticEvalRaw;
        Score ret = static_cast<Score>(ss->staticEvalRaw.sum(pos.turn())) / FVScale;
        // printf("evaluatePosition return: %d\n", ret);
        return ret;
    }
