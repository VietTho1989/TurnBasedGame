...
namespace Shogi
{
    ...
    void Move::toUSI(char* ret)
    {
        ...
        if (this->isDrop()){
            char strSquare[20];
            {
                squareToStringUSI(strSquare, to);
            }
            sprintf(ret, "%s%s%s", ret, handPieceToString(this->handPieceDropped()).c_str(), strSquare);
            return;
        }
        {
            // Square From, To
            {
                char strSquareFrom[20];
                {
                    squareToStringUSI(strSquareFrom, from);
                }
                char strSquareTo[20];
                {
                    squareToStringUSI(strSquareTo, to);
                }
                sprintf(ret, "%s%s%s", ret, strSquareFrom, strSquareTo);
            }
            if (this->isPromotion()){
                sprintf(ret, "%s+", ret);
            }
