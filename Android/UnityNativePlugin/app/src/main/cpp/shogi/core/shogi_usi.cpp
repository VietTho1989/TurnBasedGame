...
#include "shogi_benchmark.hpp"
#include "shogi_learner.hpp"
...
{
    namespace {
        void onThreads(Searcher* s, const USIOption&)
        {
            // s->threads.readUSIOptions(s);
        }
    ...
    bool CaseInsensitiveLess::operator () (const std::string& s1, const std::string& s2) const {
        for (size_t i = 0; i < s1.size() && i < s2.size(); ++i) {
            const int32_t c1 = tolower(s1[i]);
            const int32_t c2 = tolower(s2[i]);
            if (c1 != c2)
                return c1 < c2;
        }
    ...
    inline void printEvalTable(const Square ksq, const int32_t p0, const int32_t p1_base, const bool isTurn) {
    ...
    namespace {
        // 論理的なコア数の取得
        inline int32_t cpuCoreCount() {
            // std::thread::hardware_concurrency() は 0 を返す可能性がある。
            // HyperThreading が有効なら論理コア数だけ thread 生成した方が強い。
            return max(static_cast<int32_t>(std::thread::hardware_concurrency()), 1);
        }
    ...
    void OptionsMap::init(Searcher* s) {
        const int32_t MaxHashMB = 1024 * 1024;
    ...
    USIOption::USIOption(const int32_t v, const int32_t min, const int32_t max, Fn* f, Searcher* s)
    ...
    USIOption& USIOption::operator = (const std::string& v) {
        // assert(!type_.empty());
        if(type_.empty()){
            printf("error, assert(!type_.empty())\n");
        }
    ...
    void randomMove(Position& pos, std::mt19937& mt) {
        ...
        switch (dist(mt)) {
            ...
            case 1: { // 玉も含めた全ての合法手
                bool moved = false;
                for (int32_t i = 0; i < dist(mt) + 1; ++i) { // 自分だけ、または両者ランダムに1手指してみる。
                    MoveList<LegalAll> ml(pos);
                    if (ml.size()) {
                        std::uniform_int_distribution<int> moveDist(0, ml.size()-1);
                        pos.doMove((ml.begin() + moveDist(mt))->move, *st++);
    ...
    void make_teacher(std::istringstream& ssCmd) {
        ...
        auto func = [&omutex, &ofs, &imutex, &ifs, &inputFileDist, &teacherNodes](Position& pos, std::atomic<s64>& idx, const int32_t threadID) {
            ...
            while (idx < teacherNodes) {
                ...
                for (Ply ply = pos.gamePly(); ply < 400; ++ply, ++idx) { // 400 手くらいで終了しておく。
                    ...
                    const int32_t ScoreThresh = 10000; // 自己対局を決着がついたとして止める閾値
        ...
        auto progressFunc = [&teacherNodes] (std::atomic<s64>& index, Timer& t) {
            while (true) {
                ...
                if (progress > 0.0) // 0 除算を回避する。
                    std::cout << std::fixed << "Progress: " << std::setprecision(2) << min(100.0, progress * 100.0)
                    << "%, Elapsed: " << elapsed_msec/1000
                    << "[s], Remaining: " << max<s64>(0, elapsed_msec*(1.0 - progress)/(progress*1000)) << "[s]" << std::endl;
...
#pragma omp for
#endif
                for (int32_t ksq = SQ11; ksq < SquareNum; ++ksq) {
...
#pragma omp for
#endif
                for (int32_t ksq0 = SQ11; ksq0 < SquareNum; ++ksq0) {
...
#pragma omp for
        ...
        //            for (int32_t boardTurn = 0; boardTurn < 2; ++boardTurn)
...
#pragma omp for
                ...
                for (int32_t ksq = SQ11; ksq < SquareNoLeftNum; ++ksq) { // 5筋より左は使わない。
                    for (EvalIndex i = (EvalIndex)0; i < fe_end; ++i) {
...
#pragma omp for
        ...
        struct TeacherBuffer {
            ...
            void fill(std::ifstream& ifs) {
                ...
                if(!(state == WaitFilling)){
                    printf("error, assert(state == WaitFilling)\n");
                }
    ...
    Move usiToMove(const Position& pos, const std::string& moveStr) {
        ...
        if(!(move == usiToMoveDebug(pos, moveStr))){
            printf("error, assert(move == usiToMoveDebug(pos, moveStr))\n");
        }
    ...
    Move csaToMove(const Position& pos, const std::string& moveStr) {
        ...
        if(!(move == csaToMoveDebug(pos, moveStr))){
            printf("error, assert(move == csaToMoveDebug(pos, moveStr))\n");
        }
    ...
    void measureGenerateMoves(const Position& pos) {
        printf("measureGenerateMoves\n");
        pos.print();
        
        ExtMove legalMoves[MaxLegalMoves];
        for (int32_t i = 0; i < MaxLegalMoves; ++i) legalMoves[i].move = moveNone();
        ExtMove* pms = &legalMoves[0];
        ...
        const int32_t elapsed = t.elapsed();
    ...
    /*void Searcher::doUSICommandLoop(int32_t argc, char* argv[]) {
